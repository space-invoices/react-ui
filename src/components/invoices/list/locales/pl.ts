export default {
  Draft: "Wersja robocza",
  Number: "Numer",
  Customer: "Klient",
  Date: "Data",
  "Date Due": "Termin płatności",
  Total: "Razem",
  "Total with Tax": "Razem z VAT",
  Actions: "Akcje",
  "Open menu": "Otwórz menu",
  "Copy invoice ID": "Kopiuj ID faktury",
  "View invoice": "Pokaż fakturę",
  "Add Payment": "Dodaj płatność",
  "Download PDF": "Pobierz PDF",
  "Downloading...": "Pobieranie...",
  Duplicate: "Duplikuj",
  "Send Email": "Wyślij e-mail",
  "Send reminder": "Wyślij przypomnienie",
  Invoice: "Faktura",
  "Failed to download PDF": "Nie udało się pobrać PDF",
  // Filter translations
  "Search...": "Szukaj...",
  Filters: "Filtry",
  "Date Range": "Zakres dat",
  From: "Od",
  To: "Do",
  "Clear filters": "Wyczyść filtry",
  Status: "Status",
  Paid: "Opłacona",
  Unpaid: "Nieopłacona",
  Overdue: "Przeterminowana",
  Voided: "Anulowana",
  "Created At": "Utworzono",
  Unshare: "Cofnij udostępnienie",
  // Empty state translations
  "Your list is empty": "Lista faktur jest pusta",
  "Get started by creating your first entry": "Zacznij od utworzenia pierwszej faktury",
  "Create new": "Utwórz fakturę",
  // No results translations
  "No results found": "Nie znaleziono faktur",
  "Try adjusting your search criteria": "Spróbuj dostosować kryteria wyszukiwania",
  "Clear search": "Wyczyść wyszukiwanie",
  "Clear all": "Wyczyść wszystko",
  selected: "wybranych",
  "Export PDFs": "Eksportuj PDF-y",
  "Deselect all": "Odznacz wszystko",
  Void: "Anuluj",
  "Partially Paid": "Częściowo opłacona",
  "FURS fiscalization failed": "Fiskalizacja FURS nie powiodła się",
  "FINA fiscalization failed": "Fiskalizacja FINA nie powiodła się",
  "Retry Fiscalization": "Spróbuj fiskalizacji ponownie",
  "Some selected documents don't have failed fiscalization":
    "Niektóre wybrane dokumenty nie mają nieudanej fiskalizacji",
} as const;
