export default {
  Date: "Datum",
  Amount: "Znesek",
  Type: "Vrsta",
  Invoice: "Račun",
  Document: "Dokument",
  Reference: "Referenca",
  Note: "Pripis",
  Actions: "Akcije",
  "Open menu": "Odpri meni",
  "Edit payment": "Uredi plačilo",
  "Delete payment": "Izbriši plačilo",
  "Deleting...": "Brišem...",
  "View invoice": "Prikaži račun",
  "type.cash": "Gotovina",
  "type.bank_transfer": "Bančno nakazilo",
  "type.card": "Kartica",
  "type.check": "Ček",
  "type.other": "Drugo",
  cash: "Gotovina",
  bank_transfer: "Bančno nakazilo",
  card: "Kartica",
  check: "Ček",
  credit_note: "Dobropis",
  advance_payment: "Avansno plačilo",
  other: "Drugo",
  Cash: "Gotovina",
  "Bank Transfer": "Bančno nakazilo",
  Card: "Kartica",
  Check: "Ček",
  "Credit note": "Dobropis",
  "Advance payment": "Avansno plačilo",
  Other: "Drugo",
  "Auto-applied credit note for voided invoice {number}": "Samodejno uveljavljen dobropis za storniran račun {number}",
  "Automatic billing payment via Stripe": "Samodejno obračunano plačilo prek Stripe",
  // Empty state translations
  "Your list is empty": "Vaš seznam plačil je prazen",
  "Get started by creating your first entry": "Začnite z dodajanjem prvega plačila",
  "No results found": "Ni najdenih plačil",
  "Try adjusting your search criteria": "Poskusite prilagoditi iskalne kriterije",
  "Clear search": "Počisti iskanje",
  "Clear all": "Počisti vse",
} as const;
