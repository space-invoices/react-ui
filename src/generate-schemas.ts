...
const STRING_LENGTH_CONSTRAINT_OPERATION_IDS = new Set(["createEntity", "createTax", "replaceTax"]);
...
`;
  ...
  // Compatibility defaults come from the API policy owner, never a parallel UI matrix.
  const { getCountryCapabilityFallbacks } = await import("../../../apps/api/src/config/country-rules");
  await fs.writeFile(
    path.join(STAGING_SCHEMAS_DIR, "country-capability-defaults.ts"),
    `// @generated by generate-schemas.ts from API country rules. Do not edit.\nexport const countryCapabilityDefaults = ${JSON.stringify(getCountryCapabilityFallbacks(), null, 2)};\n`,
  );

  const { PT_EXEMPTION_DEFAULT_REASONS } = await import(
    "../../../apps/api/src/modules/fiscalization/pt/pt-exemption-defaults"
  );
  await fs.writeFile(
    path.join(STAGING_SCHEMAS_DIR, "pt-exemption-defaults.ts"),
    `// @generated by generate-schemas.ts from the API PT exemption catalogue. Do not edit.\nexport const ptExemptionDefaultReasons: Readonly<Record<string, string>> = ${JSON.stringify(PT_EXEMPTION_DEFAULT_REASONS, null, 2)};\n`,
  );
