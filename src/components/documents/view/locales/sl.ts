export default {
  // Actions
  PDF: "PDF",
  Send: "Pošlji",
  Payment: "Plačilo",
  Share: "Deli",
  Unshare: "Odstrani deljenje",
  Copied: "Kopirano",
  "Copy shareable link": "Kopiraj povezavo za deljenje",
  "Unshare document": "Odstrani deljenje dokumenta",
  "Unshare document confirmation": "Ali ste prepričani, da želite odstraniti javni dostop do tega dokumenta?",
  "Document unshared": "Dokument ni več deljen",
  "Unshare failed": "Odstranitev deljenja ni uspela",
  "Download failed": "Prenos ni uspel",
  "e-SLOG download failed": "Prenos e-SLOG ni uspel",
  "UJP package": "UJP paket",
  "UJP package download failed": "Prenos UJP paketa ni uspel",
  XRechnung: "XRechnung",
  ZUGFeRD: "ZUGFeRD",
  "XRechnung download failed": "Prenos XRechnung ni uspel",
  "ZUGFeRD download failed": "Prenos ZUGFeRD ni uspel",
  "e-SLOG validation must be valid before downloading.": "Validacija e-SLOG mora biti uspešna pred prenosom.",
  "UJP validation must be valid before downloading.": "Validacija UJP mora biti uspešna pred prenosom.",
  "German e-invoice validation must be valid before downloading.":
    "Validacija nemškega e-računa mora biti pred prenosom veljavna.",
  "XRechnung validation must be valid before downloading.": "Validacija XRechnung mora biti pred prenosom veljavna.",
  "ZUGFeRD validation must be valid before downloading.": "Validacija ZUGFeRD mora biti pred prenosom veljavna.",
  Recurring: "Ponavljajoče",
  Duplicate: "Podvoji",
  "Duplicate invoice": "Podvoji račun",
  "Duplicate estimate": "Podvoji ponudbo",
  "Duplicate credit_note": "Podvoji dobropis",
  "Duplicate advance_invoice": "Podvoji avansni račun",
  "Duplicate delivery_note": "Podvoji dobavnico",
  "Create invoice": "Ustvari račun",
  "Create estimate": "Ustvari ponudbo",
  "Create credit_note": "Ustvari dobropis",
  "Create advance_invoice": "Ustvari avansni račun",
  "Create delivery_note": "Ustvari dobavnico",

  // Details card
  Details: "Podrobnosti",
  Number: "Številka",
  Date: "Datum",
  "Due date": "Datum zapadlosti",
  "Service date": "Datum storitve",
  "Service period": "Obdobje storitve",
  "Valid until": "Veljavno do",
  Customer: "Stranka",
  Discount: "Popust",
  Subtotal: "Vmesni seštevek",
  Tax: "DDV",
  of: "od",
  Total: "Skupaj",
  Paid: "Plačano",
  Due: "Za plačilo",
  Status: "Status",

  // Payment status
  "Paid in full": "Plačano v celoti",
  "Partially paid": "Delno plačano",
  Unpaid: "Neplačano",
  Voided: "Stornirano",
  Overdue: "Zapadlo",
  Shareable: "Deljivo",
  "Proforma invoice": "Predračun",
  "Packing list": "Pakirni list",
  "FURS fiscalization failed": "FURS fiskalizacija ni uspela",
  "FINA fiscalization failed": "FINA fiskalizacija ni uspela",

  // Payments list
  Payments: "Plačila",
  Refunds: "Vračila",
  "No payments": "Ni plačil",
  "No refunds": "Ni vračil",
  "Add payment": "Dodaj plačilo",
  Edit: "Uredi",
  Delete: "Izbriši",
  "Delete payment": "Izbriši plačilo",
  "Delete payment confirmation": "Ali ste prepričani, da želite izbrisati to plačilo?",
  Cancel: "Prekliči",

  // Payment types
  cash: "Gotovina",
  bank_transfer: "Bančno nakazilo",
  card: "Kartica",
  check: "Ček",
  credit_note: "Dobropis",
  other: "Drugo",
  advance_invoice: "Avansni račun",
  advance_payment: "Avansno plačilo",

  // Errors
  "Delete failed": "Brisanje ni uspelo",

  // Draft actions
  Finalize: "Izdaj",
  "Delete Draft": "Izbriši osnutek",
  "Finalize document": "Izdaj dokument",
  "Finalize document confirmation":
    "S tem boste dokumentu dodelili končno številko. Po izdaji dokumenta ga ni več mogoče izbrisati. Ali želite nadaljevati?",
  "Delete draft document": "Izbriši osnutek dokumenta",
  "Delete draft confirmation":
    "Ali ste prepričani, da želite izbrisati ta osnutek? Tega dejanja ni mogoče razveljaviti.",
  "Document finalized": "Dokument je bil izdan",
  "Finalize failed": "Izdaja ni uspela",
  "Draft deleted": "Osnutek je bil izbrisan",
  "Delete draft failed": "Brisanje osnutka ni uspelo",
  Draft: "Osnutek",

  // Void
  Void: "Storniraj",
  "Voiding...": "Storniranje...",

  // Activity list
  Activity: "Aktivnost",
  "No activity": "Ni aktivnosti",
  Created: "Ustvarjeno",
  Updated: "Posodobljeno",
  Sent: "Poslano",
  Deleted: "Izbrisano",
  by: "od",
  me: "jaz",
  User: "Uporabnik",
  System: "Sistem",
  Scheduled: "Načrtovano",
  "Version history": "Zgodovina različic",
  "No version history": "Ni zgodovine različic",
  Version: "Različica",

  // Fiscalization
  Fiscalization: "Fiskalizacija",
  Fiscalized: "Fiskalizirano",
  Pending: "V čakanju",
  Failed: "Neuspešno",
  Skipped: "Preskočeno",
  "Skipped by user": "Uporabnik preskočil",
  "Not fiscalized": "Ni fiskalizirano",
  Fiscalize: "Fiskaliziraj",
  "Retry fiscalization": "Ponovi fiskalizacijo",

  // PDF language names
  English: "Angleščina",
  German: "Nemščina",
  Slovenian: "Slovenščina",
  Italian: "Italijanščina",
  French: "Francoščina",
  Spanish: "Španščina",
  Portuguese: "Portugalščina",
  Dutch: "Nizozemščina",
  Polish: "Poljščina",
  Croatian: "Hrvaščina",
  Swedish: "Švedščina",
  Finnish: "Finščina",
  Estonian: "Estonščina",
  Bulgarian: "Bolgarščina",
  Czech: "Češčina",
  Slovak: "Slovaščina",
  Norwegian: "Norveščina",
  Icelandic: "Islandščina",

  // Linked documents
  "Linked documents": "Povezani dokumenti",
  "No linked documents": "Ni povezanih dokumentov",
  Invoice: "Račun",
  Estimate: "Ponudba",
  "Credit note": "Dobropis",
  "Advance invoice": "Avansni račun",
  "Delivery note": "Dobavnica",
  "Credit for": "Dobropis za",
  "Converted from": "Pretvorjeno iz",
  "Converted to": "Pretvorjeno v",
  "Has credit": "Ima dobropis",
  "Advance applied": "Avans uporabljen",
  "Applied to": "Uporabljen za",
  "Delivered for": "Dostavljeno za",
  "Has delivery": "Ima dobavnico",
  Fulfills: "Izpolnjuje",
  "Fulfilled by": "Izpolnjeno z",
  References: "Sklicuje se na",
  View: "Poglej",
} as const;
