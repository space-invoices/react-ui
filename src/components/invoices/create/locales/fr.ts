export default {
  Peppol: "Peppol",
  "French e-invoice": "Facture électronique française",
  "Click to skip Peppol sending for this invoice": "Cliquez pour ignorer l'envoi de Peppol pour cette facture",
  "Click to enable Peppol sending": "Cliquez pour activer l'envoi Peppol",
  "French electronic delivery or reporting is required for this invoice":
    "La transmission ou la déclaration électronique française est obligatoire pour cette facture",
  "Click to skip French electronic delivery or reporting for this invoice":
    "Cliquez pour ignorer la transmission ou la déclaration électronique française pour cette facture",
  "Click to enable French electronic delivery or reporting":
    "Cliquez pour activer la transmission ou la déclaration électronique française",
  "Peppol Scheme": "Schéma Peppol",
  "Peppol ID": "Identifiant Peppol",
  Scheme: "Schème",
  Recipient: "Destinataire",
  Name: "Nom",
  Details: "Détails",
  Number: "Numéro",
  "Number format can be changed in settings": "Le format du numéro peut être modifié dans les paramètres",
  Date: "Date",
  "Pick a date": "Choisir une date",
  Reference: "Référence",
  "e.g., PO-2024-001": "p. ex. PO-2024-001",
  "Due Date": "Date d'échéance",
  "Valid Until": "Valable jusqu'au",
  Items: "Articles",
  "Add item": "Ajouter un article",
  Save: "Enregistrer",
  Update: "Mettre à jour",
  "Save as Draft": "Enregistrer comme brouillon",
  Quantity: "Quantité",
  Price: "Prix",
  Unit: "Unité",
  "E-invoicing unit code": "Code d'unité pour l'e-facturation",
  "E-invoicing unit settings": "Paramètres d'unité pour l'e-facturation",
  "Only affects e-invoicing XML. The document unit stays unchanged.":
    "Affecte uniquement le XML d'e-facturation. L'unité du document reste inchangée.",
  "Select XML unit code": "Sélectionner le code d'unité XML",
  "Enter a supported UN/ECE code": "Saisissez un code UN/ECE pris en charge",
  "Use display unit": "Utiliser l'unité affichée",
  "Clear unit code": "Effacer le code d'unité",
  "custom code": "code personnalisé",
  Discount: "Remise",
  Tax: "Taxe",
  "Tax rate": "Taux de taxe",
  "Add tax": "Ajouter une taxe",
  "Select tax...": "Sélectionner une taxe...",
  "Loading...": "Chargement...",
  "No taxes found": "Aucune taxe trouvée",
  "Add new tax...": "Ajouter une nouvelle taxe...",
  Description: "Description",
  "Mark as Paid": "Marquer comme payé",
  Paid: "Payé",
  "Invoice will be marked as fully paid upon creation": "La facture sera marquée comme entièrement payée à la création",
  "Advance payments are applied automatically. Record any remaining payment after creating the invoice.":
    "Les acomptes sont appliqués automatiquement. Enregistrez tout paiement restant après la création de la facture.",
  "Payment Type": "Type de paiement",
  "Select payment type": "Sélectionner le type de paiement",
  Cash: "Espèces",
  "Bank Transfer": "Virement bancaire",
  Card: "Carte",
  Check: "Chèque",
  Other: "Autre",
  Note: "Note",
  "Insert variable": "Insérer une variable",
  Bold: "Gras",
  Italic: "Italique",
  "Bullet list": "Liste à puces",
  "Add payment instructions, terms, or other notes...":
    "Ajoutez des instructions de paiement, des conditions ou d'autres notes...",
  Signature: "Signature",
  "Add signature text...": "Ajouter un texte de signature...",
  "Payment Terms": "Conditions de paiement",
  "Add payment terms...": "Ajoutez des conditions de paiement...",
  Entity: "Entreprise",
  Document: "Document",
  Customer: "Client",
  "Company name": "Nom de l'entreprise",
  "Email address": "Adresse e-mail",
  "Invoice number": "Numéro de facture",
  "Invoice date": "Date de facture",
  "Due date": "Date d'échéance",
  "Total amount": "Montant total",
  Currency: "Devise",
  "Select currency": "Sélectionner la devise",
  "Customer name": "Nom du client",
  "Customer email": "E-mail du client",
  "Today's date": "Date du jour",
  "Current year": "Année en cours",
  "Search or create customer...": "Rechercher ou créer un client...",
  Clear: "Effacer",
  Address: "Adresse",
  "Address 2": "Adresse 2",
  "Buyer reference": "Référence acheteur",
  Required: "Obligatoire",
  "Add a different delivery address": "Ajouter une adresse de livraison différente",
  "Delivery address": "Adresse de livraison",
  "Complete this only when goods are delivered somewhere other than the billing address.":
    "À renseigner uniquement si les biens sont livrés à une adresse différente de l'adresse de facturation.",
  "Country code": "Code pays",
  "Post Code": "Code postal",
  City: "Ville",
  State: "Région",
  Country: "Pays",
  "Bank Account": "Compte bancaire",
  "Account Type": "Type de compte",
  BIC: "BIC",
  IBAN: "IBAN",
  "US domestic": "National États-Unis",
  "UK domestic": "National Royaume-Uni",
  "Account Number": "Numéro de compte",
  "Routing Number": "Numéro d'acheminement",
  "Sort Code": "Sort code",
  "Company Number": "Numéro d'entreprise",
  "UJP receiver identifier": "Identifiant du destinataire UJP",
  "UJP receiver agent": "Agent du destinataire UJP",
  "UJP receiver mailbox": "Boîte aux lettres du destinataire UJP",
  "Tax Number": "Numéro de TVA",
  "Percentage discount": "Remise en pourcentage",
  "Fixed amount discount": "Remise fixe",
  Percentage: "Pourcentage",
  "Fixed amount": "Montant fixe",
  "FURS Fiscalization": "Fiscalisation FURS",
  "Business Premise": "Local commercial",
  "Electronic Device": "Appareil électronique",
  Premise: "Local",
  Device: "Appareil",
  "Select premise": "Sélectionner le local",
  "Select device": "Sélectionner l'appareil",
  "Skip Fiscalization": "Ignorer la fiscalisation",
  "Skip fiscalization for this invoice (e.g., bank transfers or unpaid invoices)":
    "Ignorer la fiscalisation pour cette facture (ex. virements bancaires ou factures impayées)",
  "Cannot skip fiscalization for cash payments": "Impossible d'ignorer la fiscalisation pour les paiements en espèces",
  Skipped: "Ignorée",
  Enabled: "Activée",
  "Click to enable fiscalization": "Cliquez pour activer la fiscalisation",
  "Fiscally verify": "Vérification fiscale",
  "Click to skip fiscalization for this invoice": "Cliquez pour ignorer la fiscalisation pour cette facture",
  "Business unit": "Unité commerciale",
  "No unit": "Aucune unité",
  "e-SLOG": "e-SLOG",
  "Click to skip e-SLOG validation for this invoice": "Cliquez pour ignorer la validation e-SLOG pour cette facture",
  "Click to enable e-SLOG validation": "Cliquez pour activer la validation e-SLOG",
  "e-SLOG Validation Failed": "Échec de la validation e-SLOG",
  "The following entity settings need to be updated:": "Les paramètres d'entité suivants doivent être mis à jour :",
  "Entity name is required for e-SLOG. Update in Settings > Company.":
    "Le nom de l'entité est requis pour e-SLOG. Mettez-le à jour dans Paramètres > Société.",
  "Entity address is required for e-SLOG. Update in Settings > Company.":
    "L'adresse de l'entité est requise pour e-SLOG. Mettez-la à jour dans Paramètres > Société.",
  "Entity postal code is required for e-SLOG. Update in Settings > Company.":
    "Le code postal de l'entité est requis pour e-SLOG. Mettez-le à jour dans Paramètres > Société.",
  "Entity city is required for e-SLOG. Update in Settings > Company.":
    "La ville de l'entité est requise pour e-SLOG. Mettez-la à jour dans Paramètres > Société.",
  "Entity must be from Slovenia (SI) for e-SLOG.": "L'entité doit être située en Slovénie (SI) pour e-SLOG.",
  "Entity tax number is required for e-SLOG. Update in Settings > Company.":
    "Le numéro fiscal de l'entité est requis pour e-SLOG. Mettez-le à jour dans Paramètres > Société.",
  "Slovenian tax number must be 8 digits.": "Le numéro fiscal slovène doit comporter 8 chiffres.",
  "Invoice date is required for e-SLOG.": "La date de facture est requise pour e-SLOG.",
  "Currency is required for e-SLOG.": "La devise est requise pour e-SLOG.",
  "Entity currency must be EUR for e-SLOG when invoice uses a different currency.":
    "La devise de l'entité doit être l'EUR pour e-SLOG lorsque la facture utilise une devise différente.",
  "Customer name is required when customer is provided.": "Le nom du client est requis lorsqu'un client est renseigné.",
  "Entity IBAN is required for UJP package validation. Update in Settings > Company.":
    "L’IBAN de l’entité est obligatoire pour la validation du paquet UJP. Mettez-le à jour dans Paramètres > Société.",
  "Entity IBAN must be a valid Slovenian IBAN for UJP package validation.":
    "L’IBAN de l’entité doit être un IBAN slovène valide pour la validation du paquet UJP.",
  "Entity BIC is required for UJP package validation. Update in Settings > Company.":
    "Le BIC de l’entité est obligatoire pour la validation du paquet UJP. Mettez-le à jour dans Paramètres > Société.",
  "Entity BIC must be a valid 8- or 11-character BIC for UJP package validation.":
    "Le BIC de l’entité doit être un BIC valide à 8 ou 11 caractères pour la validation du paquet UJP.",
  "Customer IBAN must be a valid Slovenian IBAN for UJP package validation.":
    "L’IBAN du client doit être un IBAN slovène valide pour la validation du paquet UJP.",
  "Customer BIC must be a valid 8- or 11-character BIC for UJP package validation.":
    "Le BIC du client doit être un BIC valide à 8 ou 11 caractères pour la validation du paquet UJP.",
  "At least one line item is required for e-SLOG.": "Au moins une ligne est requise pour e-SLOG.",
  "Item name is required for e-SLOG.": "Le nom de l'article est requis pour e-SLOG.",
  "Quantity must be greater than 0.": "La quantité doit être supérieure à 0.",
  "Price is required for e-SLOG.": "Le prix est requis pour e-SLOG.",
  "Invalid Slovenian tax rate {{rate}}%. Valid: {{validRates}}":
    "Taux de TVA slovène invalide {{rate}}%. Valides : {{validRates}}",
  "Reverse charge - tax exempt EU B2B sale": "Autoliquidation - vente B2B UE exonérée de taxe",
  "Taxes disabled": "Taxes désactivées",
  "Gross price": "Prix brut",
  "Net price": "Prix net",
  "Gross price (tax included)": "Prix brut (taxes incluses)",
  "Net price (before tax)": "Prix net (avant taxes)",
  "Linked documents": "Documents liés",
  Type: "Type",
  Total: "Total",
  "Delivery note": "Bon de livraison",
  Invoice: "Facture",
  Estimate: "Devis",
  "Proforma invoice": "Facture pro forma",
  "Credit note": "Avoir",
  "Advance invoice": "Facture d'acompte",
  "FINA fiscalized invoices always use the current date":
    "Les factures fiscalisées FINA utilisent toujours la date actuelle",
  "On receipt": "À réception",
  "7 days": "7 jours",
  "14 days": "14 jours",
  "30 days": "30 jours",
  "60 days": "60 jours",
  "90 days": "90 jours",
  Custom: "Personnalisé",
  "Service Date": "Date de service",
  "Service Period": "Période de service",
  From: "Du",
  To: "Au",
  "Add separator": "Ajouter un séparateur",
  "Section header": "En-tête de section",
  "Section title...": "Titre de section...",
  "Transaction type": "Type de transaction",
  Domestic: "Nationale",
  "EU B2B": "EU B2B",
  "EU B2C": "EU B2C",
  "3W B2B": "3W B2B",
  "3W B2C": "3W B2C",
  "Determining transaction type...": "Détermination du type de transaction...",
  "This invoice will not be fiscalized (non-domestic transaction)":
    "Cette facture ne sera pas fiscalisée (transaction non nationale)",
  "Tax Clause": "Clause fiscale",
  "Add tax clause...": "Ajouter une clause fiscale...",
  Footer: "Pied de page",
  "Add document footer...": "Ajouter un pied de page...",
  "End consumer": "Consommateur final",
  "Domestic B2B invoicing in Croatia is not supported":
    "La facturation B2B domestique en Croatie n'est pas prise en charge. La Croatie exige la conformité à Fiskalizacija 2.0 pour la facturation B2B.",
  "FINA fiscalization must be enabled for domestic invoices":
    "La fiscalisation FINA doit être activée pour les factures domestiques. Activez-la dans les paramètres de l'entité.",
  "Please select": "Veuillez sélectionner",
  Amount: "Montant",
  "Add payment": "Ajouter un paiement",
  "Recorded now": "Enregistré maintenant",
  "Remaining due": "Reste dû",
  "Remaining to allocate": "Reste à affecter",
  "This document must be fully paid on creation": "Ce document doit être entièrement payé à la création",
  "Record one or more payments on creation": "Enregistrer un ou plusieurs paiements à la création",
  "Entity bank account is required for e-SLOG payment instructions. Update in Settings > Company.":
    "Le compte bancaire de l’entité est requis pour les instructions de paiement e-SLOG. Mettez-le à jour dans Paramètres > Société.",
  "Customer tax number is required for e-SLOG business recipients.":
    "Le numéro fiscal du client est requis pour les destinataires professionnels e-SLOG.",
  "Customer address is required for UJP package validation.":
    "L’adresse du client est requise pour la validation du package UJP.",
  "Customer postal code is required for UJP package validation.":
    "Le code postal du client est requis pour la validation du package UJP.",
  "Customer city is required for UJP package validation.":
    "La ville du client est requise pour la validation du package UJP.",
  "Customer tax number or company number is required for UJP package validation.":
    "Le numéro fiscal ou le numéro d'entreprise du client est requis pour la validation du paquet UJP.",
  "Customer IBAN and BIC are required for UJP package validation.":
    "L’IBAN et le BIC du client sont requis pour la validation du paquet UJP.",
  "Customer IBAN is required for UJP package validation.":
    "L’IBAN du client est requis pour la validation du paquet UJP.",
  "Customer BIC is required for UJP package validation.":
    "Le BIC du client est requis pour la validation du paquet UJP.",
} as const;
