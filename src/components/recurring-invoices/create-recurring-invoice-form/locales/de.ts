export default {
  Name: "Name",
  "Enter schedule name": "Planname eingeben",
  Frequency: "Häufigkeit",
  "Select frequency": "Häufigkeit wählen",
  Daily: "Täglich",
  Weekly: "Wöchentlich",
  Monthly: "Monatlich",
  Yearly: "Jährlich",
  Interval: "Intervall",
  "Interval help": "Hilfe zum Intervall",
  "Interval controls how often the schedule repeats. For example, Monthly with interval 3 issues every 3 months, which is quarterly.":
    "Das Intervall steuert, wie oft der Plan wiederholt wird. Zum Beispiel erstellt Monatlich mit Intervall 3 alle 3 Monate eine Rechnung, also vierteljährlich.",
  "Day of Week": "Wochentag",
  "Select day": "Tag wählen",
  Sunday: "Sonntag",
  Monday: "Montag",
  Tuesday: "Dienstag",
  Wednesday: "Mittwoch",
  Thursday: "Donnerstag",
  Friday: "Freitag",
  Saturday: "Samstag",
  "Day of Month": "Tag im Monat",
  Month: "Monat",
  "Select month": "Monat wählen",
  January: "Januar",
  February: "Februar",
  March: "März",
  April: "April",
  May: "Mai",
  June: "Juni",
  July: "Juli",
  August: "August",
  September: "September",
  October: "Oktober",
  November: "November",
  December: "Dezember",
  "Start Date": "Startdatum",
  "End Date": "Enddatum",
  "Pick a date": "Datum wählen",
  "No end date": "Kein Enddatum",
  "Auto-send": "Automatisch senden",
  "Automatically email generated invoices": "Erstellte Rechnungen automatisch per E-Mail senden",
  "Create as draft": "Als Entwurf erstellen",
  "Generated invoices will be drafts for review":
    "Erstellte Rechnungen werden als Entwürfe zur Überprüfung gespeichert",
  "There was an error creating the schedule": "Beim Erstellen des Plans ist ein Fehler aufgetreten",
} as const;
