export default {
  PDF: "PDF",
  Send: "Send",
  Payment: "Payment",
  Share: "Share",
  Unshare: "Unshare",
  Copied: "Copied",
  "Copy shareable link": "Copy shareable link",
  "Unshare document": "Unshare document",
  "Unshare document confirmation": "Unshare document confirmation",
  "Document unshared": "Document unshared",
  "Unshare failed": "Unshare failed",
  "Download failed": "Download failed",
  "e-SLOG download failed": "e-SLOG download failed",
  "UJP package": "UJP package",
  "UJP package download failed": "UJP package download failed",
  XRechnung: "XRechnung",
  ZUGFeRD: "ZUGFeRD",
  "XRechnung download failed": "XRechnung download failed",
  "ZUGFeRD download failed": "ZUGFeRD download failed",
  "e-SLOG validation must be valid before downloading.": "e-SLOG validation must be valid before downloading.",
  "UJP validation must be valid before downloading.": "UJP validation must be valid before downloading.",
  "German e-invoice validation must be valid before downloading.":
    "German e-invoice validation must be valid before downloading.",
  "XRechnung validation must be valid before downloading.": "XRechnung validation must be valid before downloading.",
  "ZUGFeRD validation must be valid before downloading.": "ZUGFeRD validation must be valid before downloading.",
  Recurring: "Recurring",
  Duplicate: "Duplicate",
  "Duplicate invoice": "Duplicate invoice",
  "Duplicate estimate": "Duplicate estimate",
  "Duplicate credit_note": "Duplicate credit note",
  "Duplicate advance_invoice": "Duplicate advance invoice",
  "Duplicate delivery_note": "Duplicate delivery note",
  "Create invoice": "Create invoice",
  "Create estimate": "Create estimate",
  "Create credit_note": "Create credit note",
  "Create advance_invoice": "Create advance invoice",
  "Create delivery_note": "Create delivery note",
  Details: "Details",
  Number: "Number",
  Date: "Date",
  "Due date": "Due date",
  "Service date": "Service date",
  "Service period": "Service period",
  "Valid until": "Valid until",
  Customer: "Customer",
  Discount: "Discount",
  Subtotal: "Subtotal",
  Tax: "Tax",
  of: "of",
  Total: "Total",
  Paid: "Paid",
  Due: "Due",
  Status: "Status",
  "Paid in full": "Paid in full",
  "Partially paid": "Partially paid",
  Unpaid: "Unpaid",
  Voided: "Voided",
  Overdue: "Overdue",
  Shareable: "Shareable",
  "Proforma invoice": "Proforma invoice",
  "Packing list": "Packing list",
  "FURS fiscalization failed": "FURS fiscalization failed",
  "FINA fiscalization failed": "FINA fiscalization failed",
  Payments: "Payments",
  Refunds: "Refunds",
  "No payments": "No payments",
  "No refunds": "No refunds",
  "Add payment": "Add payment",
  Edit: "Edit",
  Delete: "Delete",
  "Delete payment": "Delete payment",
  "Delete payment confirmation": "Delete payment confirmation",
  Cancel: "Cancel",
  cash: "cash",
  bank_transfer: "bank_transfer",
  card: "card",
  check: "check",
  credit_note: "credit note",
  other: "other",
  advance_invoice: "advance invoice",
  advance_payment: "advance_payment",
  "Delete failed": "Delete failed",
  Finalize: "Issue",
  "Delete Draft": "Delete Draft",
  "Finalize document": "Issue document",
  "Finalize document confirmation": "Issue document confirmation",
  "Delete draft document": "Delete draft document",
  "Delete draft confirmation": "Delete draft confirmation",
  "Document finalized": "Document issued",
  "Finalize failed": "Issuing failed",
  "Draft deleted": "Draft deleted",
  "Delete draft failed": "Delete draft failed",
  Draft: "Draft",
  Void: "Void",
  "Voiding...": "Voiding...",
  Activity: "Activity",
  "No activity": "No activity",
  Created: "Created",
  Updated: "Updated",
  Sent: "Sent",
  Deleted: "Deleted",
  by: "by",
  me: "me",
  User: "User",
  System: "System",
  Scheduled: "Scheduled",
  "Version history": "Version history",
  "No version history": "No version history",
  Version: "Version",
  Fiscalization: "Fiscalization",
  Fiscalized: "Fiscalized",
  Pending: "Pending",
  Failed: "Failed",
  Skipped: "Skipped",
  "Skipped by user": "Skipped by user",
  "Not fiscalized": "Not fiscalized",
  Fiscalize: "Fiscalize",
  "Retry fiscalization": "Retry fiscalization",
  English: "English",
  German: "German",
  Slovenian: "Slovenian",
  Italian: "Italian",
  French: "French",
  Spanish: "Spanish",
  Portuguese: "Portuguese",
  Dutch: "Dutch",
  Polish: "Polish",
  Croatian: "Croatian",
  Swedish: "Swedish",
  Finnish: "Finnish",
  Estonian: "Estonian",
  Bulgarian: "Bulgarian",
  Czech: "Czech",
  Slovak: "Slovak",
  Norwegian: "Norwegian",
  Icelandic: "Icelandic",
  "Linked documents": "Linked documents",
  "No linked documents": "No linked documents",
  Invoice: "Invoice",
  Estimate: "Estimate",
  "Credit note": "Credit note",
  "Advance invoice": "Advance invoice",
  "Delivery note": "Delivery note",
  "Credit for": "Credit for",
  "Converted from": "Converted from",
  "Converted to": "Converted to",
  "Has credit": "Has credit",
  "Advance applied": "Advance applied",
  "Applied to": "Applied to",
  "Delivered for": "Delivered for",
  "Has delivery": "Has delivery",
  Fulfills: "Fulfills",
  "Fulfilled by": "Fulfilled by",
  References: "References",
  View: "View",
} as const;
