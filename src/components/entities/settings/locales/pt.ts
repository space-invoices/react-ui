export default {
  // Company settings
  "Tax subject": "Sujeito fiscal",
  "Tax ID 2": "NIF secundário",
  "Secondary tax identification number (optional)": "Número de identificação fiscal secundário (opcional)",
  "Company Information": "Informações da empresa",
  "Basic information about your company": "Informações básicas sobre a sua empresa",
  // Branding settings
  Branding: "Marca",
  "Primary Color": "Cor primária",
  Logo: "Logotipo",
  "Logo URL": "URL do logotipo",
  "Enter the URL of your company logo": "Introduza o URL do logotipo da sua empresa",
  "Upload your company logo for invoices": "Carregue o logotipo da sua empresa para as faturas",
  "Upload Logo": "Carregar logotipo",
  "Change Logo": "Alterar logotipo",
  "Current logo (displayed on invoices)": "Logotipo atual (apresentado nas faturas)",
  "Logo preview": "Pré-visualização do logotipo",
  "Crop Your Logo": "Recortar o seu logotipo",
  Signature: "Assinatura",
  "Upload a signature image for PDFs (optional)": "Carregue uma imagem de assinatura para os PDF (opcional)",
  "Upload Signature": "Carregar assinatura",
  "Change Signature": "Alterar assinatura",
  "Current signature (for PDF documents)": "Assinatura atual (para documentos PDF)",
  "Signature preview": "Pré-visualização da assinatura",
  "Crop Your Signature": "Recortar a sua assinatura",
  "Adjust the crop area or upload the full image": "Ajuste a área de recorte ou carregue a imagem completa",
  "Upload failed. Please try again.": "O carregamento falhou. Por favor, tente novamente.",
  "Uploading...": "A carregar...",
  "Upload & Save": "Carregar e guardar",
  Cancel: "Cancelar",
  Localization: "Localização",
  "Currency Code": "Código de moeda",
  "ISO 4217 currency code (e.g., USD, EUR, GBP)": "Código de moeda ISO 4217 (ex. USD, EUR, GBP)",
  Locale: "Configuração regional",
  "BCP 47 language tag (e.g., en-US, de-DE, sl-SI)": "Etiqueta de idioma BCP 47 (ex. en-US, de-DE, sl-SI)",
  "Select currency": "Selecionar moeda",
  "Select locale": "Selecionar configuração regional",
  "Configure entity localization": "Configurar a localização da entidade",
  "English (US)": "Inglês (US)",
  "German (DE)": "Alemão (DE)",
  "Italian (IT)": "Italiano (IT)",
  "French (FR)": "Francês (FR)",
  "Spanish (ES)": "Espanhol (ES)",
  "Portuguese (Portugal)": "Português (Portugal)",
  Dutch: "Neerlandês",
  Polish: "Polaco",
  Croatian: "Croata",
  "Slovenian (SI)": "Esloveno (SI)",
  "Email Settings": "Definições de email",
  "Configure email settings for invoices": "Configurar as definições de email para as faturas",
  "Email Address": "Endereço de email",
  "Email address to send invoices to": "Endereço de email para enviar faturas",
  "Sender email": "Email do remetente",
  "Billing email address": "Endereço de email de faturação",
  "Recipient for white-label subscription billing emails.":
    "Destinatário dos emails de faturação da subscrição white-label.",
  "Used as the From address for document emails and payment reminders after verification passes.":
    "Usado como endereço From dos emails de documentos e lembretes de pagamento depois de a verificação passar.",
  "DNS DKIM tokens": "Tokens DNS DKIM",
  "Add CNAME records for these tokens at your sender domain, then recheck.":
    "Adicione registos CNAME para estes tokens no domínio remetente e volte a verificar.",
  "DNS records are being prepared. Recheck in a moment if they do not appear.":
    "Os registos DNS estão a ser preparados. Verifique novamente dentro de momentos se não aparecerem.",
  "CNAME record": "Registo CNAME",
  Name: "Nome",
  Value: "Valor",
  Copy: "Copiar",
  Copied: "Copiado",
  Saving: "A guardar",
  "Save sender": "Guardar remetente",
  "Update sender": "Atualizar remetente",
  Clear: "Limpar",
  Checking: "A verificar",
  Recheck: "Verificar novamente",
  Pending: "Pendente",
  Verified: "Verificado",
  Failed: "Falhou",
  Email: "Email",
  Domain: "Domínio",
  "Inherited from white-label": "Herdado do white-label",
  "Entity Email": "Email da entidade",
  "Invalid email address": "Endereço de e-mail inválido",
  "Email shown for this entity on documents and template variables":
    "Email apresentado para esta entidade em documentos e variáveis de modelo",
  "Email Subject": "Assunto do email",
  "Subject line for email invoices": "Linha de assunto para as faturas por email",
  "Email Body": "Corpo do email",
  "Body content for email invoices": "Conteúdo do texto para as faturas por email",
  "Subject line for invoice emails": "Assunto para emails de faturas",
  "Subject line for estimate emails": "Assunto para emails de orcamentos",
  "Subject line for credit note emails": "Assunto para emails de notas de credito",
  "Subject line for advance invoice emails": "Assunto para emails de faturas adiantadas",
  "Subject line for delivery note emails": "Assunto para emails de guias de remessa",
  "Body content for invoice emails": "Conteudo do texto para emails de faturas",
  "Body content for estimate emails": "Conteudo do texto para emails de orcamentos",
  "Body content for credit note emails": "Conteudo do texto para emails de notas de credito",
  "Body content for advance invoice emails": "Conteudo do texto para emails de faturas adiantadas",
  "Body content for delivery note emails": "Conteudo do texto para emails de guias de remessa",
  Reminder: "Lembrete",
  "Payment reminders": "Lembretes de pagamento",
  "Reminder subject": "Assunto do lembrete",
  "Subject line for payment reminder emails": "Assunto dos emails de lembrete de pagamento",
  "Reminder body": "Corpo do lembrete",
  "Body content for payment reminder emails": "Conteúdo dos emails de lembrete de pagamento",
  "Payment instructions": "Instruções de pagamento",
  "Payment instructions appended to reminder emails when not included in the body":
    "Instruções de pagamento anexadas aos lembretes quando não estão incluídas no corpo",
  "Payment reminder from {entity_name}": "Lembrete de pagamento de {entity_name}",
  "Hello {customer_name},\n\nThe following invoices are overdue:\n{invoice_list}\n\n{payment_instructions}":
    "Olá {customer_name},\n\nAs seguintes faturas estão vencidas:\n{invoice_list}\n\n{payment_instructions}",
  "Please remit payment to IBAN {iban}.": "Efetue o pagamento para o IBAN {iban}.",
  "Save Settings": "Guardar definições",
  // Document Defaults section
  "Document Defaults": "Valores predefinidos de documentos",
  "Default values for new documents": "Valores predefinidos para novos documentos",
  "Default Note": "Nota predefinida",
  "Default Invoice Note": "Nota de fatura predefinida",
  "This note will be pre-filled when creating new invoices": "Esta nota será pré-preenchida ao criar novas faturas",
  "This note will be pre-filled when creating new estimates": "Esta nota será pré-preenchida ao criar novos orçamentos",
  "This note will be pre-filled when creating new credit notes":
    "Esta nota será pré-preenchida ao criar novas notas de crédito",
  "Payment terms pre-filled when creating new invoices":
    "Condições de pagamento pré-preenchidas ao criar novas faturas",
  "Payment terms pre-filled when creating new estimates":
    "Condições de pagamento pré-preenchidas ao criar novos orçamentos",
  "Payment terms pre-filled when creating new credit notes":
    "Condições de pagamento pré-preenchidas ao criar novas notas de crédito",
  "Optional note for the document.": "Nota opcional para o documento.",
  "Please remit payment using the bank details shown on the document.":
    "Efetue o pagamento utilizando os dados bancários indicados no documento.",
  "This estimate is valid until {document_valid_until}.": "Este orçamento é válido até {document_valid_until}.",
  "Payment due upon acceptance.": "Pagamento na aceitação.",
  "Credit note for invoice {document_number}.": "Nota de crédito para a fatura {document_number}.",
  "Credit will be applied to your account.": "O crédito será aplicado à sua conta.",
  "Invoice Notes": "Notas da fatura",
  "Smart Template Variables": "Variáveis de modelo inteligentes",
  "Use variables to personalize your notes automatically":
    "Use variáveis para personalizar as suas notas automaticamente",
  "Set a default note that will appear on all new invoices. Use template variables to personalize the note automatically.":
    "Defina uma nota predefinida que aparecerá em todas as novas faturas. Use as variáveis de modelo para personalizar a nota automaticamente.",
  // Smart code insert button
  "Insert variable": "Inserir variável",
  Bold: "Negrito",
  Italic: "Itálico",
  "Bullet list": "Lista com marcadores",
  Entity: "Empresa",
  Document: "Documento",
  Customer: "Cliente",
  Other: "Outro",
  "Company name": "Nome da empresa",
  "Email address": "Endereço de email",
  "Invoice number": "Número da fatura",
  "Invoice date": "Data da fatura",
  "Due date": "Data de vencimento",
  "Total amount": "Valor total",
  Currency: "Moeda",
  "Customer name": "Nome do cliente",
  "Customer email": "Email do cliente",
  "Today's date": "Data de hoje",
  "Current year": "Ano corrente",
  // Payment terms defaults
  "Default Payment Terms": "Condições de pagamento predefinidas",
  "This payment terms will be pre-filled when creating new documents":
    "Estas condições de pagamento serão pré-preenchidas ao criar novos documentos",
  "Payment Terms": "Condições de pagamento",
  "Add payment terms...": "Adicionar condições de pagamento...",
  // Document footer
  "Document Footer": "Rodapé do documento",
  "Footer text displayed at the bottom of PDF documents":
    "Texto do rodapé apresentado na parte inferior dos documentos PDF",
  "Footer text displayed at the bottom of all PDF documents":
    "Texto do rodapé apresentado na parte inferior de todos os documentos PDF",
  "{entity_name} | Due Date: {document_due_date} | Invoice #{document_number}":
    "{entity_name} | Vencimento: {document_due_date} | Fatura #{document_number}",
  // Document signature
  "Document Signature": "Assinatura do documento",
  "Signature text displayed on all PDF documents": "Texto de assinatura exibido em todos os documentos PDF",
  "Add signature text...": "Adicionar texto de assinatura...",
  "Delivery Note": "Guia de remessa",
  "Default note for advance invoices": "Nota predefinida para faturas antecipadas",
  "Default note for all new advance invoices": "Nota predefinida para todas as novas faturas antecipadas",
  "Default note for delivery notes": "Nota predefinida para guias de remessa",
  "Default note for all new delivery notes": "Nota predefinida para todas as novas guias de remessa",
  // Overdue Notifications section
  "Overdue Notifications": "Notificações de atraso",
  "Automatically remind customers about overdue invoices":
    "Lembrar automaticamente os clientes sobre faturas em atraso",
  "Enable Notifications": "Ativar notificações",
  "Send automatic payment reminder emails to customers with overdue invoices":
    "Enviar emails de lembrete de pagamento automáticos a clientes com faturas em atraso",
  "Reminder Days": "Dias de lembrete",
  "Days after due date to send reminders (comma-separated). E.g., '7, 14, 30' sends reminders at 7, 14, and 30 days overdue.":
    "Dias após a data de vencimento para enviar lembretes (separados por vírgulas). Ex. '7, 14, 30' envia lembretes aos 7, 14 e 30 dias de atraso.",
  "Custom Reminder Templates": "Modelos de lembrete personalizados",
  "Subject line for overdue notification emails": "Linha de assunto para os emails de notificação de atraso",
  "Body content for overdue emails. Use {invoice_list} to include the invoice table, or it will be auto-appended.":
    "Conteúdo do texto para os emails de atraso. Use {invoice_list} para incluir a tabela de faturas, ou será adicionada automaticamente.",
  "Dear {customer_name},\n\nThis is a reminder about your overdue invoices...":
    "Caro/a {customer_name},\n\nEste é um lembrete sobre as suas faturas em atraso...",
  "Payment Reminders": "Lembretes de pagamento",
  "When enabled, customers will automatically receive email reminders when their invoices become overdue. Emails are sent on the exact day each threshold is reached.":
    "Quando ativado, os clientes receberão automaticamente lembretes por email quando as suas faturas ficarem em atraso. Os emails são enviados exatamente no dia em que cada limite é atingido.",
  "Available Variables": "Variáveis disponíveis",
  "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}":
    "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}",
  // UPN QR Payment section
  "UPN QR Payment": "Pagamento UPN QR",
  "Configure UPN QR payment slip for invoices": "Configurar o talão de pagamento UPN QR para faturas",
  IBAN: "IBAN",
  "Bank account IBAN for receiving payments": "IBAN da conta bancária para receber pagamentos",
  "Enable UPN QR on invoices": "Ativar UPN QR nas faturas",
  "Show payment QR code on PDF invoices for easy mobile banking payments":
    "Mostrar código QR de pagamento nas faturas PDF para pagamentos bancários móveis fáceis",
  "Display Mode": "Modo de exibição",
  "QR code only": "Apenas código QR",
  "Shows compact QR code inline with invoice content": "Mostra código QR compacto em linha com o conteúdo da fatura",
  "Full UPN payment slip": "Talão de pagamento UPN completo",
  "Shows complete payment slip at bottom of page": "Mostra o talão de pagamento completo no fundo da página",
  "Purpose Code": "Código de finalidade",
  "OTHR - Other": "OTHR - Outro",
  "GDSV - Goods and Services": "GDSV - Bens e serviços",
  "SUPP - Supplier Payment": "SUPP - Pagamento a fornecedor",
  "Payment purpose code (ISO 20022)": "Código de finalidade de pagamento (ISO 20022)",
  "UPN QR Payments": "Pagamentos UPN QR",
  "UPN QR is a Slovenian standard for payment slips. When enabled, your invoices will include a QR code that customers can scan with their mobile banking app to pay instantly.":
    "UPN QR é um padrão esloveno para talões de pagamento. Quando ativado, as suas faturas incluirão um código QR que os clientes podem digitalizar com a aplicação de banca móvel para pagar instantaneamente.",
  // Number format settings
  Invoice: "Fatura",
  Estimate: "Orçamento",
  "Credit Note": "Nota de crédito",
  "Advance Invoice": "Fatura antecipada",
  "Number Format": "Formato de numeração",
  Preview: "Pré-visualização",
  // Tax rules settings
  "Main documents": "Documentos principais",
  "Used when no business unit is selected.": "Usado quando nenhuma unidade de negócio está selecionada.",
  "Business-unit documents": "Documentos da unidade de negócio",
  "Used when a business unit is selected. Default formats should include {u} to stay distinct.":
    "Usado quando uma unidade de negócio está selecionada. Os formatos padrão devem incluir {u} para permanecerem distintos.",
  "If this format matches the main document format, saving is blocked.":
    "Se este formato corresponder ao formato principal, o guardado é bloqueado.",
  "Business-unit numbering must differ from the main document format.":
    "A numeração da unidade de negócio deve diferir do formato principal.",
  "First number to use": "Primeiro número a usar",
  "First number to use for this unit": "Primeiro número a usar para esta unidade",
  "first number to use": "primeiro número a usar",
  "Only affects this business unit for this document type. Existing documents are not renumbered.":
    "Afeta apenas esta unidade de negócio para este tipo de documento. Os documentos existentes não são renumerados.",
  "Set the first number for this unit by document type. The unit number format is shared in Numbering settings.":
    "Defina o primeiro número desta unidade por tipo de documento. O formato de numeração da unidade é partilhado nas definições de numeração.",
  "Edit shared unit format": "Editar formato partilhado da unidade",
  "first number": "primeiro número",
  "Unit numbering": "Numeração da unidade",
  Numbering: "Numeração",
  "Only affects this business unit for the selected document type. Existing documents are not renumbered.":
    "Afeta apenas esta unidade de negócio para o tipo de documento selecionado. Os documentos existentes não são renumerados.",
  "Can only move future numbering forward. Existing documents are not renumbered.":
    "Só pode avançar a numeração futura. Os documentos existentes não são renumerados.",
  "Insert token": "Inserir token",
  "Number tokens": "Tokens de numeração",
  "Full year": "Ano completo",
  "Short year": "Ano curto",
  "Sequence number": "Número sequencial",
  "Business unit index": "Índice da unidade de negócio",
  "EU Tax Rules": "Regras fiscais da UE",
  "Automatic tax handling for cross-border transactions":
    "Gestão automática de impostos para transações transfronteiriças",
  "Manual taxes": "Impostos manuais",
  "Create tax rates on the Taxes page and apply them to document lines.":
    "Crie taxas fiscais na página Impostos e aplique-as às linhas do documento.",
  "tax-rules.vies_validate_vat.label": "Validação de IVA VIES",
  "tax-rules.vies_validate_vat.description":
    "Verificar os números de IVA da UE através da API VIES. Os números de IVA dos clientes são verificados antes da criação de faturas.",
  "tax-rules.auto_reverse_charge.label": "Autoliquidação automática",
  "tax-rules.auto_reverse_charge.description":
    "Aplicar automaticamente a taxa 0% (autoliquidação) para transações B2B intra-UE com números de IVA válidos.",
  "tax-rules.auto_remove_tax_export.label": "Remoção automática de imposto de exportação",
  "tax-rules.auto_remove_tax_export.description":
    "Remover automaticamente todos os impostos para exportações para países fora da UE.",
  "tax-rules.require_gross_prices.label": "Exigir preços brutos",
  "tax-rules.require_gross_prices.description":
    "Exigir preços brutos quando a autoliquidação é aplicada. Previne variações inesperadas nos valores totais das faturas.",
  "tax-rules.calculation_mode.label": "Modo de cálculo",
  "tax-rules.calculation_mode.description":
    "Escolha a ordem em que os descontos e o IVA são aplicados no cálculo dos totais do documento.",
  "tax-rules.calculation_mode.b2b_standard.label": "Padrão B2B",
  "tax-rules.calculation_mode.b2b_standard.description":
    "Os descontos reduzem primeiro o valor líquido da linha e só depois é calculado o IVA.",
  "tax-rules.calculation_mode.b2c_gross_discount.label": "Desconto bruto B2C",
  "tax-rules.calculation_mode.b2c_gross_discount.description":
    "Os descontos reduzem o valor bruto final a pagar e, a partir daí, são calculados o valor líquido e o IVA.",
  // Tax clauses settings
  "Default Tax Clauses": "Cláusulas fiscais predefinidas",
  "Set default tax clauses that are automatically added to documents based on transaction type":
    "Defina as cláusulas fiscais predefinidas que são automaticamente adicionadas aos documentos com base no tipo de transação.",
  "tax-clauses.other-title": "Cláusulas fiscais transfronteiriças",
  "tax-clauses.other-description":
    "Cláusulas fiscais para transações UE e internacionais (autoliquidação, exportação).",
  "tax-clauses.intra_eu_b2b.label": "Intra-UE B2B (autoliquidação)",
  "tax-clauses.intra_eu_b2b.description":
    "Para vendas a empresas da UE com números de IVA válidos. Normalmente inclui uma declaração de autoliquidação.",
  "Enter reverse charge clause...": "Introduza a cláusula de autoliquidação...",
  "tax-clauses.3w_b2b.label": "Fora da UE B2B (exportação)",
  "tax-clauses.3w_b2b.description":
    "Para vendas a empresas fora da UE. Normalmente indica a isenção de IVA por exportação.",
  "tax-clauses.3w_b2c.label": "Fora da UE B2C (exportação)",
  "tax-clauses.3w_b2c.description":
    "Para vendas a consumidores fora da UE. Normalmente indica a isenção de IVA por exportação.",
  "Enter export exemption clause...": "Introduza a cláusula de isenção de exportação...",
  "tax-clauses.domestic.label": "Predefinido / Nacional",
  "tax-clauses.domestic.description":
    "Cláusula predefinida para transações nacionais. Para entidades não sujeitas a IVA, aparece em todos os documentos.",
  "Enter default tax clause...": "Introduza a cláusula fiscal predefinida...",
  "tax-clauses.intra_eu_b2c.label": "Intra-UE B2C",
  "Enter EU consumer sales clause...": "Introduza a cláusula de venda ao consumidor UE...",
  "tax-clauses.other.description":
    "Estes tipos de transações normalmente não requerem cláusulas fiscais específicas, mas pode configurá-las conforme necessário.",
  // EPC QR Payment section
  "EPC QR Payment": "Pagamento EPC QR",
  "SEPA credit transfer QR code for invoices": "Código QR de transferência SEPA para faturas",
  "Enable EPC QR on invoices": "Ativar EPC QR nas faturas",
  "Show SEPA QR code on EUR invoices for easy bank payments":
    "Mostrar código QR SEPA em faturas EUR para pagamentos bancários fáceis",
  "EPC QR Payments": "Pagamentos EPC QR",
  "EPC QR is a European standard for SEPA credit transfers. When enabled, EUR invoices include a QR code that customers can scan with their banking app to pay instantly.":
    "EPC QR é um padrão europeu para transferências SEPA. Quando ativado, as faturas em EUR incluem um código QR que os clientes podem digitalizar com a sua app bancária para pagar instantaneamente.",
  // Document date defaults
  "Default due days": "Dias de vencimento predefinidos",
  "Default valid days": "Dias de validade predefinidos",
  "Number of days added to the document date for new invoice due dates":
    "Número de dias adicionados à data do documento para novos vencimentos de faturas.",
  "Number of days added to the document date for new estimate valid-till dates":
    "Número de dias adicionados à data do documento para novas datas de validade de orçamentos.",

  // Portugal entity details
  "Portugal Details": "Dados de Portugal",
  "Portuguese law requires these details on every entity": "A lei portuguesa exige estes dados em todas as entidades",
  "Company Number": "NIPC",
  "Company registration number (NIPC)": "Número de identificação de pessoa coletiva (NIPC)",
  Phone: "Telefone",
  "International format, including the country code": "Formato internacional, incluindo o indicativo do país",
  "Share Capital": "Capital social",
  "Registered share capital of the company": "Capital social registado da empresa",
  // Tax rule sections and Slovenian Article 76.a
  "Calculation settings": "Definições de cálculo",
  "Choose how prices, discounts, and taxes are calculated on documents.":
    "Escolha como os preços, descontos e impostos são calculados nos documentos.",
  "EU cross border rules": "Regras da UE para transações transfronteiriças",
  "Validate VAT numbers and automate reverse charge for eligible EU transactions.":
    "Valide números de IVA e automatize a autoliquidação para transações elegíveis na UE.",
  "Export rules": "Regras de exportação",
  "Configure automatic tax handling for transactions outside the EU.":
    "Configure o tratamento fiscal automático para transações fora da UE.",
  "Configure domestic Slovenian tax treatments.": "Configure os tratamentos fiscais nacionais eslovenos.",
  "Slovenian Article 76.a": "Artigo 76.a esloveno",
  "Allow eligible domestic business invoices to report VAT under reverse charge.":
    "Permita que faturas B2B nacionais elegíveis apresentem o IVA em regime de autoliquidação.",
  "Article 76.a tax clause": "Cláusula fiscal do artigo 76.a",
  "Default legal clause added when Article 76.a is selected on an invoice.":
    "Cláusula legal predefinida adicionada quando o artigo 76.a é selecionado numa fatura.",
  "Obrnjena davčna obveznost po 76.a členu ZDDV-1.": "Obrnjena davčna obveznost po 76.a členu ZDDV-1.",
} as const;
