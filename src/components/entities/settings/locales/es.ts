export default {
  // Company settings
  "Tax subject": "Sujeto fiscal",
  "Tax ID 2": "NIF 2",
  "Secondary tax identification number (optional)": "Número de identificación fiscal secundario (opcional)",
  "Company Information": "Información de la empresa",
  "Basic information about your company": "Información básica sobre su empresa",
  // Branding settings
  Branding: "Marca",
  "Primary Color": "Color primario",
  Logo: "Logo",
  "Logo URL": "URL del logo",
  "Enter the URL of your company logo": "Introduzca la URL del logo de su empresa",
  "Upload your company logo for invoices": "Suba el logo de su empresa para las facturas",
  "Upload Logo": "Subir logo",
  "Change Logo": "Cambiar logo",
  "Current logo (displayed on invoices)": "Logo actual (mostrado en las facturas)",
  "Logo preview": "Vista previa del logo",
  "Crop Your Logo": "Recortar su logo",
  Signature: "Firma",
  "Upload a signature image for PDFs (optional)": "Suba una imagen de firma para los PDF (opcional)",
  "Upload Signature": "Subir firma",
  "Change Signature": "Cambiar firma",
  "Current signature (for PDF documents)": "Firma actual (para documentos PDF)",
  "Signature preview": "Vista previa de la firma",
  "Crop Your Signature": "Recortar su firma",
  "Adjust the crop area or upload the full image": "Ajuste el área de recorte o suba la imagen completa",
  "Upload failed. Please try again.": "La subida ha fallado. Por favor, inténtelo de nuevo.",
  "Uploading...": "Subiendo...",
  "Upload & Save": "Subir y guardar",
  Cancel: "Cancelar",
  Localization: "Localización",
  "Currency Code": "Código de moneda",
  "ISO 4217 currency code (e.g., USD, EUR, GBP)": "Código de moneda ISO 4217 (ej. USD, EUR, GBP)",
  Locale: "Configuración regional",
  "BCP 47 language tag (e.g., en-US, de-DE, sl-SI)": "Etiqueta de idioma BCP 47 (ej. en-US, de-DE, sl-SI)",
  "Select currency": "Seleccionar moneda",
  "Select locale": "Seleccionar configuración regional",
  "Configure entity localization": "Configurar la localización de la entidad",
  "English (US)": "Inglés (US)",
  "German (DE)": "Alemán (DE)",
  "Italian (IT)": "Italiano (IT)",
  "French (FR)": "Francés (FR)",
  "Spanish (ES)": "Español (ES)",
  "Portuguese (Portugal)": "Portugués (Portugal)",
  Dutch: "Neerlandés",
  Polish: "Polaco",
  Croatian: "Croata",
  "Slovenian (SI)": "Esloveno (SI)",
  "Email Settings": "Configuración de email",
  "Configure email settings for invoices": "Configurar las opciones de email para las facturas",
  "Email Address": "Dirección de email",
  "Email address to send invoices to": "Dirección de email para enviar facturas",
  "Sender email": "Email del remitente",
  "Billing email address": "Dirección email de facturación",
  "Recipient for white-label subscription billing emails.":
    "Destinatario de los emails de facturación de la suscripción white-label.",
  "Used as the From address for document emails and payment reminders after verification passes.":
    "Se usa como dirección From para emails de documentos y recordatorios de pago después de que la verificación se complete.",
  "DNS DKIM tokens": "Tokens DNS DKIM",
  "Add CNAME records for these tokens at your sender domain, then recheck.":
    "Añade registros CNAME para estos tokens en tu dominio remitente y vuelve a comprobar.",
  "DNS records are being prepared. Recheck in a moment if they do not appear.":
    "Los registros DNS se están preparando. Vuelve a comprobar en un momento si no aparecen.",
  "CNAME record": "Registro CNAME",
  Name: "Nombre",
  Value: "Valor",
  Copy: "Copiar",
  Copied: "Copiado",
  Saving: "Guardando",
  "Save sender": "Guardar remitente",
  "Update sender": "Actualizar remitente",
  Clear: "Borrar",
  Checking: "Comprobando",
  Recheck: "Volver a comprobar",
  Pending: "Pendiente",
  Verified: "Verificado",
  Failed: "Fallido",
  Email: "Email",
  Domain: "Dominio",
  "Inherited from white-label": "Heredado del white-label",
  "Entity Email": "Correo de la entidad",
  "Invalid email address": "Dirección de correo no válida",
  "Email shown for this entity on documents and template variables":
    "Correo mostrado para esta entidad en documentos y variables de plantilla",
  "Email Subject": "Asunto del email",
  "Subject line for email invoices": "Línea de asunto para las facturas por email",
  "Email Body": "Cuerpo del email",
  "Body content for email invoices": "Contenido del texto para las facturas por email",
  "Subject line for invoice emails": "Línea de asunto para emails de facturas",
  "Subject line for estimate emails": "Línea de asunto para emails de presupuestos",
  "Subject line for credit note emails": "Línea de asunto para emails de notas de crédito",
  "Subject line for advance invoice emails": "Línea de asunto para emails de facturas anticipadas",
  "Subject line for delivery note emails": "Línea de asunto para emails de albaranes",
  "Body content for invoice emails": "Contenido del texto para emails de facturas",
  "Body content for estimate emails": "Contenido del texto para emails de presupuestos",
  "Body content for credit note emails": "Contenido del texto para emails de notas de crédito",
  "Body content for advance invoice emails": "Contenido del texto para emails de facturas anticipadas",
  "Body content for delivery note emails": "Contenido del texto para emails de albaranes",
  Reminder: "Recordatorio",
  "Payment reminders": "Recordatorios de pago",
  "Reminder subject": "Asunto del recordatorio",
  "Subject line for payment reminder emails": "Asunto de los correos de recordatorio de pago",
  "Reminder body": "Cuerpo del recordatorio",
  "Body content for payment reminder emails": "Contenido de los correos de recordatorio de pago",
  "Payment instructions": "Instrucciones de pago",
  "Payment instructions appended to reminder emails when not included in the body":
    "Instrucciones de pago añadidas a los recordatorios cuando no están incluidas en el cuerpo",
  "Payment reminder from {entity_name}": "Recordatorio de pago de {entity_name}",
  "Hello {customer_name},\n\nThe following invoices are overdue:\n{invoice_list}\n\n{payment_instructions}":
    "Hola {customer_name},\n\nLas siguientes facturas están vencidas:\n{invoice_list}\n\n{payment_instructions}",
  "Please remit payment to IBAN {iban}.": "Realice el pago al IBAN {iban}.",
  "Save Settings": "Guardar configuración",
  // Document Defaults section
  "Document Defaults": "Valores predeterminados de documentos",
  "Default values for new documents": "Valores predeterminados para nuevos documentos",
  "Default Note": "Nota predeterminada",
  "Default Invoice Note": "Nota de factura predeterminada",
  "This note will be pre-filled when creating new invoices":
    "Esta nota se completará previamente al crear nuevas facturas",
  "This note will be pre-filled when creating new estimates":
    "Esta nota se completará previamente al crear nuevos presupuestos",
  "This note will be pre-filled when creating new credit notes":
    "Esta nota se completará previamente al crear nuevas notas de crédito",
  "Payment terms pre-filled when creating new invoices":
    "Condiciones de pago completadas previamente al crear nuevas facturas",
  "Payment terms pre-filled when creating new estimates":
    "Condiciones de pago completadas previamente al crear nuevos presupuestos",
  "Payment terms pre-filled when creating new credit notes":
    "Condiciones de pago completadas previamente al crear nuevas notas de crédito",
  "Optional note for the document.": "Nota opcional para el documento.",
  "Please remit payment using the bank details shown on the document.":
    "Realice el pago utilizando los datos bancarios que figuran en el documento.",
  "This estimate is valid until {document_valid_until}.": "Este presupuesto es válido hasta el {document_valid_until}.",
  "Payment due upon acceptance.": "Pago al aceptar.",
  "Credit note for invoice {document_number}.": "Nota de crédito para la factura {document_number}.",
  "Credit will be applied to your account.": "El crédito se aplicará a su cuenta.",
  "Invoice Notes": "Notas de factura",
  "Smart Template Variables": "Variables de plantilla inteligentes",
  "Use variables to personalize your notes automatically": "Use variables para personalizar sus notas automáticamente",
  "Set a default note that will appear on all new invoices. Use template variables to personalize the note automatically.":
    "Establezca una nota predeterminada que aparecerá en todas las nuevas facturas. Use las variables de plantilla para personalizar la nota automáticamente.",
  // Smart code insert button
  "Insert variable": "Insertar variable",
  Bold: "Negrita",
  Italic: "Cursiva",
  "Bullet list": "Lista con viñetas",
  Entity: "Empresa",
  Document: "Documento",
  Customer: "Cliente",
  Other: "Otro",
  "Company name": "Nombre de la empresa",
  "Email address": "Dirección de email",
  "Invoice number": "Número de factura",
  "Invoice date": "Fecha de factura",
  "Due date": "Fecha de vencimiento",
  "Total amount": "Importe total",
  Currency: "Moneda",
  "Customer name": "Nombre del cliente",
  "Customer email": "Email del cliente",
  "Today's date": "Fecha de hoy",
  "Current year": "Año actual",
  // Payment terms defaults
  "Default Payment Terms": "Condiciones de pago predeterminadas",
  "This payment terms will be pre-filled when creating new documents":
    "Estas condiciones de pago se completarán previamente al crear nuevos documentos",
  "Payment Terms": "Condiciones de pago",
  "Add payment terms...": "Añadir condiciones de pago...",
  // Document footer
  "Document Footer": "Pie de página del documento",
  "Footer text displayed at the bottom of PDF documents":
    "Texto del pie de página mostrado en la parte inferior de los documentos PDF",
  "Footer text displayed at the bottom of all PDF documents":
    "Texto del pie de página mostrado en la parte inferior de todos los documentos PDF",
  "{entity_name} | Due Date: {document_due_date} | Invoice #{document_number}":
    "{entity_name} | Vencimiento: {document_due_date} | Factura #{document_number}",
  // Document signature
  "Document Signature": "Firma del documento",
  "Signature text displayed on all PDF documents": "Texto de firma mostrado en todos los documentos PDF",
  "Add signature text...": "Añadir texto de firma...",
  "Delivery Note": "Albarán",
  "Default note for advance invoices": "Nota predeterminada para facturas anticipadas",
  "Default note for all new advance invoices": "Nota predeterminada para todas las nuevas facturas anticipadas",
  "Default note for delivery notes": "Nota predeterminada para albaranes",
  "Default note for all new delivery notes": "Nota predeterminada para todos los nuevos albaranes",
  // Overdue Notifications section
  "Overdue Notifications": "Notificaciones de vencimiento",
  "Automatically remind customers about overdue invoices":
    "Recordar automáticamente a los clientes sobre facturas vencidas",
  "Enable Notifications": "Activar notificaciones",
  "Send automatic payment reminder emails to customers with overdue invoices":
    "Enviar emails de recordatorio de pago automáticos a clientes con facturas vencidas",
  "Reminder Days": "Días de recordatorio",
  "Days after due date to send reminders (comma-separated). E.g., '7, 14, 30' sends reminders at 7, 14, and 30 days overdue.":
    "Días después de la fecha de vencimiento para enviar recordatorios (separados por comas). Ej. '7, 14, 30' envía recordatorios a los 7, 14 y 30 días de vencimiento.",
  "Custom Reminder Templates": "Plantillas de recordatorio personalizadas",
  "Subject line for overdue notification emails": "Línea de asunto para los emails de notificación de vencimiento",
  "Body content for overdue emails. Use {invoice_list} to include the invoice table, or it will be auto-appended.":
    "Contenido del texto para los emails de vencimiento. Use {invoice_list} para incluir la tabla de facturas, o se añadirá automáticamente.",
  "Dear {customer_name},\n\nThis is a reminder about your overdue invoices...":
    "Estimado/a {customer_name},\n\nEste es un recordatorio sobre sus facturas vencidas...",
  "Payment Reminders": "Recordatorios de pago",
  "When enabled, customers will automatically receive email reminders when their invoices become overdue. Emails are sent on the exact day each threshold is reached.":
    "Cuando está activado, los clientes recibirán automáticamente recordatorios por email cuando sus facturas venzan. Los emails se envían exactamente el día en que se alcanza cada umbral.",
  "Available Variables": "Variables disponibles",
  "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}":
    "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}",
  // UPN QR Payment section
  "UPN QR Payment": "Pago UPN QR",
  "Configure UPN QR payment slip for invoices": "Configurar el comprobante de pago UPN QR para facturas",
  IBAN: "IBAN",
  "Bank account IBAN for receiving payments": "IBAN de la cuenta bancaria para recibir pagos",
  "Enable UPN QR on invoices": "Activar UPN QR en las facturas",
  "Show payment QR code on PDF invoices for easy mobile banking payments":
    "Mostrar código QR de pago en facturas PDF para pagos bancarios móviles fáciles",
  "Display Mode": "Modo de visualización",
  "QR code only": "Solo código QR",
  "Shows compact QR code inline with invoice content":
    "Muestra código QR compacto en línea con el contenido de la factura",
  "Full UPN payment slip": "Comprobante de pago UPN completo",
  "Shows complete payment slip at bottom of page":
    "Muestra el comprobante de pago completo en la parte inferior de la página",
  "Purpose Code": "Código de propósito",
  "OTHR - Other": "OTHR - Otro",
  "GDSV - Goods and Services": "GDSV - Bienes y servicios",
  "SUPP - Supplier Payment": "SUPP - Pago a proveedor",
  "Payment purpose code (ISO 20022)": "Código de propósito de pago (ISO 20022)",
  "UPN QR Payments": "Pagos UPN QR",
  "UPN QR is a Slovenian standard for payment slips. When enabled, your invoices will include a QR code that customers can scan with their mobile banking app to pay instantly.":
    "UPN QR es un estándar esloveno para comprobantes de pago. Cuando está activado, sus facturas incluirán un código QR que los clientes pueden escanear con su aplicación de banca móvil para pagar instantáneamente.",
  // Number format settings
  Invoice: "Factura",
  Estimate: "Presupuesto",
  "Credit Note": "Nota de crédito",
  "Advance Invoice": "Factura anticipada",
  "Number Format": "Formato de numeración",
  Preview: "Vista previa",
  // Tax rules settings
  "Main documents": "Documentos principales",
  "Used when no business unit is selected.": "Se usa cuando no hay ninguna unidad de negocio seleccionada.",
  "Business-unit documents": "Documentos de unidad de negocio",
  "Used when a business unit is selected. Default formats should include {u} to stay distinct.":
    "Se usa cuando se selecciona una unidad de negocio. Los formatos predeterminados deberían incluir {u} para mantenerse distintos.",
  "If this format matches the main document format, saving is blocked.":
    "Si este formato coincide con el formato principal, se bloquea el guardado.",
  "Business-unit numbering must differ from the main document format.":
    "La numeración de la unidad de negocio debe diferir del formato principal.",
  "First number to use": "Primer número a usar",
  "First number to use for this unit": "Primer número a usar para esta unidad",
  "first number to use": "primer número a usar",
  "Only affects this business unit for this document type. Existing documents are not renumbered.":
    "Solo afecta a esta unidad de negocio para este tipo de documento. Los documentos existentes no se renumeran.",
  "Set the first number for this unit by document type. The unit number format is shared in Numbering settings.":
    "Define el primer número de esta unidad por tipo de documento. El formato de numeración de unidades se comparte en los ajustes de numeración.",
  "Edit shared unit format": "Editar formato compartido de unidades",
  "first number": "primer número",
  "Unit numbering": "Numeración de la unidad",
  Numbering: "Numeración",
  "Only affects this business unit for the selected document type. Existing documents are not renumbered.":
    "Solo afecta a esta unidad de negocio para el tipo de documento seleccionado. Los documentos existentes no se renumeran.",
  "Can only move future numbering forward. Existing documents are not renumbered.":
    "Solo puede adelantar la numeración futura. Los documentos existentes no se renumeran.",
  "Insert token": "Insertar token",
  "Number tokens": "Tokens de numeración",
  "Full year": "Año completo",
  "Short year": "Año corto",
  "Sequence number": "Número secuencial",
  "Business unit index": "Índice de unidad de negocio",
  "EU Tax Rules": "Reglas fiscales de la UE",
  "Automatic tax handling for cross-border transactions":
    "Gestión automática de impuestos para transacciones transfronterizas",
  "Manual taxes": "Impuestos manuales",
  "Create tax rates on the Taxes page and apply them to document lines.":
    "Crea tasas fiscales en la página Impuestos y aplícalas a las líneas del documento.",
  "tax-rules.vies_validate_vat.label": "Validación de IVA VIES",
  "tax-rules.vies_validate_vat.description":
    "Verificar los números de IVA de la UE a través de la API VIES. Los números de IVA de los clientes se verifican antes de la creación de facturas.",
  "tax-rules.auto_reverse_charge.label": "Inversión del sujeto pasivo automática",
  "tax-rules.auto_reverse_charge.description":
    "Aplicar automáticamente el tipo 0% (inversión del sujeto pasivo) para transacciones B2B intra-UE con números de IVA válidos.",
  "tax-rules.auto_remove_tax_export.label": "Eliminación automática de impuestos de exportación",
  "tax-rules.auto_remove_tax_export.description":
    "Eliminar automáticamente todos los impuestos para exportaciones a países fuera de la UE.",
  "tax-rules.require_gross_prices.label": "Exigir precios brutos",
  "tax-rules.require_gross_prices.description":
    "Exigir precios brutos cuando se aplica la inversión del sujeto pasivo. Previene cambios inesperados en los importes totales de las facturas.",
  "tax-rules.calculation_mode.label": "Modo de cálculo",
  "tax-rules.calculation_mode.description":
    "Elija cómo se aplican los descuentos y el IVA al calcular los totales del documento.",
  "tax-rules.calculation_mode.b2b_standard.label": "Estándar B2B",
  "tax-rules.calculation_mode.b2b_standard.description":
    "Los descuentos reducen primero el importe neto de la línea y después se calcula el IVA.",
  "tax-rules.calculation_mode.b2c_gross_discount.label": "Descuento bruto B2C",
  "tax-rules.calculation_mode.b2c_gross_discount.description":
    "Los descuentos reducen el importe bruto final a pagar y, a partir de ese resultado, se obtienen el neto y el IVA.",
  // Tax clauses settings
  "Default Tax Clauses": "Cláusulas fiscales predeterminadas",
  "Set default tax clauses that are automatically added to documents based on transaction type":
    "Establezca las cláusulas fiscales predeterminadas que se añaden automáticamente a los documentos según el tipo de transacción.",
  "tax-clauses.other-title": "Cláusulas fiscales transfronterizas",
  "tax-clauses.other-description":
    "Cláusulas fiscales para transacciones UE e internacionales (inversión del sujeto pasivo, exportación).",
  "tax-clauses.intra_eu_b2b.label": "Intra-UE B2B (inversión del sujeto pasivo)",
  "tax-clauses.intra_eu_b2b.description":
    "Para ventas a empresas de la UE con números de IVA válidos. Normalmente incluye una declaración de inversión del sujeto pasivo.",
  "Enter reverse charge clause...": "Introduzca la cláusula de inversión del sujeto pasivo...",
  "tax-clauses.3w_b2b.label": "Fuera de UE B2B (exportación)",
  "tax-clauses.3w_b2b.description":
    "Para ventas a empresas fuera de la UE. Normalmente indica la exención de IVA por exportación.",
  "tax-clauses.3w_b2c.label": "Fuera de UE B2C (exportación)",
  "tax-clauses.3w_b2c.description":
    "Para ventas a consumidores fuera de la UE. Normalmente indica la exención de IVA por exportación.",
  "Enter export exemption clause...": "Introduzca la cláusula de exención de exportación...",
  "tax-clauses.domestic.label": "Predeterminado / Nacional",
  "tax-clauses.domestic.description":
    "Cláusula predeterminada para transacciones nacionales. Para entidades no sujetas a IVA, aparece en todos los documentos.",
  "Enter default tax clause...": "Introduzca la cláusula fiscal predeterminada...",
  "tax-clauses.intra_eu_b2c.label": "Intra-UE B2C",
  "Enter EU consumer sales clause...": "Introduzca la cláusula de venta a consumidores de la UE...",
  "tax-clauses.other.description":
    "Estos tipos de transacciones normalmente no requieren cláusulas fiscales específicas, pero puede configurarlas según sea necesario.",
  // EPC QR Payment section
  "EPC QR Payment": "Pago EPC QR",
  "SEPA credit transfer QR code for invoices": "Código QR de transferencia SEPA para facturas",
  "Enable EPC QR on invoices": "Activar EPC QR en facturas",
  "Show SEPA QR code on EUR invoices for easy bank payments":
    "Mostrar código QR SEPA en facturas en EUR para pagos bancarios fáciles",
  "EPC QR Payments": "Pagos EPC QR",
  "EPC QR is a European standard for SEPA credit transfers. When enabled, EUR invoices include a QR code that customers can scan with their banking app to pay instantly.":
    "EPC QR es un estándar europeo para transferencias SEPA. Cuando está activado, las facturas en EUR incluyen un código QR que los clientes pueden escanear con su app bancaria para pagar al instante.",
  // Document date defaults
  "Default due days": "Días de vencimiento predeterminados",
  "Default valid days": "Días de validez predeterminados",
  "Number of days added to the document date for new invoice due dates":
    "Número de días añadidos a la fecha del documento para nuevos vencimientos de facturas.",
  "Number of days added to the document date for new estimate valid-till dates":
    "Número de días añadidos a la fecha del documento para nuevas fechas de validez de presupuestos.",

  // Portugal entity details
  "Portugal Details": "Datos de Portugal",
  "Portuguese invoices carry these details. What is required depends on your legal form.":
    "Las facturas portuguesas incluyen estos datos. Cuáles son obligatorios depende de tu forma jurídica.",
  "Legal Form": "Forma jurídica",
  "Select your legal form": "Selecciona tu forma jurídica",
  "Decides which registration and capital details Portugal asks you for":
    "Determina qué datos de registro y de capital te exige Portugal",
  "Individual professional or sole trader": "Profesional independiente o empresario individual",
  "Private limited company (Lda.)": "Sociedad de responsabilidad limitada (Lda.)",
  "Public limited company (S.A.)": "Sociedad anónima (S.A.)",
  "Partnership limited by shares": "Sociedad comanditaria por acciones",
  "Other company": "Otra sociedad",
  "Company Number": "Número de empresa",
  "Digits and slashes only, as shown on your registration.": "Solo dígitos y barras, tal como aparece en su registro.",
  "Registry Office": "Oficina del registro mercantil",
  "Commercial registry where the company is registered": "Registro mercantil en el que está inscrita la sociedad",
  Phone: "Teléfono",
  "Enter 912 345 678 for Portugal, or include + and the country code for another country.":
    "Introduzca 912 345 678 para Portugal, o incluya + y el prefijo del país para otro país.",
  "Enter 7 digits; the hyphen is added automatically.": "Introduzca 7 dígitos; el guion se añade automáticamente.",
  "9 digits. You can paste it with spaces or a PT prefix.":
    "9 dígitos. Puede pegarlo con espacios o con el prefijo PT.",
  "Share Capital": "Capital social",
  "Registered share capital of the company": "Capital social registrado de la empresa",
  "Conditional company disclosures": "Menciones societarias condicionales",
  "Leave these empty unless one of them applies to your company.":
    "Deja estos campos vacíos salvo que alguno de ellos se aplique a tu sociedad.",
  "Paid-up Capital": "Capital desembolsado",
  "Only needed when less than the share capital has actually been paid in":
    "Solo es necesario si se ha desembolsado efectivamente menos del capital social",
  Equity: "Patrimonio neto",
  "Only needed when equity has fallen to half the share capital or less. Use the figure from your last approved balance sheet.":
    "Solo es necesario si el patrimonio neto ha caído a la mitad del capital social o menos. Usa la cifra de tu último balance aprobado.",
  "Company is in liquidation": "Sociedad en liquidación",
  "Documents must say so while the company is being wound up":
    "Los documentos deben indicarlo mientras dure la liquidación de la sociedad",
  // Tax rule sections and Slovenian Article 76.a
  "Calculation settings": "Configuración de cálculo",
  "Choose how prices, discounts, and taxes are calculated on documents.":
    "Elige cómo se calculan los precios, los descuentos y los impuestos en los documentos.",
  "EU cross border rules": "Reglas transfronterizas de la UE",
  "Validate VAT numbers and automate reverse charge for eligible EU transactions.":
    "Valida los números de IVA y automatiza la inversión del sujeto pasivo en las operaciones elegibles de la UE.",
  "Export rules": "Reglas de exportación",
  "Configure automatic tax handling for transactions outside the EU.":
    "Configura el tratamiento fiscal automático de las operaciones fuera de la UE.",
  "Configure domestic Slovenian tax treatments.": "Configura los tratamientos fiscales nacionales de Eslovenia.",
  "Slovenian Article 76.a": "Artículo 76.a de Eslovenia",
  "Allow eligible domestic business invoices to report VAT under reverse charge.":
    "Permite que las facturas nacionales entre empresas elegibles declaren el IVA mediante inversión del sujeto pasivo.",
  "Article 76.a tax clause": "Cláusula fiscal del artículo 76.a",
  "Default legal clause added when Article 76.a is selected on an invoice.":
    "Cláusula legal predeterminada que se añade al seleccionar el artículo 76.a en una factura.",
  "Obrnjena davčna obveznost po 76.a členu ZDDV-1.": "Obrnjena davčna obveznost po 76.a členu ZDDV-1.",
} as const;
