export default {
  Draft: "Brouillon",
  Number: "Numéro",
  Customer: "Client",
  Date: "Date",
  "Date Due": "Date d'échéance",
  Total: "Total",
  "Total with Tax": "Total TTC",
  Actions: "Actions",
  "Open menu": "Ouvrir le menu",
  "Copy invoice ID": "Copier l'ID de la facture",
  "View invoice": "Voir la facture",
  "Add Payment": "Ajouter un paiement",
  "Download PDF": "Télécharger le PDF",
  "Downloading...": "Téléchargement...",
  Duplicate: "Dupliquer",
  "Send Email": "Envoyer un e-mail",
  "Send reminder": "Envoyer le rappel",
  Invoice: "Facture",
  "Failed to download PDF": "Échec du téléchargement du PDF",
  // Filter translations
  "Search...": "Rechercher...",
  Filters: "Filtres",
  "Date Range": "Plage de dates",
  From: "Du",
  To: "Au",
  "Clear filters": "Effacer les filtres",
  Status: "Statut",
  Paid: "Payé",
  Unpaid: "Non payé",
  Overdue: "En retard",
  Voided: "Annulé",
  "Created At": "Créé le",
  Unshare: "Annuler le partage",
  // Empty state translations
  "Your list is empty": "Votre liste de factures est vide",
  "Get started by creating your first entry": "Commencez par créer votre première facture",
  "Create new": "Créer une facture",
  // No results translations
  "No results found": "Aucune facture trouvée",
  "Try adjusting your search criteria": "Essayez de modifier vos critères de recherche",
  "Clear search": "Effacer la recherche",
  "Clear all": "Tout effacer",
  selected: "sélectionnés",
  "Export PDFs": "Exporter les PDF",
  "Deselect all": "Tout désélectionner",
  Void: "Annuler",
  "Partially Paid": "Partiellement payé",
  "FURS fiscalization failed": "La fiscalisation FURS a échoué",
  "FINA fiscalization failed": "La fiscalisation FINA a échoué",
  "Retry Fiscalization": "Réessayer la fiscalisation",
  "Some selected documents don't have failed fiscalization":
    "Certains documents sélectionnés n'ont pas de fiscalisation en échec",
} as const;
