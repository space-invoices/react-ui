export default {
  // Actions
  PDF: "PDF",
  Send: "Invia",
  Payment: "Pagamento",
  Share: "Condividi",
  Unshare: "Rimuovi condivisione",
  Copied: "Copiato",
  "Copy shareable link": "Copia link condivisibile",
  "Unshare document": "Rimuovi condivisione documento",
  "Unshare document confirmation": "Sei sicuro di voler rimuovere l'accesso pubblico a questo documento?",
  "Document unshared": "Documento non più condiviso",
  "Unshare failed": "Rimozione condivisione fallita",
  "Download failed": "Download fallito",
  "e-SLOG download failed": "Download e-SLOG fallito",
  "UJP package": "Pacchetto UJP",
  "UJP package download failed": "Download del pacchetto UJP fallito",
  XRechnung: "XRechnung",
  ZUGFeRD: "ZUGFeRD",
  "XRechnung download failed": "Download XRechnung fallito",
  "ZUGFeRD download failed": "Download ZUGFeRD fallito",
  "e-SLOG validation must be valid before downloading.": "La validazione e-SLOG deve essere valida prima del download.",
  "UJP validation must be valid before downloading.": "La validazione UJP deve essere valida prima del download.",
  "German e-invoice validation must be valid before downloading.":
    "La validazione della fattura elettronica tedesca deve essere valida prima del download.",
  "XRechnung validation must be valid before downloading.":
    "La validazione XRechnung deve essere valida prima del download.",
  "ZUGFeRD validation must be valid before downloading.":
    "La validazione ZUGFeRD deve essere valida prima del download.",
  Recurring: "Ricorrente",
  Duplicate: "Duplica",
  "Duplicate invoice": "Duplica fattura",
  "Duplicate estimate": "Duplica preventivo",
  "Duplicate credit_note": "Duplica nota di credito",
  "Duplicate advance_invoice": "Duplica fattura di acconto",
  "Duplicate delivery_note": "Duplica bolla di consegna",
  "Create invoice": "Crea fattura",
  "Create estimate": "Crea preventivo",
  "Create credit_note": "Crea nota di credito",
  "Create advance_invoice": "Crea fattura di acconto",
  "Create delivery_note": "Crea bolla di consegna",

  // Details card
  Details: "Dettagli",
  Number: "Numero",
  Date: "Data",
  "Due date": "Data di scadenza",
  "Service date": "Data del servizio",
  "Service period": "Periodo di servizio",
  "Valid until": "Valido fino al",
  Customer: "Cliente",
  Discount: "Sconto",
  Subtotal: "Subtotale",
  Tax: "Imposta",
  of: "di",
  Total: "Totale",
  Paid: "Pagato",
  Due: "Da pagare",
  Status: "Stato",

  // Payment status
  "Paid in full": "Pagato interamente",
  "Partially paid": "Parzialmente pagato",
  Unpaid: "Non pagato",
  Voided: "Annullato",
  Overdue: "Scaduto",
  Shareable: "Condivisibile",
  "Proforma invoice": "Fattura proforma",
  "Packing list": "Lista di imballaggio",
  "FURS fiscalization failed": "Fiscalizzazione FURS non riuscita",
  "FINA fiscalization failed": "Fiscalizzazione FINA non riuscita",

  // Payments list
  Payments: "Pagamenti",
  Refunds: "Rimborsi",
  "No payments": "Nessun pagamento",
  "No refunds": "Nessun rimborso",
  "Add payment": "Aggiungi pagamento",
  Edit: "Modifica",
  Delete: "Elimina",
  "Delete payment": "Elimina pagamento",
  "Delete payment confirmation": "Sei sicuro di voler eliminare questo pagamento?",
  Cancel: "Annulla",

  // Payment types
  cash: "Contanti",
  bank_transfer: "Bonifico bancario",
  card: "Carta",
  check: "Assegno",
  credit_note: "Nota di credito",
  other: "Altro",
  advance_invoice: "Fattura di acconto",
  advance_payment: "Pagamento anticipato",

  // Errors
  "Delete failed": "Eliminazione fallita",

  // Draft actions
  Finalize: "Finalizza",
  "Delete Draft": "Elimina bozza",
  "Finalize document": "Finalizza documento",
  "Finalize document confirmation":
    "Questo assegnerà un numero definitivo al documento. Dopo la finalizzazione il documento non può più essere eliminato. Vuoi continuare?",
  "Delete draft document": "Elimina bozza documento",
  "Delete draft confirmation": "Sei sicuro di voler eliminare questa bozza? Questa azione non può essere annullata.",
  "Document finalized": "Documento finalizzato",
  "Finalize failed": "Finalizzazione fallita",
  "Draft deleted": "Bozza eliminata",
  "Delete draft failed": "Eliminazione bozza fallita",
  Draft: "Bozza",

  // Void
  Void: "Annulla",
  "Voiding...": "Annullamento...",

  // Activity list
  Activity: "Attività",
  "No activity": "Nessuna attività",
  Created: "Creato",
  Updated: "Aggiornato",
  Sent: "Inviato",
  Deleted: "Eliminato",
  by: "da",
  me: "me",
  User: "Utente",
  System: "Sistema",
  Scheduled: "Pianificato",
  "Version history": "Cronologia versioni",
  "No version history": "Nessuna cronologia versioni",
  Version: "Versione",

  // Fiscalization
  Fiscalization: "Fiscalizzazione",
  Fiscalized: "Fiscalizzato",
  Pending: "In attesa",
  Failed: "Fallito",
  Skipped: "Saltato",
  "Skipped by user": "Saltato dall'utente",
  "Not fiscalized": "Non fiscalizzato",
  Fiscalize: "Fiscalizza",
  "Retry fiscalization": "Riprova fiscalizzazione",

  // PDF language names
  English: "Inglese",
  German: "Tedesco",
  Slovenian: "Sloveno",
  Italian: "Italiano",
  French: "Francese",
  Spanish: "Spagnolo",
  Portuguese: "Portoghese",
  Dutch: "Olandese",
  Polish: "Polacco",
  Croatian: "Croato",
  Swedish: "Svedese",
  Finnish: "Finlandese",
  Estonian: "Estone",
  Bulgarian: "Bulgaro",
  Czech: "Ceco",
  Slovak: "Slovacco",
  Norwegian: "Norvegese",
  Icelandic: "Islandese",

  // Linked documents
  "Linked documents": "Documenti collegati",
  "No linked documents": "Nessun documento collegato",
  Invoice: "Fattura",
  Estimate: "Preventivo",
  "Credit note": "Nota di credito",
  "Advance invoice": "Fattura di acconto",
  "Delivery note": "Bolla di consegna",
  "Credit for": "Credito per",
  "Converted from": "Convertito da",
  "Converted to": "Convertito in",
  "Has credit": "Ha credito",
  "Advance applied": "Acconto applicato",
  "Applied to": "Applicato a",
  "Delivered for": "Consegnato per",
  "Has delivery": "Ha bolla di consegna",
  Fulfills: "Adempie",
  "Fulfilled by": "Adempiuto da",
  References: "Riferisce a",
  View: "Visualizza",
} as const;
