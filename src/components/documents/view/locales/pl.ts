export default {
  // Actions
  PDF: "PDF",
  Send: "Wyślij",
  Payment: "Płatność",
  Share: "Udostępnij",
  Unshare: "Cofnij udostępnienie",
  Copied: "Skopiowano",
  "Copy shareable link": "Kopiuj link do udostępnienia",
  "Unshare document": "Cofnij udostępnienie dokumentu",
  "Unshare document confirmation": "Czy na pewno chcesz usunąć publiczny dostęp do tego dokumentu?",
  "Document unshared": "Dokument nie jest już udostępniony",
  "Unshare failed": "Cofnięcie udostępnienia nie powiodło się",
  "Download failed": "Pobieranie nie powiodło się",
  "e-SLOG download failed": "Pobieranie e-SLOG nie powiodło się",
  "UJP package": "Pakiet UJP",
  "UJP package download failed": "Pobieranie pakietu UJP nie powiodło się",
  XRechnung: "XRechnung",
  ZUGFeRD: "ZUGFeRD",
  "XRechnung download failed": "Pobieranie XRechnung nie powiodło się",
  "ZUGFeRD download failed": "Pobieranie ZUGFeRD nie powiodło się",
  "e-SLOG validation must be valid before downloading.": "Walidacja e-SLOG musi być poprawna przed pobraniem.",
  "UJP validation must be valid before downloading.": "Walidacja UJP musi być poprawna przed pobraniem.",
  "German e-invoice validation must be valid before downloading.":
    "Walidacja niemieckiej e-faktury musi być prawidłowa przed pobraniem.",
  "XRechnung validation must be valid before downloading.": "Walidacja XRechnung musi byc poprawna przed pobraniem.",
  "ZUGFeRD validation must be valid before downloading.": "Walidacja ZUGFeRD musi byc poprawna przed pobraniem.",
  Recurring: "Cykliczna",
  Duplicate: "Duplikuj",
  "Duplicate invoice": "Duplikuj fakturę",
  "Duplicate estimate": "Duplikuj kosztorys",
  "Duplicate credit_note": "Duplikuj notę kredytową",
  "Duplicate advance_invoice": "Duplikuj fakturę zaliczkową",
  "Duplicate delivery_note": "Duplikuj dowód dostawy",
  "Create invoice": "Utwórz fakturę",
  "Create estimate": "Utwórz kosztorys",
  "Create credit_note": "Utwórz notę kredytową",
  "Create advance_invoice": "Utwórz fakturę zaliczkową",
  "Create delivery_note": "Utwórz dowód dostawy",

  // Details card
  Details: "Szczegóły",
  Number: "Numer",
  Date: "Data",
  "Due date": "Termin płatności",
  "Service date": "Data usługi",
  "Service period": "Okres usługi",
  "Valid until": "Ważne do",
  Customer: "Klient",
  Discount: "Rabat",
  Subtotal: "Suma częściowa",
  Tax: "Podatek",
  of: "z",
  Total: "Razem",
  Paid: "Zapłacono",
  Due: "Do zapłaty",
  Status: "Status",

  // Payment status
  "Paid in full": "Zapłacono w całości",
  "Partially paid": "Częściowo zapłacono",
  Unpaid: "Niezapłacono",
  Voided: "Anulowano",
  Overdue: "Przeterminowane",
  Shareable: "Udostępnialne",
  "Proforma invoice": "Faktura pro forma",
  "Packing list": "Lista pakowania",
  "FURS fiscalization failed": "Fiscalizacja FURS nie powiodła się",
  "FINA fiscalization failed": "Fiscalizacja FINA nie powiodła się",

  // Payments list
  Payments: "Płatności",
  Refunds: "Zwroty",
  "No payments": "Brak płatności",
  "No refunds": "Brak zwrotów",
  "Add payment": "Dodaj płatność",
  Edit: "Edytuj",
  Delete: "Usuń",
  "Delete payment": "Usuń płatność",
  "Delete payment confirmation": "Czy na pewno chcesz usunąć tę płatność?",
  Cancel: "Anuluj",

  // Payment types
  cash: "Gotówka",
  bank_transfer: "Przelew bankowy",
  card: "Karta",
  check: "Czek",
  credit_note: "Nota kredytowa",
  other: "Inne",
  advance_invoice: "Faktura zaliczkowa",
  advance_payment: "Płatność zaliczkowa",

  // Errors
  "Delete failed": "Usunięcie nie powiodło się",

  // Draft actions
  Finalize: "Zatwierdź",
  "Delete Draft": "Usuń szkic",
  "Finalize document": "Zatwierdź dokument",
  "Finalize document confirmation":
    "Spowoduje to nadanie dokumentowi ostatecznego numeru. Po zatwierdzeniu dokumentu nie można go usunąć. Czy chcesz kontynuować?",
  "Delete draft document": "Usuń szkic dokumentu",
  "Delete draft confirmation": "Czy na pewno chcesz usunąć ten szkic? Tej operacji nie można cofnąć.",
  "Document finalized": "Dokument zatwierdzony",
  "Finalize failed": "Zatwierdzenie nie powiodło się",
  "Draft deleted": "Szkic usunięty",
  "Delete draft failed": "Usunięcie szkicu nie powiodło się",
  Draft: "Szkic",

  // Void
  Void: "Anuluj",
  "Voiding...": "Anulowanie...",

  // Activity list
  Activity: "Aktywność",
  "No activity": "Brak aktywności",
  Created: "Utworzono",
  Updated: "Zaktualizowano",
  Sent: "Wysłano",
  Deleted: "Usunięto",
  by: "przez",
  me: "mnie",
  User: "Użytkownik",
  System: "System",
  Scheduled: "Zaplanowano",
  "Version history": "Historia wersji",
  "No version history": "Brak historii wersji",
  Version: "Wersja",

  // Fiscalization
  Fiscalization: "Fiskalizacja",
  Fiscalized: "Zfiskalizowano",
  Pending: "Oczekuje",
  Failed: "Niepowodzenie",
  Skipped: "Pominięto",
  "Skipped by user": "Pominięto przez użytkownika",
  "Not fiscalized": "Nie zafiskalizowano",
  Fiscalize: "Fiskalizuj",
  "Retry fiscalization": "Ponów fiskalizację",

  // PDF language names
  English: "Angielski",
  German: "Niemiecki",
  Slovenian: "Słoweński",
  Italian: "Włoski",
  French: "Francuski",
  Spanish: "Hiszpański",
  Portuguese: "Portugalski",
  Dutch: "Niderlandzki",
  Polish: "Polski",
  Croatian: "Chorwacki",
  Swedish: "Szwedzki",
  Finnish: "Fiński",
  Estonian: "Estoński",
  Bulgarian: "Bułgarski",
  Czech: "Czeski",
  Slovak: "Słowacki",
  Norwegian: "Norweski",
  Icelandic: "Islandzki",

  // Linked documents
  "Linked documents": "Powiązane dokumenty",
  "No linked documents": "Brak powiązanych dokumentów",
  Invoice: "Faktura",
  Estimate: "Kosztorys",
  "Credit note": "Nota kredytowa",
  "Advance invoice": "Faktura zaliczkowa",
  "Delivery note": "Dowód dostawy",
  "Credit for": "Kredyt dla",
  "Converted from": "Przekonwertowano z",
  "Converted to": "Przekonwertowano na",
  "Has credit": "Ma kredyt",
  "Advance applied": "Zaliczka zastosowana",
  "Applied to": "Zastosowano do",
  "Delivered for": "Dostarczone dla",
  "Has delivery": "Ma dowód dostawy",
  Fulfills: "Realizuje",
  "Fulfilled by": "Zrealizowano przez",
  References: "Odnosi się do",
  View: "Zobacz",
} as const;
