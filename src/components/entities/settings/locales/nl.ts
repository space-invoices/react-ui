export default {
  // Company settings
  "Tax subject": "Belastingplichtige",
  "Tax ID 2": "Belasting-ID 2",
  "Secondary tax identification number (optional)": "Secundair belastingidentificatienummer (optioneel)",
  "Company Information": "Bedrijfsinformatie",
  "Basic information about your company": "Basisinformatie over uw bedrijf",
  // Branding settings
  Branding: "Huisstijl",
  "Primary Color": "Primaire kleur",
  Logo: "Logo",
  "Logo URL": "Logo-URL",
  "Enter the URL of your company logo": "Voer de URL van uw bedrijfslogo in",
  "Upload your company logo for invoices": "Upload uw bedrijfslogo voor facturen",
  "Upload Logo": "Logo uploaden",
  "Change Logo": "Logo wijzigen",
  "Current logo (displayed on invoices)": "Huidig logo (weergegeven op facturen)",
  "Logo preview": "Logo-voorbeeld",
  "Crop Your Logo": "Uw logo bijsnijden",
  Signature: "Handtekening",
  "Upload a signature image for PDFs (optional)": "Upload een handtekeningafbeelding voor PDF's (optioneel)",
  "Upload Signature": "Handtekening uploaden",
  "Change Signature": "Handtekening wijzigen",
  "Current signature (for PDF documents)": "Huidige handtekening (voor PDF-documenten)",
  "Signature preview": "Handtekeningvoorbeeld",
  "Crop Your Signature": "Uw handtekening bijsnijden",
  "Adjust the crop area or upload the full image": "Pas het bijsnijdgebied aan of upload de volledige afbeelding",
  "Upload failed. Please try again.": "Upload mislukt. Probeer het opnieuw.",
  "Uploading...": "Uploaden...",
  "Upload & Save": "Uploaden en opslaan",
  Cancel: "Annuleren",
  Localization: "Lokalisatie",
  "Currency Code": "Valutacode",
  "ISO 4217 currency code (e.g., USD, EUR, GBP)": "ISO 4217 valutacode (bijv. USD, EUR, GBP)",
  Locale: "Landinstelling",
  "BCP 47 language tag (e.g., en-US, de-DE, sl-SI)": "BCP 47 taaltag (bijv. en-US, de-DE, sl-SI)",
  "Select currency": "Valuta selecteren",
  "Select locale": "Landinstelling selecteren",
  "Configure entity localization": "Lokalisatie van de entiteit configureren",
  "English (US)": "Engels (US)",
  "German (DE)": "Duits (DE)",
  "Italian (IT)": "Italiaans (IT)",
  "French (FR)": "Frans (FR)",
  "Spanish (ES)": "Spaans (ES)",
  "Portuguese (Portugal)": "Portugees (Portugal)",
  Dutch: "Nederlands",
  Polish: "Pools",
  Croatian: "Kroatisch",
  "Slovenian (SI)": "Sloveens (SI)",
  "Email Settings": "E-mailinstellingen",
  "Configure email settings for invoices": "E-mailinstellingen configureren voor facturen",
  "Email Address": "E-mailadres",
  "Email address to send invoices to": "E-mailadres voor het verzenden van facturen",
  "Sender email": "Afzender-e-mail",
  "Billing email address": "E-mailadres voor facturatie",
  "Recipient for white-label subscription billing emails.":
    "Ontvanger voor facturatie-e-mails van het white-labelabonnement.",
  "Used as the From address for document emails and payment reminders after verification passes.":
    "Wordt gebruikt als From-adres voor documentmails en betalingsherinneringen nadat de verificatie is geslaagd.",
  "DNS DKIM tokens": "DNS DKIM-tokens",
  "Add CNAME records for these tokens at your sender domain, then recheck.":
    "Voeg CNAME-records voor deze tokens toe op je afzenderdomein en controleer opnieuw.",
  "DNS records are being prepared. Recheck in a moment if they do not appear.":
    "DNS-records worden voorbereid. Controleer straks opnieuw als ze niet verschijnen.",
  "CNAME record": "CNAME-record",
  Name: "Naam",
  Value: "Waarde",
  Copy: "Kopiëren",
  Copied: "Gekopieerd",
  Saving: "Opslaan",
  "Save sender": "Afzender opslaan",
  "Update sender": "Afzender bijwerken",
  Clear: "Wissen",
  Checking: "Controleren",
  Recheck: "Opnieuw controleren",
  Pending: "In behandeling",
  Verified: "Geverifieerd",
  Failed: "Mislukt",
  Email: "E-mail",
  Domain: "Domein",
  "Inherited from white-label": "Overgenomen van white-label",
  "Entity Email": "E-mail van entiteit",
  "Invalid email address": "Ongeldig e-mailadres",
  "Email shown for this entity on documents and template variables":
    "E-mail die voor deze entiteit wordt weergegeven in documenten en sjabloonvariabelen",
  "Email Subject": "E-mailonderwerp",
  "Subject line for email invoices": "Onderwerpregel voor e-mailfacturen",
  "Email Body": "E-mailtekst",
  "Body content for email invoices": "Tekstinhoud voor e-mailfacturen",
  "Subject line for invoice emails": "Onderwerp voor factuur-e-mails",
  "Subject line for estimate emails": "Onderwerp voor offerte-e-mails",
  "Subject line for credit note emails": "Onderwerp voor creditnota-e-mails",
  "Subject line for advance invoice emails": "Onderwerp voor voorschotfactuur-e-mails",
  "Subject line for delivery note emails": "Onderwerp voor pakbon-e-mails",
  "Body content for invoice emails": "Tekst voor factuur-e-mails",
  "Body content for estimate emails": "Tekst voor offerte-e-mails",
  "Body content for credit note emails": "Tekst voor creditnota-e-mails",
  "Body content for advance invoice emails": "Tekst voor voorschotfactuur-e-mails",
  "Body content for delivery note emails": "Tekst voor pakbon-e-mails",
  Reminder: "Herinnering",
  "Payment reminders": "Betalingsherinneringen",
  "Reminder subject": "Onderwerp van herinnering",
  "Subject line for payment reminder emails": "Onderwerpregel voor betalingsherinneringen",
  "Reminder body": "Tekst van herinnering",
  "Body content for payment reminder emails": "Inhoud van betalingsherinneringen",
  "Payment instructions": "Betaalinstructies",
  "Payment instructions appended to reminder emails when not included in the body":
    "Betaalinstructies die aan herinneringen worden toegevoegd wanneer ze niet in de tekst staan",
  "Payment reminder from {entity_name}": "Betalingsherinnering van {entity_name}",
  "Hello {customer_name},\n\nThe following invoices are overdue:\n{invoice_list}\n\n{payment_instructions}":
    "Hallo {customer_name},\n\nDe volgende facturen zijn achterstallig:\n{invoice_list}\n\n{payment_instructions}",
  "Please remit payment to IBAN {iban}.": "Maak de betaling over naar IBAN {iban}.",
  "Save Settings": "Instellingen opslaan",
  // Document Defaults section
  "Document Defaults": "Standaardwaarden documenten",
  "Default values for new documents": "Standaardwaarden voor nieuwe documenten",
  "Default Note": "Standaardnotitie",
  "Default Invoice Note": "Standaard factuurnotitie",
  "This note will be pre-filled when creating new invoices":
    "Deze notitie wordt vooraf ingevuld bij het aanmaken van nieuwe facturen",
  "This note will be pre-filled when creating new estimates":
    "Deze notitie wordt vooraf ingevuld bij het aanmaken van nieuwe offertes",
  "This note will be pre-filled when creating new credit notes":
    "Deze notitie wordt vooraf ingevuld bij het aanmaken van nieuwe creditnota's",
  "Payment terms pre-filled when creating new invoices":
    "Betalingsvoorwaarden vooraf ingevuld bij het aanmaken van nieuwe facturen",
  "Payment terms pre-filled when creating new estimates":
    "Betalingsvoorwaarden vooraf ingevuld bij het aanmaken van nieuwe offertes",
  "Payment terms pre-filled when creating new credit notes":
    "Betalingsvoorwaarden vooraf ingevuld bij het aanmaken van nieuwe creditnota's",
  "Optional note for the document.": "Optionele notitie voor het document.",
  "Please remit payment using the bank details shown on the document.":
    "Gelieve te betalen met de bankgegevens op het document.",
  "This estimate is valid until {document_valid_until}.": "Deze offerte is geldig tot {document_valid_until}.",
  "Payment due upon acceptance.": "Betaling bij aanvaarding.",
  "Credit note for invoice {document_number}.": "Creditnota voor factuur {document_number}.",
  "Credit will be applied to your account.": "Het tegoed wordt op uw rekening verrekend.",
  "Invoice Notes": "Factuurnotities",
  "Smart Template Variables": "Slimme sjabloonvariabelen",
  "Use variables to personalize your notes automatically":
    "Gebruik variabelen om uw notities automatisch te personaliseren",
  "Set a default note that will appear on all new invoices. Use template variables to personalize the note automatically.":
    "Stel een standaardnotitie in die op alle nieuwe facturen verschijnt. Gebruik sjabloonvariabelen om de notitie automatisch te personaliseren.",
  // Smart code insert button
  "Insert variable": "Variabele invoegen",
  Bold: "Vet",
  Italic: "Cursief",
  "Bullet list": "Opsommingstekens",
  Entity: "Bedrijf",
  Document: "Document",
  Customer: "Klant",
  Other: "Overig",
  "Company name": "Bedrijfsnaam",
  "Email address": "E-mailadres",
  "Invoice number": "Factuurnummer",
  "Invoice date": "Factuurdatum",
  "Due date": "Vervaldatum",
  "Total amount": "Totaalbedrag",
  Currency: "Valuta",
  "Customer name": "Klantnaam",
  "Customer email": "E-mail klant",
  "Today's date": "Datum van vandaag",
  "Current year": "Huidig jaar",
  // Payment terms defaults
  "Default Payment Terms": "Standaard betalingsvoorwaarden",
  "This payment terms will be pre-filled when creating new documents":
    "Deze betalingsvoorwaarden worden vooraf ingevuld bij het aanmaken van nieuwe documenten",
  "Payment Terms": "Betalingsvoorwaarden",
  "Add payment terms...": "Betalingsvoorwaarden toevoegen...",
  // Document footer
  "Document Footer": "Documentvoettekst",
  "Footer text displayed at the bottom of PDF documents": "Voettekst weergegeven onderaan PDF-documenten",
  "Footer text displayed at the bottom of all PDF documents": "Voettekst weergegeven onderaan alle PDF-documenten",
  "{entity_name} | Due Date: {document_due_date} | Invoice #{document_number}":
    "{entity_name} | Vervaldatum: {document_due_date} | Factuur #{document_number}",
  // Document signature
  "Document Signature": "Documenthandtekening",
  "Signature text displayed on all PDF documents": "Handtekeningtekst weergegeven op alle PDF-documenten",
  "Add signature text...": "Handtekeningtekst toevoegen...",
  "Delivery Note": "Afleverbon",
  "Default note for advance invoices": "Standaardnotitie voor voorschotfacturen",
  "Default note for all new advance invoices": "Standaardnotitie voor alle nieuwe voorschotfacturen",
  "Default note for delivery notes": "Standaardnotitie voor afleverbonnen",
  "Default note for all new delivery notes": "Standaardnotitie voor alle nieuwe afleverbonnen",
  // Overdue Notifications section
  "Overdue Notifications": "Herinneringen voor achterstallige betalingen",
  "Automatically remind customers about overdue invoices": "Klanten automatisch herinneren aan achterstallige facturen",
  "Enable Notifications": "Meldingen activeren",
  "Send automatic payment reminder emails to customers with overdue invoices":
    "Stuur automatische betalingsherinneringen per e-mail naar klanten met achterstallige facturen",
  "Reminder Days": "Herinneringsdagen",
  "Days after due date to send reminders (comma-separated). E.g., '7, 14, 30' sends reminders at 7, 14, and 30 days overdue.":
    "Dagen na vervaldatum om herinneringen te sturen (kommagescheiden). Bijv. '7, 14, 30' stuurt herinneringen na 7, 14 en 30 dagen achterstand.",
  "Custom Reminder Templates": "Aangepaste herinneringssjablonen",
  "Subject line for overdue notification emails": "Onderwerpregel voor e-mails over achterstallige betalingen",
  "Body content for overdue emails. Use {invoice_list} to include the invoice table, or it will be auto-appended.":
    "Tekstinhoud voor achterstallige e-mails. Gebruik {invoice_list} om de factuurtabel op te nemen, anders wordt deze automatisch toegevoegd.",
  "Dear {customer_name},\n\nThis is a reminder about your overdue invoices...":
    "Geachte {customer_name},\n\nDit is een herinnering over uw achterstallige facturen...",
  "Payment Reminders": "Betalingsherinneringen",
  "When enabled, customers will automatically receive email reminders when their invoices become overdue. Emails are sent on the exact day each threshold is reached.":
    "Wanneer ingeschakeld, ontvangen klanten automatisch e-mailherinneringen wanneer hun facturen achterstallig worden. E-mails worden verzonden op de exacte dag dat elke drempel wordt bereikt.",
  "Available Variables": "Beschikbare variabelen",
  "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}":
    "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}",
  // UPN QR Payment section
  "UPN QR Payment": "UPN QR-betaling",
  "Configure UPN QR payment slip for invoices": "UPN QR-betalingsslip configureren voor facturen",
  IBAN: "IBAN",
  "Bank account IBAN for receiving payments": "IBAN van de bankrekening voor het ontvangen van betalingen",
  "Enable UPN QR on invoices": "UPN QR inschakelen op facturen",
  "Show payment QR code on PDF invoices for easy mobile banking payments":
    "Toon betalings-QR-code op PDF-facturen voor eenvoudige mobiele bankbetalingen",
  "Display Mode": "Weergavemodus",
  "QR code only": "Alleen QR-code",
  "Shows compact QR code inline with invoice content": "Toont compacte QR-code in lijn met de factuurinhoud",
  "Full UPN payment slip": "Volledig UPN-betalingsslip",
  "Shows complete payment slip at bottom of page": "Toont volledig betalingsslip onderaan de pagina",
  "Purpose Code": "Doelcode",
  "OTHR - Other": "OTHR - Overig",
  "GDSV - Goods and Services": "GDSV - Goederen en diensten",
  "SUPP - Supplier Payment": "SUPP - Leveranciersbetaling",
  "Payment purpose code (ISO 20022)": "Betalingsdoelcode (ISO 20022)",
  "UPN QR Payments": "UPN QR-betalingen",
  "UPN QR is a Slovenian standard for payment slips. When enabled, your invoices will include a QR code that customers can scan with their mobile banking app to pay instantly.":
    "UPN QR is een Sloveense standaard voor betalingsslips. Wanneer ingeschakeld, bevatten uw facturen een QR-code die klanten kunnen scannen met hun mobiele bankapp om direct te betalen.",
  // Number format settings
  Invoice: "Factuur",
  Estimate: "Offerte",
  "Credit Note": "Creditnota",
  "Advance Invoice": "Voorschotfactuur",
  "Number Format": "Nummerformaat",
  Preview: "Voorbeeld",
  // Tax rules settings
  "Main documents": "Hoofddocumenten",
  "Used when no business unit is selected.": "Wordt gebruikt wanneer geen bedrijfseenheid is geselecteerd.",
  "Business-unit documents": "Documenten van bedrijfseenheid",
  "Used when a business unit is selected. Default formats should include {u} to stay distinct.":
    "Wordt gebruikt wanneer een bedrijfseenheid is geselecteerd. Standaardformaten moeten {u} bevatten om onderscheidbaar te blijven.",
  "If this format matches the main document format, saving is blocked.":
    "Als dit formaat overeenkomt met het hoofdformaat, wordt opslaan geblokkeerd.",
  "Business-unit numbering must differ from the main document format.":
    "Nummering van bedrijfseenheden moet verschillen van het hoofdformaat.",
  "First number to use": "Eerste nummer om te gebruiken",
  "First number to use for this unit": "Eerste nummer om te gebruiken voor deze eenheid",
  "first number to use": "eerste nummer om te gebruiken",
  "Only affects this business unit for this document type. Existing documents are not renumbered.":
    "Heeft alleen invloed op deze bedrijfseenheid voor dit documenttype. Bestaande documenten worden niet hernummerd.",
  "Set the first number for this unit by document type. The unit number format is shared in Numbering settings.":
    "Stel het eerste nummer voor deze eenheid per documenttype in. De nummerindeling voor eenheden wordt gedeeld in de nummeringsinstellingen.",
  "Edit shared unit format": "Gedeelde eenheidsindeling bewerken",
  "first number": "eerste nummer",
  "Unit numbering": "Nummering van de eenheid",
  Numbering: "Nummering",
  "Only affects this business unit for the selected document type. Existing documents are not renumbered.":
    "Heeft alleen invloed op deze bedrijfseenheid voor het geselecteerde documenttype. Bestaande documenten worden niet hernummerd.",
  "Can only move future numbering forward. Existing documents are not renumbered.":
    "Kan toekomstige nummering alleen verhogen. Bestaande documenten worden niet hernummerd.",
  "Insert token": "Token invoegen",
  "Number tokens": "Nummeringstokens",
  "Full year": "Volledig jaar",
  "Short year": "Kort jaar",
  "Sequence number": "Volgnummer",
  "Business unit index": "Index bedrijfseenheid",
  "EU Tax Rules": "EU-belastingregels",
  "Automatic tax handling for cross-border transactions":
    "Automatische belastingafhandeling voor grensoverschrijdende transacties",
  "Manual taxes": "Handmatige belastingen",
  "Create tax rates on the Taxes page and apply them to document lines.":
    "Maak belastingtarieven aan op de pagina Belastingen en pas ze toe op documentregels.",
  "tax-rules.vies_validate_vat.label": "VIES btw-validatie",
  "tax-rules.vies_validate_vat.description":
    "Verifieer EU-btw-nummers via de VIES API. Btw-nummers van klanten worden geverifieerd vóór het aanmaken van facturen.",
  "tax-rules.auto_reverse_charge.label": "Automatische verlegging",
  "tax-rules.auto_reverse_charge.description":
    "Pas automatisch het 0%-tarief toe (verlegging) voor intra-EU B2B-transacties met geldige btw-nummers.",
  "tax-rules.auto_remove_tax_export.label": "Automatische verwijdering exportbelasting",
  "tax-rules.auto_remove_tax_export.description":
    "Verwijder automatisch alle belastingen voor exporten naar landen buiten de EU.",
  "tax-rules.require_gross_prices.label": "Brutoprijzen vereisen",
  "tax-rules.require_gross_prices.description":
    "Brutoprijzen vereisen wanneer verlegging wordt toegepast. Voorkomt onverwachte wijzigingen in factuurtotalen.",
  "tax-rules.calculation_mode.label": "Berekeningsmethode",
  "tax-rules.calculation_mode.description":
    "Kies hoe kortingen en btw worden toegepast bij het berekenen van documenttotalen.",
  "tax-rules.calculation_mode.b2b_standard.label": "B2B-standaard",
  "tax-rules.calculation_mode.b2b_standard.description":
    "Kortingen verlagen eerst het nettobedrag van de regel, daarna wordt de btw berekend.",
  "tax-rules.calculation_mode.b2c_gross_discount.label": "B2C-brutokorting",
  "tax-rules.calculation_mode.b2c_gross_discount.description":
    "Kortingen verlagen het uiteindelijke bruto te betalen bedrag, waarna het netto- en btw-bedrag daaruit worden afgeleid.",
  // Tax clauses settings
  "Default Tax Clauses": "Standaard belastingclausules",
  "Set default tax clauses that are automatically added to documents based on transaction type":
    "Stel standaard belastingclausules in die automatisch worden toegevoegd aan documenten op basis van transactietype.",
  "tax-clauses.other-title": "Grensoverschrijdende belastingclausules",
  "tax-clauses.other-description": "Belastingclausules voor EU- en internationale transacties (verlegging, export).",
  "tax-clauses.intra_eu_b2b.label": "Intra-EU B2B (verlegging)",
  "tax-clauses.intra_eu_b2b.description":
    "Voor verkopen aan EU-bedrijven met geldige btw-nummers. Bevat meestal een verklaring van verlegging.",
  "Enter reverse charge clause...": "Voer de verleggingsclausule in...",
  "tax-clauses.3w_b2b.label": "Niet-EU B2B (export)",
  "tax-clauses.3w_b2b.description":
    "Voor verkopen aan bedrijven buiten de EU. Vermeldt meestal de btw-vrijstelling voor export.",
  "tax-clauses.3w_b2c.label": "Niet-EU B2C (export)",
  "tax-clauses.3w_b2c.description":
    "Voor verkopen aan consumenten buiten de EU. Vermeldt meestal de btw-vrijstelling voor export.",
  "Enter export exemption clause...": "Voer de exportvrijstellingsclausule in...",
  "tax-clauses.domestic.label": "Standaard / Binnenlands",
  "tax-clauses.domestic.description":
    "Standaardclausule voor binnenlandse transacties. Voor niet-btw-plichtige entiteiten verschijnt deze op alle documenten.",
  "Enter default tax clause...": "Voer de standaard belastingclausule in...",
  "tax-clauses.intra_eu_b2c.label": "Intra-EU B2C",
  "Enter EU consumer sales clause...": "Voer de EU-consumentenverkoopclausule in...",
  "tax-clauses.other.description":
    "Deze soorten transacties vereisen doorgaans geen specifieke belastingclausules, maar u kunt ze naar behoefte configureren.",
  // EPC QR Payment section
  "EPC QR Payment": "EPC QR Betaling",
  "SEPA credit transfer QR code for invoices": "SEPA-overboeking QR-code voor facturen",
  "Enable EPC QR on invoices": "EPC QR op facturen inschakelen",
  "Show SEPA QR code on EUR invoices for easy bank payments":
    "Toon SEPA QR-code op EUR-facturen voor eenvoudige bankbetalingen",
  "EPC QR Payments": "EPC QR Betalingen",
  "EPC QR is a European standard for SEPA credit transfers. When enabled, EUR invoices include a QR code that customers can scan with their banking app to pay instantly.":
    "EPC QR is een Europese standaard voor SEPA-overboekingen. Wanneer ingeschakeld, bevatten EUR-facturen een QR-code die klanten kunnen scannen met hun bankapp om direct te betalen.",
  // Document date defaults
  "Default due days": "Standaard vervaldagen",
  "Default valid days": "Standaard geldigheidsdagen",
  "Number of days added to the document date for new invoice due dates":
    "Aantal dagen dat aan de documentdatum wordt toegevoegd voor nieuwe vervaldatums van facturen.",
  "Number of days added to the document date for new estimate valid-till dates":
    "Aantal dagen dat aan de documentdatum wordt toegevoegd voor nieuwe geldigheidsdatums van offertes.",

  // Portugal entity details
  "Portugal Details": "Portugal-gegevens",
  "Portuguese law requires these details on every entity": "De Portugese wet vereist deze gegevens voor elke entiteit",
  "Company Number": "Ondernemingsnummer",
  "Company registration number (NIPC)": "Registratienummer van het bedrijf (NIPC)",
  Phone: "Telefoon",
  "International format, including the country code": "Internationaal formaat, inclusief landnummer",
  "Share Capital": "Maatschappelijk kapitaal",
  "Registered share capital of the company": "Geregistreerd maatschappelijk kapitaal van het bedrijf",
  // Tax rule sections and Slovenian Article 76.a
  "Calculation settings": "Berekeningsinstellingen",
  "Choose how prices, discounts, and taxes are calculated on documents.":
    "Kies hoe prijzen, kortingen en belastingen op documenten worden berekend.",
  "EU cross border rules": "EU-regels voor grensoverschrijdende transacties",
  "Validate VAT numbers and automate reverse charge for eligible EU transactions.":
    "Valideer btw-nummers en automatiseer de verleggingsregeling voor in aanmerking komende EU-transacties.",
  "Export rules": "Exportregels",
  "Configure automatic tax handling for transactions outside the EU.":
    "Configureer automatische belastingverwerking voor transacties buiten de EU.",
  "Configure domestic Slovenian tax treatments.": "Configureer binnenlandse Sloveense fiscale behandelingen.",
  "Slovenian Article 76.a": "Sloveens artikel 76.a",
  "Allow eligible domestic business invoices to report VAT under reverse charge.":
    "Sta toe dat in aanmerking komende binnenlandse zakelijke facturen de btw via de verleggingsregeling vermelden.",
  "Article 76.a tax clause": "Fiscale clausule volgens artikel 76.a",
  "Default legal clause added when Article 76.a is selected on an invoice.":
    "Standaard juridische clausule die wordt toegevoegd wanneer artikel 76.a op een factuur is geselecteerd.",
  "Obrnjena davčna obveznost po 76.a členu ZDDV-1.": "Obrnjena davčna obveznost po 76.a členu ZDDV-1.",
} as const;
