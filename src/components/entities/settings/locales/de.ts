export default {
  // Company settings
  "Tax subject": "Steuerpflichtig",
  "Tax ID 2": "Steuernummer",
  "Secondary tax identification number (optional)": "Sekundäre Steueridentifikationsnummer (optional)",
  "Company Information": "Unternehmensinformationen",
  "Basic information about your company": "Grundlegende Informationen über Ihr Unternehmen",
  // Branding settings
  Branding: "Markenauftritt",
  "Primary Color": "Primärfarbe",
  Logo: "Logo",
  "Logo URL": "Logo-URL",
  "Enter the URL of your company logo": "Geben Sie die URL Ihres Firmenlogos ein",
  "Upload your company logo for invoices": "Laden Sie Ihr Firmenlogo für Rechnungen hoch",
  "Upload Logo": "Logo hochladen",
  "Change Logo": "Logo ändern",
  "Current logo (displayed on invoices)": "Aktuelles Logo (auf Rechnungen angezeigt)",
  "Logo preview": "Logo-Vorschau",
  "Crop Your Logo": "Logo zuschneiden",
  Signature: "Unterschrift",
  "Upload a signature image for PDFs (optional)": "Laden Sie ein Unterschriftsbild für PDFs hoch (optional)",
  "Upload Signature": "Unterschrift hochladen",
  "Change Signature": "Unterschrift ändern",
  "Current signature (for PDF documents)": "Aktuelle Unterschrift (für PDF-Dokumente)",
  "Signature preview": "Unterschrift-Vorschau",
  "Crop Your Signature": "Unterschrift zuschneiden",
  "Adjust the crop area or upload the full image":
    "Passen Sie den Zuschneidebereich an oder laden Sie das vollständige Bild hoch",
  "Upload failed. Please try again.": "Upload fehlgeschlagen. Bitte versuchen Sie es erneut.",
  "Uploading...": "Hochladen...",
  "Upload & Save": "Hochladen & Speichern",
  Cancel: "Abbrechen",
  Localization: "Lokalisierung",
  "Currency Code": "Währungscode",
  "ISO 4217 currency code (e.g., USD, EUR, GBP)": "ISO 4217 Währungscode (z.B. USD, EUR, GBP)",
  Locale: "Gebietsschema",
  "BCP 47 language tag (e.g., en-US, de-DE, sl-SI)": "BCP 47 Sprach-Tag (z.B. en-US, de-DE, sl-SI)",
  "Select currency": "Währung auswählen",
  "Select locale": "Gebietsschema auswählen",
  "Configure entity localization": "Lokalisierung konfigurieren",
  "English (US)": "Englisch (US)",
  "German (DE)": "Deutsch (DE)",
  "Italian (IT)": "Italienisch (IT)",
  "French (FR)": "Französisch (FR)",
  "Spanish (ES)": "Spanisch (ES)",
  "Portuguese (Portugal)": "Portugiesisch (Portugal)",
  Dutch: "Niederländisch",
  Polish: "Polnisch",
  Croatian: "Kroatisch",
  "Slovenian (SI)": "Slowenisch (SI)",
  "Email Settings": "E-Mail-Einstellungen",
  "Configure email settings for invoices": "Konfigurieren Sie E-Mail-Einstellungen für Rechnungen",
  "Email Address": "E-Mail-Adresse",
  "Email address to send invoices to": "E-Mail-Adresse zum Versenden von Rechnungen",
  "Sender email": "Absender-E-Mail",
  "Billing email address": "E-Mail-Adresse für Abrechnung",
  "Recipient for white-label subscription billing emails.":
    "Empfänger für Abrechnungs-E-Mails des White-Label-Abonnements.",
  "Used as the From address for document emails and payment reminders after verification passes.":
    "Wird nach erfolgreicher Verifizierung als From-Adresse für Dokument-E-Mails und Zahlungserinnerungen verwendet.",
  "DNS DKIM tokens": "DNS-DKIM-Token",
  "Add CNAME records for these tokens at your sender domain, then recheck.":
    "Fügen Sie CNAME-Einträge für diese Token in Ihrer Absenderdomain hinzu und prüfen Sie danach erneut.",
  "DNS records are being prepared. Recheck in a moment if they do not appear.":
    "DNS-Einträge werden vorbereitet. Prüfen Sie in Kürze erneut, falls sie nicht erscheinen.",
  "CNAME record": "CNAME-Eintrag",
  Name: "Name",
  Value: "Wert",
  Copy: "Kopieren",
  Copied: "Kopiert",
  Saving: "Speichern",
  "Save sender": "Absender speichern",
  "Update sender": "Absender aktualisieren",
  Clear: "Leeren",
  Checking: "Wird geprüft",
  Recheck: "Erneut prüfen",
  Pending: "Ausstehend",
  Verified: "Verifiziert",
  Failed: "Fehlgeschlagen",
  Email: "E-Mail",
  Domain: "Domain",
  "Inherited from white-label": "Vom White-Label übernommen",
  "Entity Email": "E-Mail des Unternehmens",
  "Invalid email address": "Ungültige E-Mail-Adresse",
  "Email shown for this entity on documents and template variables":
    "E-Mail, die für dieses Unternehmen in Dokumenten und Vorlagenvariablen angezeigt wird",
  "Email Subject": "E-Mail-Betreff",
  "Subject line for email invoices": "Betreffzeile für E-Mail-Rechnungen",
  "Email Body": "E-Mail-Text",
  "Body content for email invoices": "Textinhalt für E-Mail-Rechnungen",
  "Subject line for invoice emails": "Betreffzeile für Rechnungs-E-Mails",
  "Subject line for estimate emails": "Betreffzeile für Angebots-E-Mails",
  "Subject line for credit note emails": "Betreffzeile für Gutschrift-E-Mails",
  "Subject line for advance invoice emails": "Betreffzeile für Vorausrechnungs-E-Mails",
  "Subject line for delivery note emails": "Betreffzeile für Lieferschein-E-Mails",
  "Body content for invoice emails": "Textinhalt für Rechnungs-E-Mails",
  "Body content for estimate emails": "Textinhalt für Angebots-E-Mails",
  "Body content for credit note emails": "Textinhalt für Gutschrift-E-Mails",
  "Body content for advance invoice emails": "Textinhalt für Vorausrechnungs-E-Mails",
  "Body content for delivery note emails": "Textinhalt für Lieferschein-E-Mails",
  Reminder: "Erinnerung",
  "Payment reminders": "Zahlungserinnerungen",
  "Reminder subject": "Betreff der Erinnerung",
  "Subject line for payment reminder emails": "Betreffzeile für Zahlungserinnerungen",
  "Reminder body": "Erinnerungstext",
  "Body content for payment reminder emails": "Nachrichtentext für Zahlungserinnerungen",
  "Payment instructions": "Zahlungsanweisungen",
  "Payment instructions appended to reminder emails when not included in the body":
    "Zahlungsanweisungen, die an Zahlungserinnerungen angehängt werden, wenn sie nicht im Text enthalten sind",
  "Payment reminder from {entity_name}": "Zahlungserinnerung von {entity_name}",
  "Hello {customer_name},\n\nThe following invoices are overdue:\n{invoice_list}\n\n{payment_instructions}":
    "Hallo {customer_name},\n\nDie folgenden Rechnungen sind überfällig:\n{invoice_list}\n\n{payment_instructions}",
  "Please remit payment to IBAN {iban}.": "Bitte überweisen Sie die Zahlung an IBAN {iban}.",
  "Save Settings": "Einstellungen speichern",
  // Document Defaults section
  "Document Defaults": "Dokumentvorlagen",
  "Default values for new documents": "Standardwerte für neue Dokumente",
  "Default Note": "Standardnotiz",
  "Default Invoice Note": "Standard-Rechnungsnotiz",
  "This note will be pre-filled when creating new invoices":
    "Diese Notiz wird beim Erstellen neuer Rechnungen vorausgefüllt",
  "This note will be pre-filled when creating new estimates":
    "Diese Notiz wird beim Erstellen neuer Angebote vorausgefüllt",
  "This note will be pre-filled when creating new credit notes":
    "Diese Notiz wird beim Erstellen neuer Gutschriften vorausgefüllt",
  "Payment terms pre-filled when creating new invoices":
    "Zahlungsbedingungen werden beim Erstellen neuer Rechnungen vorausgefüllt",
  "Payment terms pre-filled when creating new estimates":
    "Zahlungsbedingungen werden beim Erstellen neuer Angebote vorausgefüllt",
  "Payment terms pre-filled when creating new credit notes":
    "Zahlungsbedingungen werden beim Erstellen neuer Gutschriften vorausgefüllt",
  "Optional note for the document.": "Optionale Notiz für das Dokument.",
  "Please remit payment using the bank details shown on the document.":
    "Bitte verwenden Sie die auf dem Dokument angegebenen Bankdaten.",
  "This estimate is valid until {document_valid_until}.": "Dieses Angebot ist gültig bis {document_valid_until}.",
  "Payment due upon acceptance.": "Zahlung bei Annahme fällig.",
  "Credit note for invoice {document_number}.": "Gutschrift für Rechnung {document_number}.",
  "Credit will be applied to your account.": "Die Gutschrift wird Ihrem Konto gutgeschrieben.",
  "Invoice Notes": "Rechnungsnotizen",
  "Smart Template Variables": "Intelligente Vorlagenvariablen",
  "Use variables to personalize your notes automatically":
    "Verwenden Sie Variablen zur automatischen Personalisierung Ihrer Notizen",
  "Set a default note that will appear on all new invoices. Use template variables to personalize the note automatically.":
    "Legen Sie eine Standardnotiz fest, die auf allen neuen Rechnungen erscheint. Verwenden Sie Vorlagenvariablen zur automatischen Personalisierung.",
  // Smart code insert button
  "Insert variable": "Variable einfügen",
  Bold: "Fett",
  Italic: "Kursiv",
  "Bullet list": "Aufzählung",
  Entity: "Unternehmen",
  Document: "Dokument",
  Customer: "Kunde",
  Other: "Andere",
  "Company name": "Firmenname",
  "Email address": "E-Mail-Adresse",
  "Invoice number": "Rechnungsnummer",
  "Invoice date": "Rechnungsdatum",
  "Due date": "Fälligkeitsdatum",
  "Total amount": "Gesamtbetrag",
  Currency: "Währung",
  "Customer name": "Kundenname",
  "Customer email": "Kunden-E-Mail",
  "Today's date": "Heutiges Datum",
  "Current year": "Aktuelles Jahr",
  // Payment terms defaults
  "Default Payment Terms": "Standard-Zahlungsbedingungen",
  "This payment terms will be pre-filled when creating new documents":
    "Diese Zahlungsbedingungen werden beim Erstellen neuer Dokumente vorausgefüllt",
  "Payment Terms": "Zahlungsbedingungen",
  "Add payment terms...": "Zahlungsbedingungen hinzufügen...",
  // Document footer
  "Document Footer": "Dokumentfußzeile",
  "Footer text displayed at the bottom of PDF documents": "Fußzeilentext am unteren Rand von PDF-Dokumenten",
  "Footer text displayed at the bottom of all PDF documents": "Fußzeilentext am unteren Rand aller PDF-Dokumente",
  "{entity_name} | Due Date: {document_due_date} | Invoice #{document_number}":
    "{entity_name} | Fällig am: {document_due_date} | Rechnung #{document_number}",
  // Document signature
  "Document Signature": "Dokumentunterschrift",
  "Signature text displayed on all PDF documents": "Unterschriftstext auf allen PDF-Dokumenten",
  "Add signature text...": "Unterschriftstext hinzufügen...",
  "Delivery Note": "Lieferschein",
  "Default note for advance invoices": "Standardnotiz für Vorausrechnungen",
  "Default note for all new advance invoices": "Standardnotiz für alle neuen Vorausrechnungen",
  "Default note for delivery notes": "Standardnotiz für Lieferscheine",
  "Default note for all new delivery notes": "Standardnotiz für alle neuen Lieferscheine",
  // Overdue Notifications section
  "Overdue Notifications": "Überfälligkeits-Benachrichtigungen",
  "Automatically remind customers about overdue invoices": "Erinnern Sie Kunden automatisch an überfällige Rechnungen",
  "Enable Notifications": "Benachrichtigungen aktivieren",
  "Send automatic payment reminder emails to customers with overdue invoices":
    "Senden Sie automatische Zahlungserinnerungen per E-Mail an Kunden mit überfälligen Rechnungen",
  "Reminder Days": "Erinnerungstage",
  "Days after due date to send reminders (comma-separated). E.g., '7, 14, 30' sends reminders at 7, 14, and 30 days overdue.":
    "Tage nach Fälligkeit für Erinnerungen (durch Komma getrennt). Z.B. '7, 14, 30' sendet Erinnerungen bei 7, 14 und 30 Tagen Überfälligkeit.",
  "Custom Reminder Templates": "Benutzerdefinierte Erinnerungsvorlagen",
  "Subject line for overdue notification emails": "Betreffzeile für Überfälligkeits-E-Mails",
  "Body content for overdue emails. Use {invoice_list} to include the invoice table, or it will be auto-appended.":
    "Textinhalt für Überfälligkeits-E-Mails. Verwenden Sie {invoice_list} für die Rechnungstabelle, sonst wird sie automatisch angehängt.",
  "Dear {customer_name},\n\nThis is a reminder about your overdue invoices...":
    "Sehr geehrte(r) {customer_name},\n\nDies ist eine Erinnerung an Ihre überfälligen Rechnungen...",
  "Payment Reminders": "Zahlungserinnerungen",
  "When enabled, customers will automatically receive email reminders when their invoices become overdue. Emails are sent on the exact day each threshold is reached.":
    "Wenn aktiviert, erhalten Kunden automatisch E-Mail-Erinnerungen, wenn ihre Rechnungen überfällig werden. E-Mails werden genau am Tag gesendet, an dem jeder Schwellenwert erreicht wird.",
  "Available Variables": "Verfügbare Variablen",
  "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}":
    "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}",
  // UPN QR Payment section
  "UPN QR Payment": "UPN QR Zahlung",
  "Configure UPN QR payment slip for invoices": "UPN QR-Zahlungsbeleg für Rechnungen konfigurieren",
  IBAN: "IBAN",
  "Bank account IBAN for receiving payments": "IBAN des Bankkontos für den Zahlungsempfang",
  "Enable UPN QR on invoices": "UPN QR auf Rechnungen aktivieren",
  "Show payment QR code on PDF invoices for easy mobile banking payments":
    "QR-Code für Zahlung auf PDF-Rechnungen anzeigen für einfache Mobile-Banking-Zahlungen",
  "Display Mode": "Anzeigemodus",
  "QR code only": "Nur QR-Code",
  "Shows compact QR code inline with invoice content": "Zeigt kompakten QR-Code inline mit Rechnungsinhalt",
  "Full UPN payment slip": "Vollständiger UPN-Zahlungsbeleg",
  "Shows complete payment slip at bottom of page": "Zeigt vollständigen Zahlungsbeleg am Seitenende",
  "Purpose Code": "Verwendungszweck-Code",
  "OTHR - Other": "OTHR - Sonstiges",
  "GDSV - Goods and Services": "GDSV - Waren und Dienstleistungen",
  "SUPP - Supplier Payment": "SUPP - Lieferantenzahlung",
  "Payment purpose code (ISO 20022)": "Zahlungszweck-Code (ISO 20022)",
  "UPN QR Payments": "UPN QR-Zahlungen",
  "UPN QR is a Slovenian standard for payment slips. When enabled, your invoices will include a QR code that customers can scan with their mobile banking app to pay instantly.":
    "UPN QR ist ein slowenischer Standard für Zahlungsbelege. Wenn aktiviert, enthalten Ihre Rechnungen einen QR-Code, den Kunden mit ihrer Mobile-Banking-App scannen können, um sofort zu bezahlen.",
  // Number format settings
  Invoice: "Rechnung",
  Estimate: "Angebot",
  "Credit Note": "Gutschrift",
  "Advance Invoice": "Vorausrechnung",
  "Number Format": "Nummernformat",
  Preview: "Vorschau",
  // Tax rules settings
  "Main documents": "Hauptdokumente",
  "Used when no business unit is selected.": "Wird verwendet, wenn keine Geschäftseinheit ausgewählt ist.",
  "Business-unit documents": "Dokumente der Geschäftseinheit",
  "Used when a business unit is selected. Default formats should include {u} to stay distinct.":
    "Wird verwendet, wenn eine Geschäftseinheit ausgewählt ist. Standardformate sollten {u} enthalten, damit sie eindeutig bleiben.",
  "If this format matches the main document format, saving is blocked.":
    "Wenn dieses Format dem Hauptdokumentformat entspricht, wird das Speichern blockiert.",
  "Business-unit numbering must differ from the main document format.":
    "Die Nummerierung der Geschäftseinheit muss sich vom Hauptdokumentformat unterscheiden.",
  "First number to use": "Erste zu verwendende Nummer",
  "First number to use for this unit": "Erste zu verwendende Nummer für diese Einheit",
  "first number to use": "erste zu verwendende Nummer",
  "Only affects this business unit for this document type. Existing documents are not renumbered.":
    "Gilt nur für diese Geschäftseinheit und diesen Dokumenttyp. Bestehende Dokumente werden nicht neu nummeriert.",
  "Set the first number for this unit by document type. The unit number format is shared in Numbering settings.":
    "Legen Sie die erste Nummer für diese Einheit je Dokumenttyp fest. Das Nummernformat für Einheiten wird gemeinsam in den Nummerierungseinstellungen verwaltet.",
  "Edit shared unit format": "Gemeinsames Einheitsformat bearbeiten",
  "first number": "erste Nummer",
  "Unit numbering": "Einheitsnummerierung",
  Numbering: "Nummerierung",
  "Only affects this business unit for the selected document type. Existing documents are not renumbered.":
    "Gilt nur für diese Geschäftseinheit und den ausgewählten Dokumenttyp. Bestehende Dokumente werden nicht neu nummeriert.",
  "Can only move future numbering forward. Existing documents are not renumbered.":
    "Kann zukünftige Nummerierung nur erhöhen. Bestehende Dokumente werden nicht neu nummeriert.",
  "Insert token": "Token einfügen",
  "Number tokens": "Nummern-Tokens",
  "Full year": "Vollständiges Jahr",
  "Short year": "Kurzes Jahr",
  "Sequence number": "Fortlaufende Nummer",
  "Business unit index": "Index der Geschäftseinheit",
  "EU Tax Rules": "EU-Steuerregeln",
  "Automatic tax handling for cross-border transactions":
    "Automatische Steuerbehandlung für grenzüberschreitende Transaktionen",
  "Manual taxes": "Manuelle Steuern",
  "Create tax rates on the Taxes page and apply them to document lines.":
    "Erstellen Sie Steuersätze auf der Seite Steuern und wenden Sie sie auf Dokumentzeilen an.",
  "tax-rules.vies_validate_vat.label": "VIES USt-IdNr. Prüfung",
  "tax-rules.vies_validate_vat.description":
    "EU USt-IdNr. über die VIES-API validieren. USt-IdNr. von Kunden werden vor Rechnungserstellung geprüft.",
  "tax-rules.auto_reverse_charge.label": "Automatisches Reverse-Charge",
  "tax-rules.auto_reverse_charge.description":
    "Automatisch 0% Steuer (Reverse-Charge) für innergemeinschaftliche B2B-Transaktionen mit gültiger USt-IdNr. anwenden.",
  "tax-rules.auto_remove_tax_export.label": "Automatisch Exportsteuer entfernen",
  "tax-rules.auto_remove_tax_export.description": "Automatisch alle Steuern für Exporte in Nicht-EU-Länder entfernen.",
  "tax-rules.require_gross_prices.label": "Bruttopreise erforderlich",
  "tax-rules.require_gross_prices.description":
    "Bruttopreise bei Reverse-Charge erforderlich. Verhindert unerwartete Änderungen der Rechnungssummen.",
  "tax-rules.calculation_mode.label": "Berechnungsmodus",
  "tax-rules.calculation_mode.description":
    "Wählen Sie, in welcher Reihenfolge Rabatte und Umsatzsteuer bei der Dokumentberechnung angewendet werden.",
  "tax-rules.calculation_mode.b2b_standard.label": "B2B-Standard",
  "tax-rules.calculation_mode.b2b_standard.description":
    "Rabatte reduzieren zuerst den Nettozeilenbetrag, danach wird die Umsatzsteuer berechnet.",
  "tax-rules.calculation_mode.b2c_gross_discount.label": "B2C-Bruttorabatt",
  "tax-rules.calculation_mode.b2c_gross_discount.description":
    "Rabatte reduzieren den endgültigen Bruttobetrag, danach werden Netto und Umsatzsteuer daraus abgeleitet.",
  // Tax clauses settings
  "Default Tax Clauses": "Standard-Steuerklauseln",
  "Set default tax clauses that are automatically added to documents based on transaction type":
    "Legen Sie Standard-Steuerklauseln fest, die automatisch zu Dokumenten basierend auf dem Transaktionstyp hinzugefügt werden.",
  "tax-clauses.other-title": "Grenzüberschreitende Steuerklauseln",
  "tax-clauses.other-description": "Steuerklauseln für EU- und internationale Transaktionen (Reverse-Charge, Export).",
  "tax-clauses.intra_eu_b2b.label": "Innergemeinschaftlich B2B (Reverse-Charge)",
  "tax-clauses.intra_eu_b2b.description":
    "Für Verkäufe an EU-Unternehmen mit gültiger USt-IdNr. Enthält typischerweise eine Reverse-Charge-Erklärung.",
  "Enter reverse charge clause...": "Reverse-Charge-Klausel eingeben...",
  "tax-clauses.3w_b2b.label": "Nicht-EU B2B (Export)",
  "tax-clauses.3w_b2b.description":
    "Für Verkäufe an Unternehmen außerhalb der EU. Enthält typischerweise die Steuerbefreiung für Exporte.",
  "tax-clauses.3w_b2c.label": "Nicht-EU B2C (Export)",
  "tax-clauses.3w_b2c.description":
    "Für Verkäufe an Verbraucher außerhalb der EU. Enthält typischerweise die Steuerbefreiung für Exporte.",
  "Enter export exemption clause...": "Exportbefreiungsklausel eingeben...",
  "tax-clauses.domestic.label": "Standard / Inländisch",
  "tax-clauses.domestic.description":
    "Standardklausel für inländische Transaktionen. Für Kleinunternehmer erscheint diese auf allen Dokumenten.",
  "Enter default tax clause...": "Standard-Steuerklausel eingeben...",
  "tax-clauses.intra_eu_b2c.label": "Innergemeinschaftlich B2C",
  "Enter EU consumer sales clause...": "EU-Verbraucherklausel eingeben...",
  "tax-clauses.other.description":
    "Diese Transaktionstypen erfordern normalerweise keine speziellen Steuerklauseln, können aber bei Bedarf konfiguriert werden.",
  // EPC QR Payment section
  "EPC QR Payment": "EPC QR Zahlung",
  "SEPA credit transfer QR code for invoices": "SEPA-Überweisungs-QR-Code für Rechnungen",
  "Enable EPC QR on invoices": "EPC QR auf Rechnungen aktivieren",
  "Show SEPA QR code on EUR invoices for easy bank payments":
    "SEPA-QR-Code auf EUR-Rechnungen für einfache Bankzahlungen anzeigen",
  "EPC QR Payments": "EPC QR Zahlungen",
  "EPC QR is a European standard for SEPA credit transfers. When enabled, EUR invoices include a QR code that customers can scan with their banking app to pay instantly.":
    "EPC QR ist ein europäischer Standard für SEPA-Überweisungen. Wenn aktiviert, enthalten EUR-Rechnungen einen QR-Code, den Kunden mit ihrer Banking-App scannen können, um sofort zu bezahlen.",
  // Document date defaults
  "Default due days": "Standard-Fälligkeitstage",
  "Default valid days": "Standard-Gültigkeitstage",
  "Number of days added to the document date for new invoice due dates":
    "Anzahl der Tage, die dem Dokumentdatum für neue Rechnungsfälligkeiten hinzugefügt werden.",
  "Number of days added to the document date for new estimate valid-till dates":
    "Anzahl der Tage, die dem Dokumentdatum für neue Angebotsgültigkeiten hinzugefügt werden.",

  // Portugal entity details
  "Portugal Details": "Portugal-Details",
  "Portuguese invoices carry these details. What is required depends on your legal form.":
    "Portugiesische Rechnungen enthalten diese Angaben. Welche davon Pflicht sind, hängt von Ihrer Rechtsform ab.",
  "Legal Form": "Rechtsform",
  "Select your legal form": "Rechtsform auswählen",
  "Decides which registration and capital details Portugal asks you for":
    "Bestimmt, welche Registrierungs- und Kapitalangaben Portugal von Ihnen verlangt",
  "Individual professional or sole trader": "Freiberufler oder Einzelunternehmer",
  "Private limited company (Lda.)": "Gesellschaft mit beschränkter Haftung (Lda.)",
  "Public limited company (S.A.)": "Aktiengesellschaft (S.A.)",
  "Partnership limited by shares": "Kommanditgesellschaft auf Aktien",
  "Other company": "Sonstige Gesellschaft",
  "Company Number": "Handelsregisternummer",
  "Digits and slashes only, as shown on your registration.":
    "Nur Ziffern und Schrägstriche, wie in Ihrer Registrierung angegeben.",
  "Registry Office": "Handelsregisteramt",
  "Commercial registry where the company is registered": "Handelsregister, in dem die Gesellschaft eingetragen ist",
  Phone: "Telefon",
  "Enter 912 345 678 for Portugal, or include + and the country code for another country.":
    "Geben Sie für Portugal 912 345 678 ein oder für ein anderes Land + und die Ländervorwahl.",
  "Enter 7 digits; the hyphen is added automatically.":
    "Geben Sie 7 Ziffern ein; der Bindestrich wird automatisch ergänzt.",
  "9 digits. You can paste it with spaces or a PT prefix.":
    "9 Ziffern. Sie können sie mit Leerzeichen oder mit dem Präfix PT einfügen.",
  "Share Capital": "Stammkapital",
  "Registered share capital of the company": "Eingetragenes Stammkapital des Unternehmens",
  "Conditional company disclosures": "Bedingte Pflichtangaben zur Gesellschaft",
  "Leave these empty unless one of them applies to your company.":
    "Lassen Sie diese Felder leer, sofern keine der Angaben auf Ihre Gesellschaft zutrifft.",
  "Paid-up Capital": "Eingezahltes Kapital",
  "Only needed when less than the share capital has actually been paid in":
    "Nur erforderlich, wenn weniger als das Stammkapital tatsächlich eingezahlt wurde",
  Equity: "Eigenkapital",
  "Only needed when equity has fallen to half the share capital or less. Use the figure from your last approved balance sheet.":
    "Nur erforderlich, wenn das Eigenkapital auf die Hälfte des Stammkapitals oder weniger gesunken ist. Verwenden Sie den Wert aus Ihrer letzten festgestellten Bilanz.",
  "Company is in liquidation": "Gesellschaft in Liquidation",
  "Documents must say so while the company is being wound up":
    "Während der Abwicklung der Gesellschaft muss dies auf den Dokumenten angegeben werden",
  // Tax rule sections and Slovenian Article 76.a
  "Calculation settings": "Berechnungseinstellungen",
  "Choose how prices, discounts, and taxes are calculated on documents.":
    "Legen Sie fest, wie Preise, Rabatte und Steuern auf Dokumenten berechnet werden.",
  "EU cross border rules": "EU-Regeln für grenzüberschreitende Transaktionen",
  "Validate VAT numbers and automate reverse charge for eligible EU transactions.":
    "Umsatzsteuer-Identifikationsnummern prüfen und Reverse Charge für berechtigte EU-Transaktionen automatisieren.",
  "Export rules": "Exportregeln",
  "Configure automatic tax handling for transactions outside the EU.":
    "Automatische Steuerbehandlung für Transaktionen außerhalb der EU konfigurieren.",
  "Configure domestic Slovenian tax treatments.": "Inländische slowenische Steuerregelungen konfigurieren.",
  "Slovenian Article 76.a": "Slowenischer Artikel 76.a",
  "Allow eligible domestic business invoices to report VAT under reverse charge.":
    "Berechtigten inländischen Geschäftsrechnungen ermöglichen, die Umsatzsteuer per Reverse Charge auszuweisen.",
  "Article 76.a tax clause": "Steuerklausel nach Artikel 76.a",
  "Default legal clause added when Article 76.a is selected on an invoice.":
    "Standardmäßige Rechtsklausel, die hinzugefügt wird, wenn Artikel 76.a auf einer Rechnung ausgewählt ist.",
  "Obrnjena davčna obveznost po 76.a členu ZDDV-1.": "Obrnjena davčna obveznost po 76.a členu ZDDV-1.",
} as const;
