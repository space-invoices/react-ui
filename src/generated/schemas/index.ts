/**
 * This file was automatically generated using 'bun generate-schemas'.
 * Do not edit this file manually. To update, run the generator again.
 * @generated
 */

export * from './acceptentityinvitation_body';
export * from './accountwebhook';
export * from './addentityuser_body';
export * from './advanceinvoice';
export * from './appendpostabitems_body';
export * from './authorizeshopify_body';
export * from './backfillexpenserecognition';
export * from './backfillrevenuerecognition_body';
export * from './businessunit';
export * from './calculatedocumentpreview';
export * from './checkeinvoicingrecipient';
export * from './checktransactiontype';
export * from './claimpostab';
export * from './closepostab_body';
export * from './confirmpasswordreset_body';
export * from './country-capability-defaults';
export * from './creditnote';
export * from './customadvanceinvoice';
export * from './customcreditnote';
export * from './customdeliverynote';
export * from './customer';
export * from './customestimate';
export * from './customexpense';
export * from './custominvoice';
export * from './deliverynote';
export * from './documentitemfinancialcategories';
export * from './documentitemfinancialcategory';
export * from './einvoice';
export * from './einvoicingdocument';
export * from './einvoicingenrollmentauthorization';
export * from './einvoicingsettings';
export * from './einvoicingsupplier';
export * from './enrolposoperator_body';
export * from './entity';
export * from './entityapikey';
export * from './entityuserrole';
export * from './estimate';
export * from './expense';
export * from './expenseattachment';
export * from './expensepayment';
export * from './expenserecognition';
export * from './expenserecognitionsettings';
export * from './exportsloveniavodxml_body';
export * from './exportsloveniayearlynormiranireport_body';
export * from './finalizedocument';
export * from './financialcategory';
export * from './finapremise';
export * from './finasettings';
export * from './fiscalizedocument';
export * from './fursbusinesspremise';
export * from './furssettings';
export * from './image';
export * from './invoice';
export * from './item';
export * from './legacymigrationauth';
export * from './login_body';
export * from './lookupfranceeinvoicingdirectory';
export * from './me';
export * from './migrationjob';
export * from './order';
export * from './orderintegration';
export * from './payment';
export * from './paymentreminder';
export * from './postab';
export * from './postable';
export * from './pt-exemption-defaults';
export * from './ptatcudseries';
export * from './putentityemailsender';
export * from './queryentitystats';
export * from './receivedeinvoicelifecycle';
export * from './recurringinvoice';
export * from './registerfinadevice_body';
export * from './registerfurselectronicdevice_body';
export * from './registerfursmovablepremise_body';
export * from './registerfursrealestatepremise_body';
export * from './releasepostab';
export * from './renderadvanceinvoicepreview_body';
export * from './rendercreditnotepreview_body';
export * from './renderdeliverynotepreview_body';
export * from './renderestimatepreview_body';
export * from './renderinvoicepreview_body';
export * from './requestpasswordreset';
export * from './resumeshopifyinstall';
export * from './revenuerecognitionsettings';
export * from './reviewsloveniayearlynormiranireport';
export * from './searcheinvoicingcustomer';
export * from './senddocument_body';
export * from './sendemail_body';
export * from './setposoperatorpin';
export * from './signup_body';
export * from './sloveniataxprofile';
export * from './spendinglimit';
export * from './starteslogexport_body';
export * from './startpdfexport_body';
export * from './stripeappconnect_body';
export * from './stripeappentityupdatesettings_body';
export * from './stripeappmarketplaceauthorize';
export * from './switchposoperator_body';
export * from './syncshopifyorders_body';
export * from './tax';
export * from './uploadfile_body';
export * from './uploadfurscertificate_body';
export * from './userfinasettings';
export * from './userfurssettings';
export * from './voidinvoice_body';
export * from './webhook';
export * from './wlsubscriptionbillingemail';
export * from './woocommerce';

// Re-export invoice create schema as credit note create schema (same body structure)
// Placed here (after invoice is exported) to avoid circular dependency issues
export { createInvoiceSchema as createCreditNoteSchema, type CreateInvoiceSchema as CreateCreditNoteSchema } from './invoice';
