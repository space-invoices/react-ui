export default {
  // Company settings
  "Tax subject": "Assujetti à la TVA",
  "Tax ID 2": "ID fiscal 2",
  "Secondary tax identification number (optional)": "Numéro d'identification fiscale secondaire (optionnel)",
  "Company Information": "Informations sur l'entreprise",
  "Basic information about your company": "Informations de base sur votre entreprise",
  // Branding settings
  Branding: "Image de marque",
  "Primary Color": "Couleur primaire",
  Logo: "Logo",
  "Logo URL": "URL du logo",
  "Enter the URL of your company logo": "Entrez l'URL du logo de votre entreprise",
  "Upload your company logo for invoices": "Téléchargez le logo de votre entreprise pour les factures",
  "Upload Logo": "Télécharger le logo",
  "Change Logo": "Changer le logo",
  "Current logo (displayed on invoices)": "Logo actuel (affiché sur les factures)",
  "Logo preview": "Aperçu du logo",
  "Crop Your Logo": "Recadrer votre logo",
  Signature: "Signature",
  "Upload a signature image for PDFs (optional)": "Téléchargez une image de signature pour les PDF (optionnel)",
  "Upload Signature": "Télécharger la signature",
  "Change Signature": "Changer la signature",
  "Current signature (for PDF documents)": "Signature actuelle (pour les documents PDF)",
  "Signature preview": "Aperçu de la signature",
  "Crop Your Signature": "Recadrer votre signature",
  "Adjust the crop area or upload the full image": "Ajustez la zone de recadrage ou téléchargez l'image complète",
  "Upload failed. Please try again.": "Le téléchargement a échoué. Veuillez réessayer.",
  "Uploading...": "Téléchargement...",
  "Upload & Save": "Télécharger et enregistrer",
  Cancel: "Annuler",
  Localization: "Localisation",
  "Currency Code": "Code devise",
  "ISO 4217 currency code (e.g., USD, EUR, GBP)": "Code devise ISO 4217 (ex. USD, EUR, GBP)",
  Locale: "Paramètres régionaux",
  "BCP 47 language tag (e.g., en-US, de-DE, sl-SI)": "Balise de langue BCP 47 (ex. en-US, de-DE, sl-SI)",
  "Select currency": "Sélectionner la devise",
  "Select locale": "Sélectionner les paramètres régionaux",
  "Configure entity localization": "Configurer la localisation de l'entité",
  "English (US)": "Anglais (US)",
  "German (DE)": "Allemand (DE)",
  "Italian (IT)": "Italien (IT)",
  "French (FR)": "Français (FR)",
  "Spanish (ES)": "Espagnol (ES)",
  "Portuguese (Portugal)": "Portugais (Portugal)",
  Dutch: "Néerlandais",
  Polish: "Polonais",
  Croatian: "Croate",
  "Slovenian (SI)": "Slovène (SI)",
  "Email Settings": "Paramètres email",
  "Configure email settings for invoices": "Configurer les paramètres email pour les factures",
  "Email Address": "Adresse email",
  "Email address to send invoices to": "Adresse email pour l'envoi des factures",
  "Sender email": "Email d’expéditeur",
  "Billing email address": "Adresse email de facturation",
  "Recipient for white-label subscription billing emails.":
    "Destinataire des emails de facturation de l’abonnement white-label.",
  "Used as the From address for document emails and payment reminders after verification passes.":
    "Utilisée comme adresse From des emails de documents et des rappels de paiement une fois la validation réussie.",
  "DNS DKIM tokens": "Jetons DNS DKIM",
  "Add CNAME records for these tokens at your sender domain, then recheck.":
    "Ajoutez des enregistrements CNAME pour ces jetons sur votre domaine d’expéditeur, puis revérifiez.",
  "DNS records are being prepared. Recheck in a moment if they do not appear.":
    "Les enregistrements DNS sont en cours de préparation. Revérifiez dans un instant s’ils n’apparaissent pas.",
  "CNAME record": "Enregistrement CNAME",
  Name: "Nom",
  Value: "Valeur",
  Copy: "Copier",
  Copied: "Copié",
  Saving: "Enregistrement",
  "Save sender": "Enregistrer l’expéditeur",
  "Update sender": "Mettre à jour l’expéditeur",
  Clear: "Effacer",
  Checking: "Vérification",
  Recheck: "Revérifier",
  Pending: "En attente",
  Verified: "Vérifié",
  Failed: "Échec",
  Email: "Email",
  Domain: "Domaine",
  "Inherited from white-label": "Hérité du white-label",
  "Entity Email": "E-mail de l'entité",
  "Invalid email address": "Adresse e-mail invalide",
  "Email shown for this entity on documents and template variables":
    "E-mail affiché pour cette entité dans les documents et variables de modèle",
  "Email Subject": "Objet de l'email",
  "Subject line for email invoices": "Ligne d'objet pour les factures par email",
  "Email Body": "Corps de l'email",
  "Body content for email invoices": "Contenu du texte pour les factures par email",
  "Subject line for invoice emails": "Objet pour les emails de factures",
  "Subject line for estimate emails": "Objet pour les emails de devis",
  "Subject line for credit note emails": "Objet pour les emails des avoirs",
  "Subject line for advance invoice emails": "Objet pour les emails des factures d'acompte",
  "Subject line for delivery note emails": "Objet pour les emails des bons de livraison",
  "Body content for invoice emails": "Contenu du texte pour les emails de factures",
  "Body content for estimate emails": "Contenu du texte pour les emails de devis",
  "Body content for credit note emails": "Contenu du texte pour les emails des avoirs",
  "Body content for advance invoice emails": "Contenu du texte pour les emails des factures d'acompte",
  "Body content for delivery note emails": "Contenu du texte pour les emails des bons de livraison",
  Reminder: "Rappel",
  "Payment reminders": "Rappels de paiement",
  "Reminder subject": "Objet du rappel",
  "Subject line for payment reminder emails": "Objet des e-mails de rappel de paiement",
  "Reminder body": "Corps du rappel",
  "Body content for payment reminder emails": "Contenu des e-mails de rappel de paiement",
  "Payment instructions": "Instructions de paiement",
  "Payment instructions appended to reminder emails when not included in the body":
    "Instructions de paiement ajoutées aux rappels lorsqu'elles ne sont pas incluses dans le message",
  "Payment reminder from {entity_name}": "Rappel de paiement de {entity_name}",
  "Hello {customer_name},\n\nThe following invoices are overdue:\n{invoice_list}\n\n{payment_instructions}":
    "Bonjour {customer_name},\n\nLes factures suivantes sont en retard :\n{invoice_list}\n\n{payment_instructions}",
  "Please remit payment to IBAN {iban}.": "Veuillez effectuer le paiement sur l'IBAN {iban}.",
  "Save Settings": "Enregistrer les paramètres",
  // Document Defaults section
  "Document Defaults": "Valeurs par défaut des documents",
  "Default values for new documents": "Valeurs par défaut pour les nouveaux documents",
  "Default Note": "Note par défaut",
  "Default Invoice Note": "Note de facture par défaut",
  "This note will be pre-filled when creating new invoices":
    "Cette note sera pré-remplie lors de la création de nouvelles factures",
  "This note will be pre-filled when creating new estimates":
    "Cette note sera pré-remplie lors de la création de nouveaux devis",
  "This note will be pre-filled when creating new credit notes":
    "Cette note sera pré-remplie lors de la création de nouveaux avoirs",
  "Payment terms pre-filled when creating new invoices":
    "Conditions de paiement pré-remplies lors de la création de nouvelles factures",
  "Payment terms pre-filled when creating new estimates":
    "Conditions de paiement pré-remplies lors de la création de nouveaux devis",
  "Payment terms pre-filled when creating new credit notes":
    "Conditions de paiement pré-remplies lors de la création de nouveaux avoirs",
  "Optional note for the document.": "Note facultative pour le document.",
  "Please remit payment using the bank details shown on the document.":
    "Merci d'effectuer le paiement en utilisant les coordonnées bancaires indiquées sur le document.",
  "This estimate is valid until {document_valid_until}.": "Ce devis est valable jusqu'au {document_valid_until}.",
  "Payment due upon acceptance.": "Paiement dû à l'acceptation.",
  "Credit note for invoice {document_number}.": "Avoir pour la facture {document_number}.",
  "Credit will be applied to your account.": "Le crédit sera appliqué à votre compte.",
  "Invoice Notes": "Notes de facture",
  "Smart Template Variables": "Variables de modèle intelligentes",
  "Use variables to personalize your notes automatically":
    "Utilisez des variables pour personnaliser automatiquement vos notes",
  "Set a default note that will appear on all new invoices. Use template variables to personalize the note automatically.":
    "Définissez une note par défaut qui apparaîtra sur toutes les nouvelles factures. Utilisez les variables de modèle pour personnaliser automatiquement la note.",
  // Smart code insert button
  "Insert variable": "Insérer une variable",
  Bold: "Gras",
  Italic: "Italique",
  "Bullet list": "Liste à puces",
  Entity: "Entreprise",
  Document: "Document",
  Customer: "Client",
  Other: "Autre",
  "Company name": "Nom de l'entreprise",
  "Email address": "Adresse email",
  "Invoice number": "Numéro de facture",
  "Invoice date": "Date de facture",
  "Due date": "Date d'échéance",
  "Total amount": "Montant total",
  Currency: "Devise",
  "Customer name": "Nom du client",
  "Customer email": "Email du client",
  "Today's date": "Date du jour",
  "Current year": "Année en cours",
  // Payment terms defaults
  "Default Payment Terms": "Conditions de paiement par défaut",
  "This payment terms will be pre-filled when creating new documents":
    "Ces conditions de paiement seront pré-remplies lors de la création de nouveaux documents",
  "Payment Terms": "Conditions de paiement",
  "Add payment terms...": "Ajouter des conditions de paiement...",
  // Document footer
  "Document Footer": "Pied de page du document",
  "Footer text displayed at the bottom of PDF documents": "Texte de pied de page affiché en bas des documents PDF",
  "Footer text displayed at the bottom of all PDF documents":
    "Texte de pied de page affiché en bas de tous les documents PDF",
  "{entity_name} | Due Date: {document_due_date} | Invoice #{document_number}":
    "{entity_name} | Échéance : {document_due_date} | Facture #{document_number}",
  // Document signature
  "Document Signature": "Signature du document",
  "Signature text displayed on all PDF documents": "Texte de signature affiché sur tous les documents PDF",
  "Add signature text...": "Ajouter un texte de signature...",
  "Delivery Note": "Bon de livraison",
  "Default note for advance invoices": "Note par défaut pour les factures d'acompte",
  "Default note for all new advance invoices": "Note par défaut pour toutes les nouvelles factures d'acompte",
  "Default note for delivery notes": "Note par défaut pour les bons de livraison",
  "Default note for all new delivery notes": "Note par défaut pour tous les nouveaux bons de livraison",
  // Overdue Notifications section
  "Overdue Notifications": "Notifications de retard",
  "Automatically remind customers about overdue invoices":
    "Rappeler automatiquement aux clients les factures en retard",
  "Enable Notifications": "Activer les notifications",
  "Send automatic payment reminder emails to customers with overdue invoices":
    "Envoyer des emails de rappel de paiement automatiques aux clients avec des factures en retard",
  "Reminder Days": "Jours de rappel",
  "Days after due date to send reminders (comma-separated). E.g., '7, 14, 30' sends reminders at 7, 14, and 30 days overdue.":
    "Jours après la date d'échéance pour envoyer des rappels (séparés par des virgules). Ex. '7, 14, 30' envoie des rappels à 7, 14 et 30 jours de retard.",
  "Custom Reminder Templates": "Modèles de rappel personnalisés",
  "Subject line for overdue notification emails": "Ligne d'objet pour les emails de notification de retard",
  "Body content for overdue emails. Use {invoice_list} to include the invoice table, or it will be auto-appended.":
    "Contenu du texte pour les emails de retard. Utilisez {invoice_list} pour inclure le tableau des factures, sinon il sera ajouté automatiquement.",
  "Dear {customer_name},\n\nThis is a reminder about your overdue invoices...":
    "Cher/Chère {customer_name},\n\nCeci est un rappel concernant vos factures en retard...",
  "Payment Reminders": "Rappels de paiement",
  "When enabled, customers will automatically receive email reminders when their invoices become overdue. Emails are sent on the exact day each threshold is reached.":
    "Lorsqu'activé, les clients recevront automatiquement des rappels par email lorsque leurs factures seront en retard. Les emails sont envoyés le jour exact où chaque seuil est atteint.",
  "Available Variables": "Variables disponibles",
  "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}":
    "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}",
  // UPN QR Payment section
  "UPN QR Payment": "Paiement UPN QR",
  "Configure UPN QR payment slip for invoices": "Configurer le bordereau de paiement UPN QR pour les factures",
  IBAN: "IBAN",
  "Bank account IBAN for receiving payments": "IBAN du compte bancaire pour recevoir les paiements",
  "Enable UPN QR on invoices": "Activer UPN QR sur les factures",
  "Show payment QR code on PDF invoices for easy mobile banking payments":
    "Afficher le code QR de paiement sur les factures PDF pour des paiements bancaires mobiles faciles",
  "Display Mode": "Mode d'affichage",
  "QR code only": "Code QR uniquement",
  "Shows compact QR code inline with invoice content":
    "Affiche un code QR compact en ligne avec le contenu de la facture",
  "Full UPN payment slip": "Bordereau de paiement UPN complet",
  "Shows complete payment slip at bottom of page": "Affiche le bordereau de paiement complet en bas de page",
  "Purpose Code": "Code objet",
  "OTHR - Other": "OTHR - Autre",
  "GDSV - Goods and Services": "GDSV - Biens et services",
  "SUPP - Supplier Payment": "SUPP - Paiement fournisseur",
  "Payment purpose code (ISO 20022)": "Code objet du paiement (ISO 20022)",
  "UPN QR Payments": "Paiements UPN QR",
  "UPN QR is a Slovenian standard for payment slips. When enabled, your invoices will include a QR code that customers can scan with their mobile banking app to pay instantly.":
    "UPN QR est un standard slovène pour les bordereaux de paiement. Lorsqu'activé, vos factures incluront un code QR que les clients peuvent scanner avec leur application bancaire mobile pour payer instantanément.",
  // Number format settings
  Invoice: "Facture",
  Estimate: "Devis",
  "Credit Note": "Avoir",
  "Advance Invoice": "Facture d'acompte",
  "Number Format": "Format de numérotation",
  Preview: "Aperçu",
  // Tax rules settings
  "Main documents": "Documents principaux",
  "Used when no business unit is selected.": "Utilisé lorsqu’aucune unité commerciale n’est sélectionnée.",
  "Business-unit documents": "Documents de l’unité commerciale",
  "Used when a business unit is selected. Default formats should include {u} to stay distinct.":
    "Utilisé lorsqu’une unité commerciale est sélectionnée. Les formats par défaut doivent inclure {u} pour rester distincts.",
  "If this format matches the main document format, saving is blocked.":
    "Si ce format correspond au format principal, l’enregistrement est bloqué.",
  "Business-unit numbering must differ from the main document format.":
    "La numérotation de l’unité commerciale doit différer du format principal.",
  "First number to use": "Premier numéro à utiliser",
  "First number to use for this unit": "Premier numéro à utiliser pour cette unité",
  "first number to use": "premier numéro à utiliser",
  "Only affects this business unit for this document type. Existing documents are not renumbered.":
    "S’applique uniquement à cette unité commerciale pour ce type de document. Les documents existants ne sont pas renumérotés.",
  "Set the first number for this unit by document type. The unit number format is shared in Numbering settings.":
    "Définissez le premier numéro de cette unité par type de document. Le format de numérotation des unités est partagé dans les paramètres de numérotation.",
  "Edit shared unit format": "Modifier le format partagé des unités",
  "first number": "premier numéro",
  "Unit numbering": "Numérotation de l’unité",
  Numbering: "Numérotation",
  "Only affects this business unit for the selected document type. Existing documents are not renumbered.":
    "S’applique uniquement à cette unité commerciale pour le type de document sélectionné. Les documents existants ne sont pas renumérotés.",
  "Can only move future numbering forward. Existing documents are not renumbered.":
    "Permet uniquement d’avancer la numérotation future. Les documents existants ne sont pas renumérotés.",
  "Insert token": "Insérer un jeton",
  "Number tokens": "Jetons de numérotation",
  "Full year": "Année complète",
  "Short year": "Année courte",
  "Sequence number": "Numéro de séquence",
  "Business unit index": "Index de l’unité commerciale",
  "EU Tax Rules": "Règles fiscales UE",
  "Automatic tax handling for cross-border transactions":
    "Gestion automatique des taxes pour les transactions transfrontalières",
  "Manual taxes": "Taxes manuelles",
  "Create tax rates on the Taxes page and apply them to document lines.":
    "Créez des taux de taxe sur la page Taxes et appliquez-les aux lignes de document.",
  "tax-rules.vies_validate_vat.label": "Validation TVA VIES",
  "tax-rules.vies_validate_vat.description":
    "Vérifier les numéros de TVA UE via l'API VIES. Les numéros de TVA des clients sont vérifiés avant la création des factures.",
  "tax-rules.auto_reverse_charge.label": "Autoliquidation automatique",
  "tax-rules.auto_reverse_charge.description":
    "Appliquer automatiquement le taux 0% (autoliquidation) pour les transactions B2B intra-UE avec des numéros de TVA valides.",
  "tax-rules.auto_remove_tax_export.label": "Suppression automatique de la taxe à l'exportation",
  "tax-rules.auto_remove_tax_export.description":
    "Supprimer automatiquement toutes les taxes pour les exportations vers des pays hors UE.",
  "tax-rules.require_gross_prices.label": "Exiger les prix TTC",
  "tax-rules.require_gross_prices.description":
    "Exiger les prix TTC lorsque l'autoliquidation est appliquée. Empêche les variations inattendues des montants totaux des factures.",
  "tax-rules.calculation_mode.label": "Mode de calcul",
  "tax-rules.calculation_mode.description":
    "Choisissez l'ordre d'application des remises et de la TVA lors du calcul des totaux du document.",
  "tax-rules.calculation_mode.b2b_standard.label": "Standard B2B",
  "tax-rules.calculation_mode.b2b_standard.description":
    "Les remises réduisent d'abord le montant net de la ligne, puis la TVA est calculée.",
  "tax-rules.calculation_mode.b2c_gross_discount.label": "Remise TTC B2C",
  "tax-rules.calculation_mode.b2c_gross_discount.description":
    "Les remises réduisent le montant TTC final à payer, puis le net et la TVA sont calculés à partir de ce résultat.",
  // Tax clauses settings
  "Default Tax Clauses": "Clauses fiscales par défaut",
  "Set default tax clauses that are automatically added to documents based on transaction type":
    "Définissez les clauses fiscales par défaut qui sont automatiquement ajoutées aux documents en fonction du type de transaction.",
  "tax-clauses.other-title": "Clauses fiscales transfrontalières",
  "tax-clauses.other-description":
    "Clauses fiscales pour les transactions UE et internationales (autoliquidation, exportation).",
  "tax-clauses.intra_eu_b2b.label": "Intra-UE B2B (autoliquidation)",
  "tax-clauses.intra_eu_b2b.description":
    "Pour les ventes à des entreprises UE avec des numéros de TVA valides. Inclut généralement une déclaration d'autoliquidation.",
  "Enter reverse charge clause...": "Entrez la clause d'autoliquidation...",
  "tax-clauses.3w_b2b.label": "Hors UE B2B (exportation)",
  "tax-clauses.3w_b2b.description":
    "Pour les ventes à des entreprises hors UE. Indique généralement l'exonération de TVA à l'exportation.",
  "tax-clauses.3w_b2c.label": "Hors UE B2C (exportation)",
  "tax-clauses.3w_b2c.description":
    "Pour les ventes à des consommateurs hors UE. Indique généralement l'exonération de TVA à l'exportation.",
  "Enter export exemption clause...": "Entrez la clause d'exonération à l'exportation...",
  "tax-clauses.domestic.label": "Par défaut / National",
  "tax-clauses.domestic.description":
    "Clause par défaut pour les transactions nationales. Pour les entités non assujetties à la TVA, elle apparaît sur tous les documents.",
  "Enter default tax clause...": "Entrez la clause fiscale par défaut...",
  "tax-clauses.intra_eu_b2c.label": "Intra-UE B2C",
  "Enter EU consumer sales clause...": "Entrez la clause de vente aux consommateurs UE...",
  "tax-clauses.other.description":
    "Ces types de transactions ne nécessitent généralement pas de clauses fiscales spécifiques, mais vous pouvez les configurer selon vos besoins.",
  // EPC QR Payment section
  "EPC QR Payment": "Paiement EPC QR",
  "SEPA credit transfer QR code for invoices": "Code QR de virement SEPA pour les factures",
  "Enable EPC QR on invoices": "Activer EPC QR sur les factures",
  "Show SEPA QR code on EUR invoices for easy bank payments":
    "Afficher le code QR SEPA sur les factures en EUR pour des paiements bancaires faciles",
  "EPC QR Payments": "Paiements EPC QR",
  "EPC QR is a European standard for SEPA credit transfers. When enabled, EUR invoices include a QR code that customers can scan with their banking app to pay instantly.":
    "EPC QR est un standard européen pour les virements SEPA. Lorsqu'il est activé, les factures en EUR incluent un code QR que les clients peuvent scanner avec leur application bancaire pour payer instantanément.",
  // Document date defaults
  "Default due days": "Jours d’échéance par défaut",
  "Default valid days": "Jours de validité par défaut",
  "Number of days added to the document date for new invoice due dates":
    "Nombre de jours ajoutés à la date du document pour les nouvelles échéances de facture.",
  "Number of days added to the document date for new estimate valid-till dates":
    "Nombre de jours ajoutés à la date du document pour les nouvelles dates de validité des devis.",

  // Portugal entity details
  "Portugal Details": "Détails Portugal",
  "Portuguese invoices carry these details. What is required depends on your legal form.":
    "Les factures portugaises comportent ces informations. Celles qui sont obligatoires dépendent de votre forme juridique.",
  "Legal Form": "Forme juridique",
  "Select your legal form": "Sélectionnez votre forme juridique",
  "Decides which registration and capital details Portugal asks you for":
    "Détermine les informations d'immatriculation et de capital que le Portugal vous demande",
  "Individual professional or sole trader": "Professionnel indépendant ou entrepreneur individuel",
  "Private limited company (Lda.)": "Société à responsabilité limitée (Lda.)",
  "Public limited company (S.A.)": "Société anonyme (S.A.)",
  "Partnership limited by shares": "Société en commandite par actions",
  "Other company": "Autre société",
  "Company Number": "Numéro d'entreprise",
  "Digits and slashes only, as shown on your registration.":
    "Chiffres et barres obliques uniquement, tels qu'ils figurent sur votre immatriculation.",
  "Registry Office": "Greffe du registre du commerce",
  "Commercial registry where the company is registered": "Registre du commerce où la société est immatriculée",
  Phone: "Téléphone",
  "Enter 912 345 678 for Portugal, or include + and the country code for another country.":
    "Saisissez 912 345 678 pour le Portugal, ou indiquez + et l'indicatif du pays pour un autre pays.",
  "Enter 7 digits; the hyphen is added automatically.": "Saisissez 7 chiffres ; le tiret est ajouté automatiquement.",
  "9 digits. You can paste it with spaces or a PT prefix.":
    "9 chiffres. Vous pouvez le coller avec des espaces ou le préfixe PT.",
  "Share Capital": "Capital social",
  "Registered share capital of the company": "Capital social enregistré de la société",
  "Conditional company disclosures": "Mentions conditionnelles sur la société",
  "Leave these empty unless one of them applies to your company.":
    "Laissez ces champs vides, sauf si l'un d'eux s'applique à votre société.",
  "Paid-up Capital": "Capital libéré",
  "Only needed when less than the share capital has actually been paid in":
    "Nécessaire uniquement si le capital effectivement libéré est inférieur au capital social",
  Equity: "Capitaux propres",
  "Only needed when equity has fallen to half the share capital or less. Use the figure from your last approved balance sheet.":
    "Nécessaire uniquement si les capitaux propres sont tombés à la moitié du capital social ou moins. Utilisez le montant de votre dernier bilan approuvé.",
  "Company is in liquidation": "Société en liquidation",
  "Documents must say so while the company is being wound up":
    "Les documents doivent le mentionner pendant toute la liquidation de la société",
  // Tax rule sections and Slovenian Article 76.a
  "Calculation settings": "Paramètres de calcul",
  "Choose how prices, discounts, and taxes are calculated on documents.":
    "Choisissez comment les prix, les remises et les taxes sont calculés sur les documents.",
  "EU cross border rules": "Règles de l’UE pour les opérations transfrontalières",
  "Validate VAT numbers and automate reverse charge for eligible EU transactions.":
    "Validez les numéros de TVA et automatisez l’autoliquidation pour les opérations européennes éligibles.",
  "Export rules": "Règles d’exportation",
  "Configure automatic tax handling for transactions outside the EU.":
    "Configurez le traitement fiscal automatique des opérations hors de l’UE.",
  "Configure domestic Slovenian tax treatments.": "Configurez les traitements fiscaux nationaux slovènes.",
  "Slovenian Article 76.a": "Article 76.a slovène",
  "Allow eligible domestic business invoices to report VAT under reverse charge.":
    "Autorisez les factures nationales B2B éligibles à déclarer la TVA en autoliquidation.",
  "Article 76.a tax clause": "Clause fiscale de l’article 76.a",
  "Default legal clause added when Article 76.a is selected on an invoice.":
    "Clause juridique par défaut ajoutée lorsque l’article 76.a est sélectionné sur une facture.",
  "Obrnjena davčna obveznost po 76.a členu ZDDV-1.": "Obrnjena davčna obveznost po 76.a členu ZDDV-1.",
} as const;
