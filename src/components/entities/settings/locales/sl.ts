export default {
  // Company settings
  "Tax subject": "Davčni zavezanec",
  "Tax ID 2": "Davčna številka 2",
  "Secondary tax identification number (optional)": "Sekundarna davčna identifikacijska številka (neobvezno)",
  "Company Information": "Podatki o podjetju",
  "Basic information about your company": "Osnovni podatki o vašem podjetju",
  // Branding settings
  Branding: "Blagovna znamka",
  "Primary Color": "Primarna barva",
  Logo: "Logotip",
  "Logo URL": "URL logotipa",
  "Enter the URL of your company logo": "Vnesite URL logotipa vašega podjetja",
  "Upload your company logo for invoices": "Naložite logotip vašega podjetja za račune",
  "Upload Logo": "Naloži logotip",
  "Change Logo": "Spremeni logotip",
  "Current logo (displayed on invoices)": "Trenutni logotip (prikazan na računih)",
  "Logo preview": "Predogled logotipa",
  "Crop Your Logo": "Obreži logotip",
  Signature: "Podpis",
  "Upload a signature image for PDFs (optional)": "Naložite sliko podpisa za PDF-je (neobvezno)",
  "Upload Signature": "Naloži podpis",
  "Change Signature": "Spremeni podpis",
  "Current signature (for PDF documents)": "Trenutni podpis (za PDF dokumente)",
  "Signature preview": "Predogled podpisa",
  "Crop Your Signature": "Obreži podpis",
  "Adjust the crop area or upload the full image": "Prilagodite območje obrezovanja ali naložite celotno sliko",
  "Upload failed. Please try again.": "Nalaganje ni uspelo. Poskusite znova.",
  "Uploading...": "Nalaganje...",
  "Upload & Save": "Naloži in shrani",
  Cancel: "Prekliči",
  Localization: "Lokalizacija",
  "Currency Code": "Koda valute",
  "ISO 4217 currency code (e.g., USD, EUR, GBP)": "ISO 4217 koda valute (npr. USD, EUR, GBP)",
  Locale: "Lokalnost",
  "BCP 47 language tag (e.g., en-US, de-DE, sl-SI)": "BCP 47 jezikovna oznaka (npr. en-US, de-DE, sl-SI)",
  "Select currency": "Izberi valuto",
  "Select locale": "Izberi lokalnost",
  "Configure entity localization": "Konfigurirajte lokalizacijo pravne osebe",
  "English (US)": "Angleščina (US)",
  "German (DE)": "Nemščina (DE)",
  "Italian (IT)": "Italijanščina (IT)",
  "French (FR)": "Francoščina (FR)",
  "Spanish (ES)": "Španščina (ES)",
  "Portuguese (Portugal)": "Portugalščina (Portugalska)",
  Dutch: "Nizozemščina",
  Polish: "Poljščina",
  Croatian: "Hrvaščina",
  "Slovenian (SI)": "Slovenščina (SI)",
  "Email Settings": "Nastavitve e-pošte",
  "Configure email settings for invoices": "Konfigurirajte nastavitve e-pošte za račune",
  "Email Address": "Naslov e-pošte",
  "Email address to send invoices to": "Naslov e-pošte za pošiljanje računov",
  "Sender email": "E-pošta pošiljatelja",
  "Billing email address": "E-poštni naslov za obračun",
  "Recipient for white-label subscription billing emails.": "Prejemnik e-pošte za obračun white-label naročnine.",
  "Used as the From address for document emails and payment reminders after verification passes.":
    "Po uspešnem preverjanju se uporablja kot naslov From za e-pošto z dokumenti in opomnike za plačilo.",
  "DNS DKIM tokens": "DNS DKIM žetoni",
  "Add CNAME records for these tokens at your sender domain, then recheck.":
    "Dodajte CNAME zapise za te žetone na domeni pošiljatelja in nato ponovno preverite.",
  "DNS records are being prepared. Recheck in a moment if they do not appear.":
    "Zapisi DNS se pripravljajo. Če se ne prikažejo, čez trenutek preverite znova.",
  "CNAME record": "Zapis CNAME",
  Name: "Ime",
  Value: "Vrednost",
  Copy: "Kopiraj",
  Copied: "Kopirano",
  Saving: "Shranjevanje",
  "Save sender": "Shrani pošiljatelja",
  "Update sender": "Posodobi pošiljatelja",
  Clear: "Počisti",
  Checking: "Preverjanje",
  Recheck: "Ponovno preveri",
  Pending: "V teku",
  Verified: "Preverjeno",
  Failed: "Neuspešno",
  Email: "E-pošta",
  Domain: "Domena",
  "Inherited from white-label": "Podedovano od white-label",
  "Entity Email": "E-pošta pravne osebe",
  "Invalid email address": "Neveljaven e-poštni naslov",
  "Email shown for this entity on documents and template variables":
    "E-pošta, prikazana za to pravno osebo na dokumentih in v spremenljivkah predloge",
  "Email Subject": "Zadeva e-pošte",
  "Subject line for email invoices": "Vrstica zadeve za e-pošte računov",
  "Email Body": "Vsebina e-pošte",
  "Body content for email invoices": "Vsebina teksta za e-pošte računov",
  "Subject line for invoice emails": "Vrstica zadeve za e-pošte računov",
  "Subject line for estimate emails": "Vrstica zadeve za e-pošte ponudb",
  "Subject line for credit note emails": "Vrstica zadeve za e-pošte dobropisov",
  "Subject line for advance invoice emails": "Vrstica zadeve za e-pošte avansnih računov",
  "Subject line for delivery note emails": "Vrstica zadeve za e-pošte dobavnic",
  "Body content for invoice emails": "Vsebina teksta za e-pošte računov",
  "Body content for estimate emails": "Vsebina teksta za e-pošte ponudb",
  "Body content for credit note emails": "Vsebina teksta za e-pošte dobropisov",
  "Body content for advance invoice emails": "Vsebina teksta za e-pošte avansnih računov",
  "Body content for delivery note emails": "Vsebina teksta za e-pošte dobavnic",
  Reminder: "Opomin",
  "Payment reminders": "Opomini za plačilo",
  "Reminder subject": "Zadeva opomina",
  "Subject line for payment reminder emails": "Zadeva e-poštnih opominov za plačilo",
  "Reminder body": "Besedilo opomina",
  "Body content for payment reminder emails": "Vsebina e-poštnih opominov za plačilo",
  "Payment instructions": "Navodila za plačilo",
  "Payment instructions appended to reminder emails when not included in the body":
    "Navodila za plačilo, dodana opominom, kadar niso vključena v besedilo",
  "Payment reminder from {entity_name}": "Opomin za plačilo od {entity_name}",
  "Hello {customer_name},\n\nThe following invoices are overdue:\n{invoice_list}\n\n{payment_instructions}":
    "Pozdravljeni {customer_name},\n\nNaslednji računi so zapadli:\n{invoice_list}\n\n{payment_instructions}",
  "Please remit payment to IBAN {iban}.": "Plačilo nakažite na IBAN {iban}.",
  "Save Settings": "Shrani nastavitve",
  // Document Defaults section
  "Document Defaults": "Privzete vrednosti dokumentov",
  "Default values for new documents": "Privzete vrednosti za nove dokumente",
  "Default Note": "Privzeti pripis",
  "Default Invoice Note": "Privzeti pripis računa",
  "This note will be pre-filled when creating new invoices":
    "Ta pripis bo vnaprej izpolnjen pri ustvarjanju novih računov",
  "This note will be pre-filled when creating new estimates":
    "Ta pripis bo vnaprej izpolnjen pri ustvarjanju novih ponudb",
  "This note will be pre-filled when creating new credit notes":
    "Ta pripis bo vnaprej izpolnjen pri ustvarjanju novih dobropisov",
  "Payment terms pre-filled when creating new invoices":
    "Plačilni pogoji bodo vnaprej izpolnjeni pri ustvarjanju novih računov",
  "Payment terms pre-filled when creating new estimates":
    "Plačilni pogoji bodo vnaprej izpolnjeni pri ustvarjanju novih ponudb",
  "Payment terms pre-filled when creating new credit notes":
    "Plačilni pogoji bodo vnaprej izpolnjeni pri ustvarjanju novih dobropisov",
  "Optional note for the document.": "Neobvezna opomba za dokument.",
  "Please remit payment using the bank details shown on the document.":
    "Plačilo izvedite po bančnih podatkih, navedenih na dokumentu.",
  "This estimate is valid until {document_valid_until}.": "Ta ponudba velja do {document_valid_until}.",
  "Payment due upon acceptance.": "Plačilo ob sprejemu.",
  "Credit note for invoice {document_number}.": "Dobropis za račun {document_number}.",
  "Credit will be applied to your account.": "Dobropis bo upoštevan na vašem računu.",
  "Invoice Notes": "Pripisi računa",
  "Smart Template Variables": "Pametne spremenljivke predloge",
  "Use variables to personalize your notes automatically": "Uporabite spremenljivke za samodejno prilagajanje pripisov",
  "Set a default note that will appear on all new invoices. Use template variables to personalize the note automatically.":
    "Nastavite privzeti pripis, ki bo prikazan na vseh novih računih. Uporabite spremenljivke predloge za samodejno prilagajanje pripisa.",
  // Smart code insert button
  "Insert variable": "Vstavi spremenljivko",
  Bold: "Krepko",
  Italic: "Ležeče",
  "Bullet list": "Seznam z oznakami",
  Entity: "Podjetje",
  Document: "Dokument",
  Customer: "Stranka",
  Other: "Drugo",
  "Company name": "Ime podjetja",
  "Email address": "E-poštni naslov",
  "Invoice number": "Številka računa",
  "Invoice date": "Datum računa",
  "Due date": "Rok plačila",
  "Total amount": "Skupni znesek",
  Currency: "Valuta",
  "Customer name": "Ime stranke",
  "Customer email": "E-pošta stranke",
  "Today's date": "Današnji datum",
  "Current year": "Tekoče leto",
  // Payment terms defaults
  "Default Payment Terms": "Privzeti plačilni pogoji",
  "This payment terms will be pre-filled when creating new documents":
    "Ti plačilni pogoji bodo vnaprej izpolnjeni pri ustvarjanju novih dokumentov",
  "Payment Terms": "Plačilni pogoji",
  "Add payment terms...": "Dodaj plačilne pogoje...",
  // Document footer
  "Document Footer": "Noga dokumenta",
  "Footer text displayed at the bottom of PDF documents": "Besedilo noge, prikazano na dnu PDF dokumentov",
  "Footer text displayed at the bottom of all PDF documents": "Besedilo noge, prikazano na dnu vseh PDF dokumentov",
  "{entity_name} | Due Date: {document_due_date} | Invoice #{document_number}":
    "{entity_name} | Rok plačila: {document_due_date} | Račun #{document_number}",
  // Document signature
  "Document Signature": "Podpis dokumenta",
  "Signature text displayed on all PDF documents": "Besedilo podpisa, prikazano na vseh PDF dokumentih",
  "Add signature text...": "Dodaj besedilo podpisa...",
  "Delivery Note": "Dobavnica",
  "Default note for advance invoices": "Privzeti pripis za avansne račune",
  "Default note for all new advance invoices": "Privzeti pripis za vse nove avansne račune",
  "Default note for delivery notes": "Privzeti pripis za dobavnice",
  "Default note for all new delivery notes": "Privzeti pripis za vse nove dobavnice",
  // Overdue Notifications section
  "Overdue Notifications": "Opomniki o zapadlih računih",
  "Automatically remind customers about overdue invoices": "Samodejno opominjajte stranke o zapadlih računih",
  "Enable Notifications": "Omogoči opominke",
  "Send automatic payment reminder emails to customers with overdue invoices":
    "Pošiljajte avtomatska opomniška e-poštna sporočila strankam z zapadlimi računi",
  "Reminder Days": "Dnevi opomnikov",
  "Days after due date to send reminders (comma-separated). E.g., '7, 14, 30' sends reminders at 7, 14, and 30 days overdue.":
    "Dnevi po roku plačila za pošiljanje opomnikov (ločeni z vejico). Npr. '7, 14, 30' pošlje opomnike 7, 14 in 30 dni po zapadlosti.",
  "Custom Reminder Templates": "Prilagojene predloge opomnikov",
  "Subject line for overdue notification emails": "Vrstica zadeve za e-pošte opomnikov o zapadlosti",
  "Body content for overdue emails. Use {invoice_list} to include the invoice table, or it will be auto-appended.":
    "Vsebina teksta za e-pošte o zapadlosti. Uporabite {invoice_list} za vključitev tabele računov, sicer bo samodejno dodana.",
  "Dear {customer_name},\n\nThis is a reminder about your overdue invoices...":
    "Spoštovani {customer_name},\n\nTo je opomnik o vaših zapadlih računih...",
  "Payment Reminders": "Opomniki o plačilih",
  "When enabled, customers will automatically receive email reminders when their invoices become overdue. Emails are sent on the exact day each threshold is reached.":
    "Ko je omogočeno, bodo stranke samodejno prejele e-poštne opomnike, ko njihovi računi zapadejo. E-pošta se pošlje natančno na dan, ko je dosežen vsak prag.",
  "Available Variables": "Razpoložljive spremenljivke",
  "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}":
    "{customer_name}, {entity_name}, {entity_email}, {invoice_list}, {total_amount}, {overdue_count}",
  // UPN QR Payment section
  "UPN QR Payment": "UPN QR plačilo",
  "Configure UPN QR payment slip for invoices": "Konfigurirajte UPN QR plačilni nalog za račune",
  IBAN: "IBAN",
  "Bank account IBAN for receiving payments": "IBAN bančnega računa za prejemanje plačil",
  "Enable UPN QR on invoices": "Omogoči UPN QR na računih",
  "Show payment QR code on PDF invoices for easy mobile banking payments":
    "Prikaži QR kodo za plačilo na PDF računih za enostavna mobilna bančna plačila",
  "Display Mode": "Način prikaza",
  "QR code only": "Samo QR koda",
  "Shows compact QR code inline with invoice content": "Prikaže kompaktno QR kodo v vrstici z vsebino računa",
  "Full UPN payment slip": "Celoten UPN plačilni nalog",
  "Shows complete payment slip at bottom of page": "Prikaže celoten plačilni nalog na dnu strani",
  "Purpose Code": "Koda namena",
  "OTHR - Other": "OTHR - Drugo",
  "GDSV - Goods and Services": "GDSV - Blago in storitve",
  "SUPP - Supplier Payment": "SUPP - Plačilo dobavitelju",
  "Payment purpose code (ISO 20022)": "Koda namena plačila (ISO 20022)",
  "UPN QR Payments": "UPN QR plačila",
  "UPN QR is a Slovenian standard for payment slips. When enabled, your invoices will include a QR code that customers can scan with their mobile banking app to pay instantly.":
    "UPN QR je slovenski standard za plačilne naloge. Ko je omogočen, bodo vaši računi vključevali QR kodo, ki jo stranke lahko skenirajo z mobilno bančno aplikacijo za takojšnje plačilo.",
  // Number format settings
  Invoice: "Račun",
  Estimate: "Ponudba",
  "Credit Note": "Dobropis",
  "Advance Invoice": "Avansni račun",
  "Number Format": "Format številke",
  Preview: "Predogled",
  // Tax rules settings
  "Main documents": "Glavni dokumenti",
  "Used when no business unit is selected.": "Uporabi se, ko poslovna enota ni izbrana.",
  "Business-unit documents": "Dokumenti poslovne enote",
  "Used when a business unit is selected. Default formats should include {u} to stay distinct.":
    "Uporabi se, ko je izbrana poslovna enota. Privzeti formati naj vključujejo {u}, da ostanejo ločeni.",
  "If this format matches the main document format, saving is blocked.":
    "Če se ta format ujema z glavnim formatom dokumenta, je shranjevanje blokirano.",
  "Business-unit numbering must differ from the main document format.":
    "Številčenje poslovne enote se mora razlikovati od glavnega formata dokumenta.",
  "First number to use": "Prva številka za uporabo",
  "First number to use for this unit": "Prva številka za uporabo za to enoto",
  "first number to use": "prva številka za uporabo",
  "Only affects this business unit for this document type. Existing documents are not renumbered.":
    "Vpliva samo na to poslovno enoto za to vrsto dokumenta. Obstoječi dokumenti se ne preštevilčijo.",
  "Set the first number for this unit by document type. The unit number format is shared in Numbering settings.":
    "Nastavite prvo številko za to enoto po vrsti dokumenta. Oblika številke enote je skupna v nastavitvah številčenja.",
  "Edit shared unit format": "Uredi skupno obliko enote",
  "first number": "prva številka",
  "Unit numbering": "Številčenje enote",
  Numbering: "Številčenje",
  "Only affects this business unit for the selected document type. Existing documents are not renumbered.":
    "Vpliva samo na to poslovno enoto za izbrano vrsto dokumenta. Obstoječi dokumenti se ne preštevilčijo.",
  "Can only move future numbering forward. Existing documents are not renumbered.":
    "Prihodnje številčenje lahko samo povečate. Obstoječi dokumenti se ne preštevilčijo.",
  "Insert token": "Vstavi žeton",
  "Number tokens": "Žetoni številčenja",
  "Full year": "Polno leto",
  "Short year": "Kratko leto",
  "Sequence number": "Zaporedna številka",
  "Business unit index": "Indeks poslovne enote",
  "EU Tax Rules": "EU davčna pravila",
  "Automatic tax handling for cross-border transactions": "Samodejno obravnavanje davkov za čezmejne transakcije",
  "Manual taxes": "Ročni davki",
  "Create tax rates on the Taxes page and apply them to document lines.":
    "Ustvarite davčne stopnje na strani Davki in jih uporabite na vrsticah dokumenta.",
  "tax-rules.vies_validate_vat.label": "VIES preverjanje DDV",
  "tax-rules.vies_validate_vat.description":
    "Preveri EU DDV številke preko VIES API-ja. DDV številke strank se preverijo pred ustvarjanjem računov.",
  "tax-rules.auto_reverse_charge.label": "Samodejni obrnjeni davek",
  "tax-rules.auto_reverse_charge.description":
    "Samodejno uporabi 0% davek (obrnjena davčna obveznost) za intra-EU B2B transakcije z veljavnimi DDV številkami.",
  "tax-rules.auto_remove_tax_export.label": "Samodejno odstrani izvozni davek",
  "tax-rules.auto_remove_tax_export.description": "Samodejno odstrani vse davke za izvoz v države izven EU.",
  "tax-rules.require_gross_prices.label": "Zahtevaj bruto cene",
  "tax-rules.require_gross_prices.description":
    "Zahtevaj bruto cene, ko se uporabi obrnjena davčna obveznost. Prepreči nepričakovane spremembe skupnih zneskov računov.",
  "tax-rules.calculation_mode.label": "Način izračuna",
  "tax-rules.calculation_mode.description": "Izberite vrstni red popustov in DDV pri izračunu zneskov dokumenta.",
  "tax-rules.calculation_mode.b2b_standard.label": "Standard B2B",
  "tax-rules.calculation_mode.b2b_standard.description":
    "Popusti najprej zmanjšajo neto znesek postavke, nato se obračuna DDV.",
  "tax-rules.calculation_mode.b2c_gross_discount.label": "B2C bruto popust",
  "tax-rules.calculation_mode.b2c_gross_discount.description":
    "Popusti zmanjšajo končni bruto znesek za plačilo, neto osnova in DDV pa se izračunata iz tega rezultata.",
  // Tax clauses settings
  "Default Tax Clauses": "Privzete davčne klavzule",
  "Set default tax clauses that are automatically added to documents based on transaction type":
    "Nastavite privzete davčne klavzule, ki se samodejno dodajo dokumentom glede na vrsto transakcije.",
  "tax-clauses.other-title": "Čezmejne davčne klavzule",
  "tax-clauses.other-description": "Davčne klavzule za EU in mednarodne transakcije (samoobdavčitev, izvoz).",
  "tax-clauses.intra_eu_b2b.label": "Intra-EU B2B (obrnjena davčna obveznost)",
  "tax-clauses.intra_eu_b2b.description":
    "Za prodajo EU podjetjem z veljavnimi DDV številkami. Običajno vključuje izjavo o obrnjeni davčni obveznosti.",
  "Enter reverse charge clause...": "Vnesite klavzulo za samoobdavčitev...",
  "tax-clauses.3w_b2b.label": "Izven EU B2B (izvoz)",
  "tax-clauses.3w_b2b.description": "Za prodajo podjetjem izven EU. Običajno navaja oprostitev DDV za izvoz.",
  "tax-clauses.3w_b2c.label": "Izven EU B2C (izvoz)",
  "tax-clauses.3w_b2c.description": "Za prodajo potrošnikom izven EU. Običajno navaja oprostitev DDV za izvoz.",
  "Enter export exemption clause...": "Vnesite klavzulo za izvozno oprostitev...",
  "tax-clauses.domestic.label": "Privzeto / Domače",
  "tax-clauses.domestic.description":
    "Privzeta klavzula za domače transakcije. Za subjekte, ki niso zavezanci za DDV, se prikaže na vseh dokumentih.",
  "Enter default tax clause...": "Vnesite privzeto davčno klavzulo...",
  "tax-clauses.intra_eu_b2c.label": "Intra-EU B2C",
  "Enter EU consumer sales clause...": "Vnesite klavzulo za EU potrošniško prodajo...",
  "tax-clauses.other.description":
    "Te vrste transakcij običajno ne zahtevajo posebnih davčnih klavzul, vendar jih lahko konfigurirate po potrebi.",
  // EPC QR Payment section
  "EPC QR Payment": "EPC QR plačilo",
  "SEPA credit transfer QR code for invoices": "SEPA QR koda za kreditne prenose na računih",
  "Enable EPC QR on invoices": "Omogoči EPC QR na računih",
  "Show SEPA QR code on EUR invoices for easy bank payments":
    "Prikaži SEPA QR kodo na EUR računih za enostavna bančna plačila",
  "EPC QR Payments": "EPC QR plačila",
  "EPC QR is a European standard for SEPA credit transfers. When enabled, EUR invoices include a QR code that customers can scan with their banking app to pay instantly.":
    "EPC QR je evropski standard za SEPA kreditne prenose. Ko je omogočen, EUR računi vključujejo QR kodo, ki jo stranke lahko skenirajo z bančno aplikacijo za takojšnje plačilo.",
  // Document date defaults
  "Default due days": "Privzeti dnevi zapadlosti",
  "Default valid days": "Privzeti dnevi veljavnosti",
  "Number of days added to the document date for new invoice due dates":
    "Število dni, dodanih datumu dokumenta za nove datume zapadlosti računov.",
  "Number of days added to the document date for new estimate valid-till dates":
    "Število dni, dodanih datumu dokumenta za nove datume veljavnosti ponudb.",

  // Portugal entity details
  "Portugal Details": "Podatki za Portugalsko",
  "Portuguese invoices carry these details. What is required depends on your legal form.":
    "Portugalski računi vsebujejo te podatke. Kateri so obvezni, je odvisno od vaše pravne oblike.",
  "Legal Form": "Pravna oblika",
  "Select your legal form": "Izberite pravno obliko",
  "Decides which registration and capital details Portugal asks you for":
    "Določa, katere podatke o registraciji in kapitalu Portugalska zahteva od vas",
  "Individual professional or sole trader": "Samostojni strokovnjak ali samostojni podjetnik",
  "Private limited company (Lda.)": "Družba z omejeno odgovornostjo (Lda.)",
  "Public limited company (S.A.)": "Delniška družba (S.A.)",
  "Partnership limited by shares": "Komanditna delniška družba",
  "Other company": "Druga družba",
  "Company Number": "Matična številka",
  "Digits and slashes only, as shown on your registration.":
    "Samo števke in poševnice, kot je navedeno v vašem registrskem vpisu.",
  "Registry Office": "Registrski urad",
  "Commercial registry where the company is registered": "Poslovni register, v katerem je družba vpisana",
  Phone: "Telefon",
  "Enter 912 345 678 for Portugal, or include + and the country code for another country.":
    "Za Portugalsko vnesite 912 345 678, za drugo državo pa dodajte + in klicno številko države.",
  "Enter 7 digits; the hyphen is added automatically.": "Vnesite 7 števk; vezaj se doda samodejno.",
  "9 digits. You can paste it with spaces or a PT prefix.": "9 števk. Vnesete ga lahko s presledki ali s predpono PT.",
  "Share Capital": "Osnovni kapital",
  "Registered share capital of the company": "Registrirani osnovni kapital podjetja",
  "Conditional company disclosures": "Pogojna razkritja o družbi",
  "Leave these empty unless one of them applies to your company.":
    "Ta polja pustite prazna, razen če se katero od njih nanaša na vašo družbo.",
  "Paid-up Capital": "Vplačani kapital",
  "Only needed when less than the share capital has actually been paid in":
    "Potrebno le, če je bilo dejansko vplačanega manj, kot znaša osnovni kapital",
  Equity: "Lastniški kapital",
  "Only needed when equity has fallen to half the share capital or less. Use the figure from your last approved balance sheet.":
    "Potrebno le, če je lastniški kapital padel na polovico osnovnega kapitala ali manj. Uporabite znesek iz zadnje potrjene bilance stanja.",
  "Company is in liquidation": "Družba je v likvidaciji",
  "Documents must say so while the company is being wound up":
    "Med likvidacijo družbe mora biti to navedeno na dokumentih",
  // Tax rule sections and Slovenian Article 76.a
  "Calculation settings": "Nastavitve izračuna",
  "Choose how prices, discounts, and taxes are calculated on documents.":
    "Izberite, kako se na dokumentih izračunajo cene, popusti in davki.",
  "EU cross border rules": "Pravila za čezmejne transakcije v EU",
  "Validate VAT numbers and automate reverse charge for eligible EU transactions.":
    "Preverite identifikacijske številke za DDV in avtomatizirajte obrnjeno davčno obveznost za upravičene transakcije v EU.",
  "Export rules": "Izvozna pravila",
  "Configure automatic tax handling for transactions outside the EU.":
    "Nastavite samodejno davčno obravnavo za transakcije zunaj EU.",
  "Configure domestic Slovenian tax treatments.": "Nastavite domače slovenske davčne obravnave.",
  "Slovenian Article 76.a": "Slovenski 76.a člen",
  "Allow eligible domestic business invoices to report VAT under reverse charge.":
    "Omogočite, da upravičeni domači poslovni računi izkažejo DDV po sistemu obrnjene davčne obveznosti.",
  "Article 76.a tax clause": "Davčna klavzula po 76.a členu",
  "Default legal clause added when Article 76.a is selected on an invoice.":
    "Privzeta zakonska klavzula, dodana, ko je na računu izbran 76.a člen.",
  "Obrnjena davčna obveznost po 76.a členu ZDDV-1.": "Obrnjena davčna obveznost po 76.a členu ZDDV-1.",
} as const;
