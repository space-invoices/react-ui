export default {
  Peppol: "Peppol",
  "Click to skip Peppol sending for this invoice": "Fai clic per saltare l'invio di Peppol per questa fattura",
  "Click to enable Peppol sending": "Fare clic per abilitare l'invio Peppol",
  "Peppol Scheme": "Schema Peppol",
  "Peppol ID": "ID Peppol",
  Scheme: "Schema",
  Recipient: "Destinatario",
  Name: "Nome",
  Details: "Dettagli",
  Number: "Numero",
  "Number format can be changed in settings": "Il formato del numero può essere modificato nelle impostazioni",
  Date: "Data",
  "Pick a date": "Seleziona una data",
  Reference: "Riferimento",
  "e.g., PO-2024-001": "ad es. PO-2024-001",
  "Due Date": "Data di scadenza",
  "Valid Until": "Valido fino al",
  Items: "Voci",
  "Add item": "Aggiungi voce",
  Save: "Salva",
  Update: "Aggiorna",
  "Save as Draft": "Salva come bozza",
  Quantity: "Quantità",
  Price: "Prezzo",
  Unit: "Unità",
  "E-invoicing unit code": "Codice unità per e-fatturazione",
  "E-invoicing unit settings": "Impostazioni unità per l'e-fatturazione",
  "Only affects e-invoicing XML. The document unit stays unchanged.":
    "Influisce solo sull'XML di e-fatturazione. L'unità del documento resta invariata.",
  "Select XML unit code": "Seleziona codice unità XML",
  "Enter a supported UN/ECE code": "Inserisci un codice UN/ECE supportato",
  "Use display unit": "Usa unità visualizzata",
  "Clear unit code": "Cancella codice unità",
  "custom code": "codice personalizzato",
  Discount: "Sconto",
  Tax: "Imposta",
  "Tax rate": "Aliquota fiscale",
  "Add tax": "Aggiungi imposta",
  "Select tax...": "Seleziona imposta...",
  "Loading...": "Caricamento...",
  "No taxes found": "Nessuna imposta trovata",
  "Add new tax...": "Aggiungi nuova imposta...",
  Description: "Descrizione",
  "Mark as Paid": "Segna come pagato",
  Paid: "Pagato",
  "Invoice will be marked as fully paid upon creation":
    "La fattura sarà contrassegnata come interamente pagata alla creazione",
  "Advance payments are applied automatically. Record any remaining payment after creating the invoice.":
    "Gli acconti vengono applicati automaticamente. Registra l'eventuale pagamento residuo dopo aver creato la fattura.",
  "Payment Type": "Tipo di pagamento",
  "Select payment type": "Seleziona il tipo di pagamento",
  Cash: "Contanti",
  "Bank Transfer": "Bonifico bancario",
  Card: "Carta",
  Check: "Assegno",
  Other: "Altro",
  Note: "Nota",
  "Insert variable": "Inserisci variabile",
  Bold: "Grassetto",
  Italic: "Corsivo",
  "Bullet list": "Elenco puntato",
  "Add payment instructions, terms, or other notes...": "Aggiungi istruzioni di pagamento, condizioni o altre note...",
  Signature: "Firma",
  "Add signature text...": "Aggiungi testo della firma...",
  "Payment Terms": "Condizioni di pagamento",
  "Add payment terms...": "Aggiungi condizioni di pagamento...",
  Entity: "Azienda",
  Document: "Documento",
  Customer: "Cliente",
  "Company name": "Nome dell'azienda",
  "Email address": "Indirizzo email",
  "Invoice number": "Numero fattura",
  "Invoice date": "Data fattura",
  "Due date": "Data di scadenza",
  "Total amount": "Importo totale",
  Currency: "Valuta",
  "Select currency": "Seleziona valuta",
  "Customer name": "Nome del cliente",
  "Customer email": "Email del cliente",
  "Today's date": "Data odierna",
  "Current year": "Anno corrente",
  "Search or create customer...": "Cerca o crea cliente...",
  Clear: "Cancella",
  Address: "Indirizzo",
  "Address 2": "Indirizzo 2",
  "Post Code": "CAP",
  City: "Città",
  State: "Provincia",
  Country: "Paese",
  "Bank Account": "Conto bancario",
  "Account Type": "Tipo di conto",
  BIC: "BIC",
  IBAN: "IBAN",
  "US domestic": "Domestico USA",
  "UK domestic": "Domestico UK",
  "Account Number": "Numero di conto",
  "Routing Number": "Routing number",
  "Sort Code": "Sort code",
  "Company Number": "Numero aziendale",
  "UJP receiver identifier": "Identificativo destinatario UJP",
  "UJP receiver agent": "Agente destinatario UJP",
  "UJP receiver mailbox": "Casella destinatario UJP",
  "Tax Number": "Partita IVA",
  "Percentage discount": "Sconto percentuale",
  "Fixed amount discount": "Sconto fisso",
  Percentage: "Percentuale",
  "Fixed amount": "Importo fisso",
  "FURS Fiscalization": "FURS Fiscalizzazione",
  "Business Premise": "Sede commerciale",
  "Electronic Device": "Dispositivo elettronico",
  Premise: "Sede",
  Device: "Dispositivo",
  "Select premise": "Seleziona sede",
  "Select device": "Seleziona dispositivo",
  "Skip Fiscalization": "Salta la fiscalizzazione",
  "Skip fiscalization for this invoice (e.g., bank transfers or unpaid invoices)":
    "Salta la fiscalizzazione per questa fattura (es. bonifici bancari o fatture non pagate)",
  "Cannot skip fiscalization for cash payments":
    "Non è possibile saltare la fiscalizzazione per i pagamenti in contanti",
  Skipped: "Saltata",
  Enabled: "Abilitata",
  "Click to enable fiscalization": "Clicca per abilitare la fiscalizzazione",
  "Fiscally verify": "Verifica fiscale",
  "Click to skip fiscalization for this invoice": "Clicca per saltare la fiscalizzazione per questa fattura",
  "Business unit": "Unità aziendale",
  "No unit": "Nessuna unità",
  "e-SLOG": "e-SLOG",
  "Click to skip e-SLOG validation for this invoice": "Clicca per saltare la validazione e-SLOG per questa fattura",
  "Click to enable e-SLOG validation": "Clicca per abilitare la validazione e-SLOG",
  "e-SLOG Validation Failed": "Validazione e-SLOG non riuscita",
  "The following entity settings need to be updated:": "Le seguenti impostazioni dell'entità devono essere aggiornate:",
  "Entity name is required for e-SLOG. Update in Settings > Company.":
    "Il nome dell'entità è obbligatorio per e-SLOG. Aggiornalo in Impostazioni > Azienda.",
  "Entity address is required for e-SLOG. Update in Settings > Company.":
    "L'indirizzo dell'entità è obbligatorio per e-SLOG. Aggiornalo in Impostazioni > Azienda.",
  "Entity postal code is required for e-SLOG. Update in Settings > Company.":
    "Il codice postale dell'entità è obbligatorio per e-SLOG. Aggiornalo in Impostazioni > Azienda.",
  "Entity city is required for e-SLOG. Update in Settings > Company.":
    "La città dell'entità è obbligatoria per e-SLOG. Aggiornala in Impostazioni > Azienda.",
  "Entity must be from Slovenia (SI) for e-SLOG.": "L'entità deve essere della Slovenia (SI) per e-SLOG.",
  "Entity tax number is required for e-SLOG. Update in Settings > Company.":
    "Il numero fiscale dell'entità è obbligatorio per e-SLOG. Aggiornalo in Impostazioni > Azienda.",
  "Slovenian tax number must be 8 digits.": "Il numero fiscale sloveno deve essere di 8 cifre.",
  "Invoice date is required for e-SLOG.": "La data della fattura è obbligatoria per e-SLOG.",
  "Currency is required for e-SLOG.": "La valuta è obbligatoria per e-SLOG.",
  "Entity currency must be EUR for e-SLOG when invoice uses a different currency.":
    "La valuta dell'entità deve essere EUR per e-SLOG quando la fattura usa una valuta diversa.",
  "Customer name is required when customer is provided.":
    "Il nome del cliente è obbligatorio quando viene fornito un cliente.",
  "Entity IBAN is required for UJP package validation. Update in Settings > Company.":
    "L’IBAN dell’entità è obbligatorio per la validazione del pacchetto UJP. Aggiornalo in Impostazioni > Azienda.",
  "Entity IBAN must be a valid Slovenian IBAN for UJP package validation.":
    "L’IBAN dell’entità deve essere un IBAN sloveno valido per la validazione del pacchetto UJP.",
  "Entity BIC is required for UJP package validation. Update in Settings > Company.":
    "Il BIC dell’entità è obbligatorio per la validazione del pacchetto UJP. Aggiornalo in Impostazioni > Azienda.",
  "Entity BIC must be a valid 8- or 11-character BIC for UJP package validation.":
    "Il BIC dell’entità deve essere un BIC valido di 8 o 11 caratteri per la validazione del pacchetto UJP.",
  "Customer IBAN must be a valid Slovenian IBAN for UJP package validation.":
    "L’IBAN del cliente deve essere un IBAN sloveno valido per la validazione del pacchetto UJP.",
  "Customer BIC must be a valid 8- or 11-character BIC for UJP package validation.":
    "Il BIC del cliente deve essere un BIC valido di 8 o 11 caratteri per la validazione del pacchetto UJP.",
  "At least one line item is required for e-SLOG.": "Per e-SLOG è richiesta almeno una riga articolo.",
  "Item name is required for e-SLOG.": "Il nome dell'articolo è obbligatorio per e-SLOG.",
  "Quantity must be greater than 0.": "La quantità deve essere maggiore di 0.",
  "Price is required for e-SLOG.": "Il prezzo è obbligatorio per e-SLOG.",
  "Invalid Slovenian tax rate {{rate}}%. Valid: {{validRates}}":
    "Aliquota IVA slovena non valida {{rate}}%. Valide: {{validRates}}",
  "Reverse charge - tax exempt EU B2B sale": "Inversione contabile - vendita B2B UE esente da imposta",
  "Taxes disabled": "Imposte disabilitate",
  "Gross price": "Prezzo lordo",
  "Net price": "Prezzo netto",
  "Gross price (tax included)": "Prezzo lordo (imposte incluse)",
  "Net price (before tax)": "Prezzo netto (prima delle imposte)",
  "Linked documents": "Documenti collegati",
  Type: "Tipo",
  Total: "Totale",
  "Delivery note": "Bolla di consegna",
  Invoice: "Fattura",
  Estimate: "Preventivo",
  "Proforma invoice": "Fattura proforma",
  "Credit note": "Nota di credito",
  "Advance invoice": "Fattura di acconto",
  "FINA fiscalized invoices always use the current date":
    "Le fatture fiscalizzate FINA utilizzano sempre la data corrente",
  "On receipt": "Alla ricezione",
  "7 days": "7 giorni",
  "14 days": "14 giorni",
  "30 days": "30 giorni",
  "60 days": "60 giorni",
  "90 days": "90 giorni",
  Custom: "Personalizzato",
  "Service Date": "Data del servizio",
  "Service Period": "Periodo di servizio",
  From: "Da",
  To: "A",
  "Add separator": "Aggiungi separatore",
  "Section header": "Intestazione sezione",
  "Section title...": "Titolo sezione...",
  "Transaction type": "Tipo di transazione",
  Domestic: "Nazionale",
  "EU B2B": "EU B2B",
  "EU B2C": "EU B2C",
  "3W B2B": "3W B2B",
  "3W B2C": "3W B2C",
  "Determining transaction type...": "Determinazione tipo di transazione...",
  "This invoice will not be fiscalized (non-domestic transaction)":
    "Questa fattura non sarà fiscalizzata (transazione non nazionale)",
  "Tax Clause": "Clausola fiscale",
  "Add tax clause...": "Aggiungi clausola fiscale...",
  Footer: "Piè di pagina",
  "Add document footer...": "Aggiungi piè di pagina del documento...",
  "End consumer": "Consumatore finale",
  "Domestic B2B invoicing in Croatia is not supported":
    "La fatturazione B2B domestica in Croazia non è supportata. La Croazia richiede la conformità a Fiskalizacija 2.0 per la fatturazione B2B.",
  "FINA fiscalization must be enabled for domestic invoices":
    "La fiscalizzazione FINA deve essere abilitata per le fatture domestiche. Abilitarla nelle impostazioni dell'entità.",
  "Please select": "Seleziona",
  Amount: "Importo",
  "Add payment": "Aggiungi pagamento",
  "Recorded now": "Registrato ora",
  "Remaining due": "Residuo dovuto",
  "Remaining to allocate": "Residuo da allocare",
  "This document must be fully paid on creation": "Questo documento deve essere interamente pagato alla creazione",
  "Record one or more payments on creation": "Registra uno o più pagamenti alla creazione",
  "Entity bank account is required for e-SLOG payment instructions. Update in Settings > Company.":
    "Il conto bancario dell’entità è obbligatorio per le istruzioni di pagamento e-SLOG. Aggiornalo in Impostazioni > Azienda.",
  "Customer tax number is required for e-SLOG business recipients.":
    "Il codice fiscale del cliente è obbligatorio per i destinatari business e-SLOG.",
  "Customer address is required for UJP package validation.":
    "L’indirizzo del cliente è obbligatorio per la validazione del pacchetto UJP.",
  "Customer postal code is required for UJP package validation.":
    "Il codice postale del cliente è obbligatorio per la validazione del pacchetto UJP.",
  "Customer city is required for UJP package validation.":
    "La città del cliente è obbligatoria per la validazione del pacchetto UJP.",
  "Customer tax number or company number is required for UJP package validation.":
    "Il codice fiscale o il numero aziendale del cliente è obbligatorio per la validazione del pacchetto UJP.",
  "Customer IBAN and BIC are required for UJP package validation.":
    "IBAN e BIC del cliente sono obbligatori per la validazione del pacchetto UJP.",
  "Customer IBAN is required for UJP package validation.":
    "L’IBAN del cliente è obbligatorio per la validazione del pacchetto UJP.",
  "Customer BIC is required for UJP package validation.":
    "Il BIC del cliente è obbligatorio per la validazione del pacchetto UJP.",
  "French e-invoice": "Fattura elettronica francese",
  "French electronic delivery or reporting is required for this invoice":
    "La consegna elettronica o la comunicazione francese è obbligatoria per questa fattura",
  "Click to skip French electronic delivery or reporting for this invoice":
    "Fai clic per saltare la consegna elettronica o la comunicazione francese per questa fattura",
  "Click to enable French electronic delivery or reporting":
    "Fai clic per attivare la consegna elettronica o la comunicazione francese",
  "Buyer reference": "Riferimento acquirente",
  Required: "Obbligatorio",
  "Add a different delivery address": "Aggiungi un indirizzo di consegna diverso",
  "Delivery address": "Indirizzo di consegna",
  "Complete this only when goods are delivered somewhere other than the billing address.":
    "Compila solo se le merci vengono consegnate a un indirizzo diverso da quello di fatturazione.",
  "Country code": "Codice paese",
  // Slovenian Article 76.a
  "Article 76.a": "Articolo 76.a",
  "Click to disable Article 76.a reverse charge for this invoice":
    "Fai clic per disattivare l'inversione contabile dell'articolo 76.a per questa fattura",
  "Click to enable Article 76.a reverse charge for this invoice":
    "Fai clic per attivare l'inversione contabile dell'articolo 76.a per questa fattura",
} as const;
